 ...
 */
class AnalytesExport implements FromCollection, WithEvents, WithTitle
{
    protected $analytes;
    protected int $count;
    ...
    public function __construct($analytes, array $options = [], ?int $count = null)
    {
        ...
        $this->analytes = is_array($analytes) ? collect($analytes) : $analytes;
        ...
        $tz = $this->tz;
        $this->columnDefs = [
            'id'         => ['heading' => __('analytes.id'),        'value' => fn($c, $i) => $c->id],
            'name'       => ['heading' => __('analytes.name'),      'value' => fn($c, $i) => $c->name],
            'code'       => ['heading' => __('analytes.code'),      'value' => fn($c, $i) => $c->code],
            'sort_order' => ['heading' => __('analytes.sort_order'), 'value' => fn($c, $i) => $c->sort_order ?? ''],
            'is_active'  => ['heading' => __('analytes.is_active'), 'value' => fn($c, $i) => $c->state_text],
    ...
    {
        $title    = $this->options['title'] ?? __('analytes.export_title');
        $count    = $this->count;
        $subtitle = sprintf(
            '%s Â· %s Â· %s',
        ...
        // Row 5+ â€” data
        $i = 0;
        foreach ($this->analytes as $analyte) {
            $rows->push(array_map(
                fn($k) => $this->columnDefs[$k]['value']($analyte, $i),
                $this->activeColumns
            ));
            $i++;
        }
    ...
    {
        return mb_substr($this->options['title'] ?? __('analytes.export_title'), 0, 31);
    }
