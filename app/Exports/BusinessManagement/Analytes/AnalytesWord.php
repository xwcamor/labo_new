...
namespace App\Exports\BusinessManagement\Analytes;
...
/**
 * Generates a styled .docx report of Analytes using PhpWord directly
...
class AnalytesWord
{
    /** Map column key â†’ ['heading' => string, 'value' => fn($analyte) => mixed] */
    ...
    public function generate(
        ...
        $count = $count !== null
            ? $count
            : (is_countable($analytes) ? count($analytes) : iterator_count($analytes));

        $this->columnDefs = [
            'id'         => ['heading' => __('analytes.id'),        'value' => fn($c) => (string) $c->id],
            'name'       => ['heading' => __('analytes.name'),      'value' => fn($c) => (string) $c->name],
            'code'       => ['heading' => __('analytes.code'),      'value' => fn($c) => (string) $c->code],
            'sort_order' => ['heading' => __('analytes.sort_order'), 'value' => fn($c) => (string) ($c->sort_order ?? '')],
            'is_active'  => ['heading' => __('analytes.is_active'), 'value' => fn($c) => $c->state_text],
        ...
        $title         = $options['title']         ?? __('analytes.export_title');
        ...
        $analyteTable = $section->addTable([
            'cellMargin'   => 200,
            'borderSize'   => 0,
            'unit'         => \PhpOffice\PhpWord\SimpleType\TblWidth::TWIP,
        ]);
        $analyteTable->addRow(800);
        $analyteCell = $analyteTable->addCell(9000, [
            'bgColor' => self::COLOR_SHELL,
            'valign'  => 'center',
        ]);
        $analyteCell->addText($title, [
        ...
        if ($count === 0) {
            ...
            $phpWord->addTableStyle('AnalytesTable', [
                'borderSize'      => 4,
            ...
            $table = $section->addTable('AnalytesTable');
            ...
            foreach ($analytes as $i => $analyte) {
                ...
                foreach ($columns as $col) {
                    ...
                    $value = $this->columnDefs[$col]['value']($analyte);

                    if ($col === 'is_active') {
                        $color = $analyte->is_active ? '1D7044' : 'C8281D';
