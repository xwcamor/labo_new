 ...
 */
class EntryAuthorizersExport implements FromCollection, WithEvents, WithTitle
{
    protected $entry_authorizers;
    protected int $count;
    ...
    public function __construct($entry_authorizers, array $options = [], ?int $count = null)
    {
        ...
        $this->entry_authorizers = is_array($entry_authorizers) ? collect($entry_authorizers) : $entry_authorizers;
        ...
        $tz = $this->tz;
        $this->columnDefs = [
            'id'         => ['heading' => __('entry_authorizers.id'),        'value' => fn($c, $i) => $c->id],
            'name'       => ['heading' => __('entry_authorizers.name'),      'value' => fn($c, $i) => $c->name],
            'code'       => ['heading' => __('entry_authorizers.code'),      'value' => fn($c, $i) => $c->code],
            'sort_order' => ['heading' => __('entry_authorizers.sort_order'), 'value' => fn($c, $i) => $c->sort_order ?? ''],
            'is_active'  => ['heading' => __('entry_authorizers.is_active'), 'value' => fn($c, $i) => $c->state_text],
    ...
    {
        $title    = $this->options['title'] ?? __('entry_authorizers.export_title');
        $count    = $this->count;
        $subtitle = sprintf(
            '%s Â· %s Â· %s',
        ...
        // Row 5+ â€” data
        $i = 0;
        foreach ($this->entry_authorizers as $entryAuthorizer) {
            $rows->push(array_map(
                fn($k) => $this->columnDefs[$k]['value']($entryAuthorizer, $i),
                $this->activeColumns
            ));
            $i++;
        }
    ...
    {
        return mb_substr($this->options['title'] ?? __('entry_authorizers.export_title'), 0, 31);
    }
