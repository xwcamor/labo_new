...
namespace App\Exports\BusinessManagement\EntryAuthorizers;
...
/**
 * Generates a styled .docx report of EntryAuthorizers using PhpWord directly
 ...
 */
class EntryAuthorizersWord
{
    /** Map column key â†’ ['heading' => string, 'value' => fn($entryAuthorizer) => mixed] */
    ...
    public function generate(
        ...
        $count = $count !== null
            ? $count
            : (is_countable($entry_authorizers) ? count($entry_authorizers) : iterator_count($entry_authorizers));

        $this->columnDefs = [
            'id'         => ['heading' => __('entry_authorizers.id'),        'value' => fn($c) => (string) $c->id],
            'name'       => ['heading' => __('entry_authorizers.name'),      'value' => fn($c) => (string) $c->name],
            'code'       => ['heading' => __('entry_authorizers.code'),      'value' => fn($c) => (string) $c->code],
            'sort_order' => ['heading' => __('entry_authorizers.sort_order'), 'value' => fn($c) => (string) ($c->sort_order ?? '')],
            'is_active'  => ['heading' => __('entry_authorizers.is_active'), 'value' => fn($c) => $c->state_text],
        ...
        $title         = $options['title']         ?? __('entry_authorizers.export_title');
        ...
        $entryAuthorizerTable = $section->addTable([
            'cellMargin'   => 200,
            'borderSize'   => 0,
            'unit'         => \PhpOffice\PhpWord\SimpleType\TblWidth::TWIP,
        ]);
        $entryAuthorizerTable->addRow(800);
        $entryAuthorizerCell = $entryAuthorizerTable->addCell(9000, [
            'bgColor' => self::COLOR_SHELL,
            'valign'  => 'center',
        ]);
        $entryAuthorizerCell->addText($title, [
            'name' => 'Calibri', 'size' => 22, 'bold' => true, 'color' => 'FFFFFF',
        ], ['spaceAfter' => 60]);
        $entryAuthorizerCell->addText(
        ...
        if ($count === 0) {
            ...
            $phpWord->addTableStyle('EntryAuthorizersTable', [
                'borderSize'      => 4,
            ...
            $table = $section->addTable('EntryAuthorizersTable');
            ...
            // Data rows (zebra)
            foreach ($entry_authorizers as $i => $entryAuthorizer) {
                $table->addRow(360);
                $isEven = $i % 2 === 1;
                ...
                foreach ($columns as $col) {
                    ...
                    $value = $this->columnDefs[$col]['value']($entryAuthorizer);

                    if ($col === 'is_active') {
                        $color = $entryAuthorizer->is_active ? '1D7044' : 'C8281D';
