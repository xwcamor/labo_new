 ...
 */
class EquipmentExport implements FromCollection, WithEvents, WithTitle
{
    protected $equipment;
    protected int $count;
    ...
    public function __construct($equipment, array $options = [], ?int $count = null)
    {
        ...
        $this->equipment = is_array($equipment) ? collect($equipment) : $equipment;
    ...
    {
        $title    = $this->options['title'] ?? __('equipment.export_title');
        $count    = $this->count;
        $subtitle = sprintf(
            '%s Â· %s Â· %s',
        ...
        foreach ($this->equipment as $equipment) {
            $rows->push(array_map(
                fn($k) => $this->columnDefs[$k]['value']($equipment, $i),
                $this->activeColumns
            ));
            $i++;
        }
    ...
    {
        return mb_substr($this->options['title'] ?? __('equipment.export_title'), 0, 31);
    }
