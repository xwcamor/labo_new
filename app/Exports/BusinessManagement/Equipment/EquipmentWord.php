...
namespace App\Exports\BusinessManagement\Equipments;
...
/**
 * Generates a styled .docx report of Equipment using PhpWord directly
...
class EquipmentWord
{
    /** Map column key â†’ ['heading' => string, 'value' => fn($equipment) => mixed] */
    ...
    public function generate(
        ...
        $count = $count !== null
            ? $count
            : (is_countable($equipment) ? count($equipment) : iterator_count($equipment));
        ...
        $title         = $options['title']         ?? __('equipment.export_title');
        ...
        $equipmentTable->addRow(800);
        $equipmentCell = $equipmentTable->addCell(9000, [
            'bgColor' => self::COLOR_SHELL,
            'valign'  => 'center',
        ]);
        $equipmentCell->addText($title, [
            'name' => 'Calibri', 'size' => 22, 'bold' => true, 'color' => 'FFFFFF',
        ], ['spaceAfter' => 60]);
        $equipmentCell->addText(
        ...
        if ($count === 0) {
            ...
            $phpWord->addTableStyle('EquipmentTable', [
                'borderSize'      => 4,
            ...
            // Data rows (zebra)
            foreach ($equipment as $i => $equipment) {
                $table->addRow(360);
                $isEven = $i % 2 === 1;
                ...
                foreach ($columns as $col) {
                    ...
                    $value = $this->columnDefs[$col]['value']($equipment);

                    if ($col === 'is_active') {
                        $color = $equipment->is_active ? '1D7044' : 'C8281D';
