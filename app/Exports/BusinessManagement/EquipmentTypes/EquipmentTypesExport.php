 ...
 */
class EquipmentTypesExport implements FromCollection, WithEvents, WithTitle
{
    ...
    public function __construct($equipmentTypes, array $options = [], ?int $count = null)
    {
        ...
        $this->equipmentTypes = is_array($equipmentTypes) ? collect($equipmentTypes) : $equipmentTypes;
        ...
        $tz = $this->tz;
        $this->columnDefs = [
            'id'         => ['heading' => __('equipment_types.id'),        'value' => fn($c, $i) => $c->id],
            'name'       => ['heading' => __('equipment_types.name'),      'value' => fn($c, $i) => $c->name],
            'code'       => ['heading' => __('equipment_types.code'),      'value' => fn($c, $i) => $c->code],
            'sort_order' => ['heading' => __('equipment_types.sort_order'), 'value' => fn($c, $i) => $c->sort_order ?? ''],
            'is_active'  => ['heading' => __('equipment_types.is_active'), 'value' => fn($c, $i) => $c->state_text],
    ...
    {
        $title    = $this->options['title'] ?? __('equipment_types.export_title');
        $count    = $this->count;
        $subtitle = sprintf(
            '%s Â· %s Â· %s',
        ...
        foreach ($this->equipmentTypes as $equipmentType) {
            $rows->push(array_map(
                fn($k) => $this->columnDefs[$k]['value']($equipmentType, $i),
                $this->activeColumns
            ));
            $i++;
        }
    ...
    {
        return mb_substr($this->options['title'] ?? __('equipment_types.export_title'), 0, 31);
    }
