...
/**
 * Generates a styled .docx report of EquipmentTypes using PhpWord directly
 ...
 */
class EquipmentTypesWord
{
    /** Map column key â†’ ['heading' => string, 'value' => fn($equipmentType) => mixed] */
    ...
    public function generate(
        ...
        $count = $count !== null
            ? $count
            : (is_countable($equipmentTypes) ? count($equipmentTypes) : iterator_count($equipmentTypes));

        $this->columnDefs = [
            'id'         => ['heading' => __('equipment_types.id'),        'value' => fn($c) => (string) $c->id],
            'name'       => ['heading' => __('equipment_types.name'),      'value' => fn($c) => (string) $c->name],
            'code'       => ['heading' => __('equipment_types.code'),      'value' => fn($c) => (string) $c->code],
            'sort_order' => ['heading' => __('equipment_types.sort_order'), 'value' => fn($c) => (string) ($c->sort_order ?? '')],
            'is_active'  => ['heading' => __('equipment_types.is_active'), 'value' => fn($c) => $c->state_text],
        ...
        $title         = $options['title']         ?? __('equipment_types.export_title');
        ...
        if ($count === 0) {
            ...
            $phpWord->addTableStyle('EquipmentTypesTable', [
                'borderSize'      => 4,
            ...
            $table = $section->addTable('EquipmentTypesTable');
            ...
            // Data rows (zebra)
            foreach ($equipmentTypes as $i => $equipmentType) {
                $table->addRow(360);
                $isEven = $i % 2 === 1;
                ...
                foreach ($columns as $col) {
                    ...
                    $value = $this->columnDefs[$col]['value']($equipmentType);

                    if ($col === 'is_active') {
                        $color = $equipmentType->is_active ? '1D7044' : 'C8281D';
