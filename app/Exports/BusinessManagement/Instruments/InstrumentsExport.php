 ...
 */
class InstrumentsExport implements FromCollection, WithEvents, WithTitle
{
    ...
    public function __construct($instruments, array $options = [], ?int $count = null)
    {
        ...
        $this->instruments = is_array($instruments) ? collect($instruments) : $instruments;
        ...
        $this->columnDefs = [
            ...
            'calibrated_at'      => ['heading' => __('instruments.calibrated_at'),      'value' => fn($c, $i) => $c->calibrated_at?->format('Y-m-d') ?? ''],
            'calibration_due_at' => ['heading' => __('instruments.calibration_due_at'), 'value' => fn($c, $i) => $c->calibration_due_at?->format('Y-m-d') ?? ''],
            'calibration_status' => ['heading' => __('instruments.calibration_status'), 'value' => fn($c, $i) => $c->calibration_status_text],
            'calibration_certificate' => ['heading' => __('instruments.calibration_certificate'), 'value' => fn($c, $i) => $c->calibration_certificate ?? ''],
            'location'   => ['heading' => __('instruments.location'),  'value' => fn($c, $i) => $c->location ?? ''],
            'notes'      => ['heading' => __('instruments.notes'),     'value' => fn($c, $i) => $c->notes ?? ''],
            'sort_order' => ['heading' => __('instruments.sort_order'), 'value' => fn($c, $i) => $c->sort_order ?? ''],
            'is_active'  => ['heading' => __('instruments.is_active'), 'value' => fn($c, $i) => $c->state_text],
    ...
    public function collection()
    {
        ...
        foreach ($this->instruments as $instrument) {
            $rows->push(array_map(
                fn($k) => $this->columnDefs[$k]['value']($instrument, $i),
                $this->activeColumns
            ));
            $i++;
        }
    ...
    {
        return mb_substr($this->options['title'] ?? __('instruments.export_title'), 0, 31);
    }
