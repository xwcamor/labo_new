 ...
 */
class SamplersExport implements FromCollection, WithEvents, WithTitle
{
    protected $samplers;
    protected int $count;
    ...
    public function __construct($samplers, array $options = [], ?int $count = null)
    {
        ...
        $this->samplers = is_array($samplers) ? collect($samplers) : $samplers;
        ...
        $tz = $this->tz;
        $this->columnDefs = [
            'id'         => ['heading' => __('samplers.id'),        'value' => fn($c, $i) => $c->id],
            'name'       => ['heading' => __('samplers.name'),      'value' => fn($c, $i) => $c->name],
            'code'       => ['heading' => __('samplers.code'),      'value' => fn($c, $i) => $c->code],
            'sort_order' => ['heading' => __('samplers.sort_order'), 'value' => fn($c, $i) => $c->sort_order ?? ''],
            'is_active'  => ['heading' => __('samplers.is_active'), 'value' => fn($c, $i) => $c->state_text],
    ...
    {
        $title    = $this->options['title'] ?? __('samplers.export_title');
        $count    = $this->count;
        $subtitle = sprintf(
            '%s Â· %s Â· %s',
        ...
        foreach ($this->samplers as $sampler) {
            $rows->push(array_map(
                fn($k) => $this->columnDefs[$k]['value']($sampler, $i),
                $this->activeColumns
            ));
            $i++;
        }
    ...
    {
        return mb_substr($this->options['title'] ?? __('samplers.export_title'), 0, 31);
    }
