...
namespace App\Exports\BusinessManagement\Samplers;
...
/**
 * Generates a styled .docx report of Samplers using PhpWord directly
...
class SamplersWord
{
    /** Map column key â†’ ['heading' => string, 'value' => fn($sampler) => mixed] */
    ...
    public function generate(
        ...
        $count = $count !== null
            ? $count
            : (is_countable($samplers) ? count($samplers) : iterator_count($samplers));

        $this->columnDefs = [
            'id'         => ['heading' => __('samplers.id'),        'value' => fn($c) => (string) $c->id],
            'name'       => ['heading' => __('samplers.name'),      'value' => fn($c) => (string) $c->name],
            'code'       => ['heading' => __('samplers.code'),      'value' => fn($c) => (string) $c->code],
            'sort_order' => ['heading' => __('samplers.sort_order'), 'value' => fn($c) => (string) ($c->sort_order ?? '')],
            'is_active'  => ['heading' => __('samplers.is_active'), 'value' => fn($c) => $c->state_text],
        ...
        $title         = $options['title']         ?? __('samplers.export_title');
        ...
        $samplerTable = $section->addTable([
            'cellMargin'   => 200,
            'borderSize'   => 0,
            'unit'         => \PhpOffice\PhpWord\SimpleType\TblWidth::TWIP,
        ]);
        $samplerTable->addRow(800);
        $samplerCell = $samplerTable->addCell(9000, [
            'bgColor' => self::COLOR_SHELL,
            'valign'  => 'center',
        ]);
        $samplerCell->addText($title, [
        ...
        if ($count === 0) {
            ...
            $phpWord->addTableStyle('SamplersTable', [
                'borderSize'      => 4,
            ...
            $table = $section->addTable('SamplersTable');
            ...
            foreach ($samplers as $i => $sampler) {
                ...
                foreach ($columns as $col) {
                    ...
                    $value = $this->columnDefs[$col]['value']($sampler);

                    if ($col === 'is_active') {
                        $color = $sampler->is_active ? '1D7044' : 'C8281D';
