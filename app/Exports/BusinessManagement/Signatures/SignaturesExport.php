 ...
 */
class SignaturesExport implements FromCollection, WithEvents, WithTitle
{
    ...
    public function __construct($signatures, array $options = [], ?int $count = null)
    {
        ...
        $this->signatures = is_array($signatures) ? collect($signatures) : $signatures;
        ...
        $tz = $this->tz;
        $this->columnDefs = [
            'id'         => ['heading' => __('signatures.id'),        'value' => fn($c, $i) => $c->id],
            'name'       => ['heading' => __('signatures.name'),      'value' => fn($c, $i) => $c->name],
            'code'       => ['heading' => __('signatures.code'),      'value' => fn($c, $i) => $c->code],
            'sort_order' => ['heading' => __('signatures.sort_order'), 'value' => fn($c, $i) => $c->sort_order ?? ''],
            'is_active'  => ['heading' => __('signatures.is_active'), 'value' => fn($c, $i) => $c->state_text],
    ...
    {
        $title    = $this->options['title'] ?? __('signatures.export_title');
        $count    = $this->count;
        $subtitle = sprintf(
            '%s Â· %s Â· %s',
        ...
        foreach ($this->signatures as $signature) {
            $rows->push(array_map(
                fn($k) => $this->columnDefs[$k]['value']($signature, $i),
                $this->activeColumns
            ));
            $i++;
        }
    ...
    {
        return mb_substr($this->options['title'] ?? __('signatures.export_title'), 0, 31);
    }
