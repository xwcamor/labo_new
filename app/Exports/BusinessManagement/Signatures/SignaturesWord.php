...
namespace App\Exports\BusinessManagement\Signatures;
...
/**
 * Generates a styled .docx report of Signatures using PhpWord directly
 ...
 */
class SignaturesWord
{
    /** Map column key â†’ ['heading' => string, 'value' => fn($signature) => mixed] */
    ...
    public function generate(
        ...
        $count = $count !== null
            ? $count
            : (is_countable($signatures) ? count($signatures) : iterator_count($signatures));

        $this->columnDefs = [
            'id'         => ['heading' => __('signatures.id'),        'value' => fn($c) => (string) $c->id],
            'name'       => ['heading' => __('signatures.name'),      'value' => fn($c) => (string) $c->name],
            'code'       => ['heading' => __('signatures.code'),      'value' => fn($c) => (string) $c->code],
            'sort_order' => ['heading' => __('signatures.sort_order'), 'value' => fn($c) => (string) ($c->sort_order ?? '')],
            'is_active'  => ['heading' => __('signatures.is_active'), 'value' => fn($c) => $c->state_text],
        ...
        $title         = $options['title']         ?? __('signatures.export_title');
        ...
        $signatureCell = $signatureTable->addCell(9000, [
            'bgColor' => self::COLOR_SHELL,
            'valign'  => 'center',
        ]);
        $signatureCell->addText($title, [
        ...
        if ($count === 0) {
            ...
            $phpWord->addTableStyle('SignaturesTable', [
                'borderSize'      => 4,
            ...
            foreach ($signatures as $i => $signature) {
                ...
                foreach ($columns as $col) {
                    ...
                    $value = $this->columnDefs[$col]['value']($signature);

                    if ($col === 'is_active') {
                        $color = $signature->is_active ? '1D7044' : 'C8281D';
