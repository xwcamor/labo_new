 ...
 */
class TestDefinitionsExport implements FromCollection, WithEvents, WithTitle
{
    protected $test_definitions;
    protected int $count;
    ...
    public function __construct($test_definitions, array $options = [], ?int $count = null)
    {
        ...
        $this->test_definitions = is_array($test_definitions) ? collect($test_definitions) : $test_definitions;
        ...
        $tz = $this->tz;
        $this->columnDefs = [
            'id'         => ['heading' => __('test_definitions.id'),        'value' => fn($c, $i) => $c->id],
            'name'       => ['heading' => __('test_definitions.name'),      'value' => fn($c, $i) => $c->name],
            'code'       => ['heading' => __('test_definitions.code'),      'value' => fn($c, $i) => $c->code],
            'group'      => ['heading' => __('test_definitions.group'),     'value' => fn($c, $i) => $c->group?->name ?? ''],
            'container'  => ['heading' => __('test_definitions.container'),  'value' => fn($c, $i) => $c->container ?? ''],
            'chart_unit' => ['heading' => __('test_definitions.chart_unit'), 'value' => fn($c, $i) => $c->chart_unit ?? ''],
            'has_control'        => ['heading' => __('test_definitions.has_control'),        'value' => fn($c, $i) => __($c->has_control ? 'global.yes' : 'global.no')],
            'requires_control'   => ['heading' => __('test_definitions.requires_control'),   'value' => fn($c, $i) => __($c->requires_control ? 'global.yes' : 'global.no')],
            'requires_duplicate' => ['heading' => __('test_definitions.requires_duplicate'), 'value' => fn($c, $i) => __($c->requires_duplicate ? 'global.yes' : 'global.no')],
            'is_grouped'         => ['heading' => __('test_definitions.is_grouped'),         'value' => fn($c, $i) => __($c->is_grouped ? 'global.yes' : 'global.no')],
            'replicates'  => ['heading' => __('test_definitions.replicates'),  'value' => fn($c, $i) => $c->replicates ?? 1],
            'description' => ['heading' => __('test_definitions.description'), 'value' => fn($c, $i) => $c->description ?? ''],
            'legacy_id'   => ['heading' => __('test_definitions.legacy_id'),   'value' => fn($c, $i) => $c->legacy_id ?? ''],
            'sort_order' => ['heading' => __('test_definitions.sort_order'), 'value' => fn($c, $i) => $c->sort_order ?? ''],
            'is_active'  => ['heading' => __('test_definitions.is_active'), 'value' => fn($c, $i) => $c->state_text],
    ...
    public function collection()
    {
        ...
        // Row 5+ — data
        $i = 0;
        foreach ($this->test_definitions as $testDefinition) {
            $rows->push(array_map(
                fn($k) => $this->columnDefs[$k]['value']($testDefinition, $i),
                $this->activeColumns
            ));
            $i++;
        }
    ...
    {
        return mb_substr($this->options['title'] ?? __('test_definitions.export_title'), 0, 31);
    }
