 ...
 */
class TestGroupsExport implements FromCollection, WithEvents, WithTitle
{
    ...
    public function __construct($test_groups, array $options = [], ?int $count = null)
    {
        ...
        $this->test_groups = is_array($test_groups) ? collect($test_groups) : $test_groups;
        ...
        $tz = $this->tz;
        $this->columnDefs = [
            'id'         => ['heading' => __('test_groups.id'),        'value' => fn($c, $i) => $c->id],
            'name'       => ['heading' => __('test_groups.name'),      'value' => fn($c, $i) => $c->name],
            'code'       => ['heading' => __('test_groups.code'),      'value' => fn($c, $i) => $c->code],
            // Cuantas pruebas cuelgan del grupo. El job solo la carga si
            // la pidieron, de ahi el ?? ''.
            'tests_count' => ['heading' => __('test_groups.tests_count'), 'value' => fn($c, $i) => $c->tests_count ?? ''],
            'sort_order' => ['heading' => __('test_groups.sort_order'), 'value' => fn($c, $i) => $c->sort_order ?? ''],
            'is_active'  => ['heading' => __('test_groups.is_active'), 'value' => fn($c, $i) => $c->state_text],
    ...
    {
        $title    = $this->options['title'] ?? __('test_groups.export_title');
        $count    = $this->count;
        $subtitle = sprintf(
            '%s · %s · %s',
        ...
        // Row 5+ — data
        $i = 0;
        foreach ($this->test_groups as $testGroup) {
            $rows->push(array_map(
                fn($k) => $this->columnDefs[$k]['value']($testGroup, $i),
                $this->activeColumns
            ));
            $i++;
        }
    ...
    {
        return mb_substr($this->options['title'] ?? __('test_groups.export_title'), 0, 31);
    }
