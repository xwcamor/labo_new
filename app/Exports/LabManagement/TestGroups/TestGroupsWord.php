...
/**
 * Generates a styled .docx report of TestGroups using PhpWord directly
 ...
 */
class TestGroupsWord
{
    /** Map column key → ['heading' => string, 'value' => fn($testGroup) => mixed] */
    ...
    public function generate(
        ...
        $count = $count !== null
            ? $count
            : (is_countable($test_groups) ? count($test_groups) : iterator_count($test_groups));

        $this->columnDefs = [
            'id'         => ['heading' => __('test_groups.id'),        'value' => fn($c) => (string) $c->id],
            'name'       => ['heading' => __('test_groups.name'),      'value' => fn($c) => (string) $c->name],
            'code'       => ['heading' => __('test_groups.code'),      'value' => fn($c) => (string) $c->code],
            // Cuantas pruebas cuelgan del grupo. El job solo la carga si
            // la pidieron, de ahi el ?? ''.
            'tests_count' => ['heading' => __('test_groups.tests_count'), 'value' => fn($c) => (string) ($c->tests_count ?? '')],
            'sort_order' => ['heading' => __('test_groups.sort_order'), 'value' => fn($c) => (string) ($c->sort_order ?? '')],
            'is_active'  => ['heading' => __('test_groups.is_active'), 'value' => fn($c) => $c->state_text],
        ...
        $title         = $options['title']         ?? __('test_groups.export_title');
        ...
        // ── COVER ────────────────────────────────────────────────────────
        $testGroupTable = $section->addTable([
            'cellMargin'   => 200,
            'borderSize'   => 0,
            'unit'         => \PhpOffice\PhpWord\SimpleType\TblWidth::TWIP,
        ]);
        $testGroupTable->addRow(800);
        $testGroupCell = $testGroupTable->addCell(9000, [
            'bgColor' => self::COLOR_SHELL,
            'valign'  => 'center',
        ]);
        $testGroupCell->addText($title, [
            'name' => 'Calibri', 'size' => 22, 'bold' => true, 'color' => 'FFFFFF',
        ], ['spaceAfter' => 60]);
        $testGroupCell->addText(
        ...
        if ($count === 0) {
            ...
            $phpWord->addTableStyle('TestGroupsTable', [
                'borderSize'      => 4,
            ...
            $table = $section->addTable('TestGroupsTable');
            ...
            // Data rows (zebra)
            foreach ($test_groups as $i => $testGroup) {
                $table->addRow(360);
                $isEven = $i % 2 === 1;
                ...
                foreach ($columns as $col) {
                    ...
                    $value = $this->columnDefs[$col]['value']($testGroup);

                    if ($col === 'is_active') {
                        $color = $testGroup->is_active ? '1D7044' : 'C8281D';
