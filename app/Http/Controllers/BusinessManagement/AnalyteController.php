...
use App\Http\Requests\BusinessManagement\Analyte\BulkDeleteAnalyteRequest;
use App\Http\Requests\BusinessManagement\Analyte\BulkRestoreAnalyteRequest;
use App\Http\Requests\BusinessManagement\Analyte\BulkSetActiveAnalyteRequest;
use App\Http\Requests\BusinessManagement\Analyte\DeleteAnalyteRequest;
use App\Http\Requests\BusinessManagement\Analyte\EditAllUpdateAnalyteRequest;
use App\Http\Requests\BusinessManagement\Analyte\ForceDeleteAnalyteRequest;
use App\Http\Requests\BusinessManagement\Analyte\ImportAnalyteRequest;
use App\Http\Requests\BusinessManagement\Analyte\StoreAnalyteRequest;
use App\Http\Requests\BusinessManagement\Analyte\UpdateAnalyteRequest;
use App\Http\Resources\AuditLogResource;
use App\Jobs\BusinessManagement\Analytes\GenerateAnalytesCsvJob;
use App\Jobs\BusinessManagement\Analytes\GenerateAnalytesExcelJob;
use App\Jobs\BusinessManagement\Analytes\GenerateAnalytesPdfJob;
use App\Jobs\BusinessManagement\Analytes\GenerateAnalytesWordJob;
use App\Models\AuditLog;
use App\Models\Analyte;
use App\Services\BusinessManagement\AnalyteService;
...
class AnalyteController extends Controller
{
    ...
    public function lock(Request $request, Analyte $analyte): RedirectResponse
    {
        return $this->applyLock($analyte, $request);
    }

    /** Saca el candado (un admin no puede quitar un candado del super). */
    public function unlock(Request $request, Analyte $analyte): RedirectResponse
    {
        return $this->applyUnlock($analyte, $request);
    }
    ...
    public function index(Request $request)
    {
        ...
        // analytes es per-tenant (BelongsToTenant lo scopea solo) — eager-load creator.
        ...
        $totalUnfiltered = Analyte::count();

        $names = $request->get('name', []);
        if (is_string($names)) $names = $names === '' ? [] : [$names];

        return inertia('Analytes/Index', [
            'analytes' => array_merge($analytes->toArray(), [
            ...
            'exportLimits' => \App\Models\Setting::getExportLimits('analytes'),
            'filters' => [
            ...
            'filterSchema'   => Analyte::filterSchema(),
        ]);
    }
    ...
    public function show(Request $request, Analyte $analyte)
    {
        $analyte->load(['creator:id,name,email', 'deleter:id,name,email', 'locker:id,name']);

        $canSeeAudit = $request->user()?->hasAnyRole(['super', 'admin']) ?? false;
        $activity = $canSeeAudit
            ? AuditLogResource::collection(
                AuditLog::query()
                    ->where('auditable_type', Analyte::class)
                    ->where('auditable_id', $analyte->id)
        ...
        return inertia('Analytes/Show', [
            'analyte' => array_merge(
                $this->payload($analyte, withAudit: true),
                ['lock' => $this->lockMeta($analyte, $request)],
            ),
            'recordAudit'  => $this->recordAuditMeta($analyte),
            'activity'     => $activity,
        ]);
    }
    ...
    {
        return inertia('Analytes/Form', [
            'analyte'        => null,
        ]);
    }

    public function store(StoreAnalyteRequest $request, AnalyteService $service): RedirectResponse
    {
        ...
        return redirect()
            ->route('business_management.analytes.index')
            ->with('success', __('analytes.created'));
    }
     ...
     * Gated por permission:analytes.create (super/admin pasan por sus permisos).
     */
    public function quickStore(StoreAnalyteRequest $request, AnalyteService $service): \Illuminate\Http\JsonResponse
    {
        $tenant = $request->user()?->tenant;
        if ($tenant) {
            $max = $tenant->maxRecordsPerModule();
        ...
        $analyte = $service->create($request->validated());

        return response()->json(['id' => $analyte->id, 'name' => $analyte->name], 201);
    }

    public function edit(Analyte $analyte)
    {
        // Registro bloqueado (Lockable): ni se abre el formulario de edición.
        abort_if($analyte->is_locked, 403, __('locks.cannot_edit_locked'));

        return inertia('Analytes/Form', [
            'analyte'        => $this->payload($analyte),
        ]);
    }


    public function update(UpdateAnalyteRequest $request, Analyte $analyte, AnalyteService $service): RedirectResponse
    {
        $service->update($analyte, $request->validated());

        return redirect()
            ->route('business_management.analytes.index')
            ->with('success', __('analytes.saved'));
    }

    public function delete(Analyte $analyte)
    {
        // Registro bloqueado (Lockable): ni se abre la confirmación de borrado.
        abort_if($analyte->is_locked, 403, __('locks.cannot_delete_locked'));

        return inertia('Analytes/Delete', [
            'analyte' => $this->payload($analyte),
        ]);
    }

    public function deleteSave(DeleteAnalyteRequest $request, Analyte $analyte, AnalyteService $service): RedirectResponse
    {
        $service->delete($analyte, $request->validated()['deleted_description']);

        $this->storeUndoableDelete([$analyte->id]);

        return redirect()
            ->route('business_management.analytes.index')
            ->with('success', __('global.deleted_success'))
            ->with('recentDelete', $this->buildRecentDeletePayload([$analyte->id]));
    }

    /** Persiste el claim en sesion por el window de undo (60s). */
    protected function storeUndoableDelete(array $ids): void
    {
        session(['analytes.recent_delete' => [
    ...
    public function restore(Request $request, $slug, AnalyteService $service): RedirectResponse
    {
        abort_unless($request->user()?->hasRole('super'), 403);
        $model = Analyte::onlyTrashed()->where('slug', $slug)->firstOrFail();
        $service->restore($model);

        return redirect()
            ->route('business_management.analytes.trash')
    ...
    public function editAll(Request $request)
    {
        ...
        $analytes = Analyte::query()
            ->filter($request)
            ->select('analytes.id', 'analytes.slug', 'analytes.name', 'analytes.code', 'analytes.is_active')
            ->paginate($perPage)
            ->withQueryString();

        return inertia('Analytes/EditAll', [
            'analytes' => $analytes,
            'filters'   => [
                'name'      => $request->get('name', ''),
                'is_active' => $request->filled('is_active')
    ...
    public function editAllUpdate(EditAllUpdateAnalyteRequest $request, AnalyteService $service): RedirectResponse
    {
        $changes = $request->validated()['changes'];

        // Excluir registros BLOQUEADOS (Lockable) de la edición masiva.
        $ids = array_column($changes, 'id');
        [, $lockedIds] = $this->splitLockedIds(Analyte::class, $ids);
        if (!empty($lockedIds)) {
        ...
        return redirect()
            ->route('business_management.analytes.edit_all')
            ->with('success', $msg);
    }
     ...
     */
    public function duplicate(Request $request, Analyte $analyte, AnalyteService $service): RedirectResponse
    {
        ...
        $clone = $service->duplicate($analyte);
    ...
    public function bulkRestore(BulkRestoreAnalyteRequest $request, AnalyteService $service): RedirectResponse
    {
    ...
    public function forceDelete(ForceDeleteAnalyteRequest $request, $slug, AnalyteService $service): RedirectResponse
    {
        $model = Analyte::onlyTrashed()->where('slug', $slug)->firstOrFail();
    ...
    protected function payload(Analyte $m, bool $withAudit = false): array
    {
        $base = [
            'id'         => $m->id,
            'slug'       => $m->slug,
    ...
    {
        return $this->dispatchExport($request, 'csv', GenerateAnalytesCsvJob::class);
    }

    public function exportExcel(Request $request)
    {
        return $this->dispatchExport($request, 'excel', GenerateAnalytesExcelJob::class);
    }

    public function exportPdf(Request $request)
    {
        return $this->dispatchExport($request, 'pdf', GenerateAnalytesPdfJob::class);
    }

    public function exportWord(Request $request)
    {
        return $this->dispatchExport($request, 'word', GenerateAnalytesWordJob::class);
    }
    ...
    protected function assertExportLimit(string $format, array $options): void
    {
        ...
        $limit = \App\Models\Setting::getExportLimit('analytes', $format);
        if ($limit === 0) return; // CSV streaming, sin limite

        $count = $this->countForExport($options);
        if ($count > $limit) {
            abort(422, __('analytes.export_limit_exceeded', [
    ...
    public function importTemplate()
    {
        return \Maatwebsite\Excel\Facades\Excel::download(
            new \App\Exports\BusinessManagement\Analytes\AnalytesImportTemplate(),
            __('analytes.import_template_filename')
        );
    }

    public function import(ImportAnalyteRequest $request)
    {
        $data    = $request->validated();
        ...
        // el lookup por nombre case-insensitive matchearÃ­a analytes de cualquier
        ...
        if ($user && $user->hasRole('super') && empty($user->tenant_id)) {
            return response()->json([
                'ok'      => false,
                'dry_run' => $dryRun,
                'message' => __('analytes.import_super_blocked', [], 'Super sin workspace asignado no puede importar â€” el match por nombre puede actualizar registros de otro tenant.'),
            ], 422);
        }

        $importer = new \App\Imports\BusinessManagement\Analytes\AnalytesImport(
            mode:   $mode,
            dryRun: $dryRun,
        );
        ...
        try {
            ...
            \Log::error('AnalytesImport failed', [
                'error' => $e->getMessage(),
    ...
    public function bulkDelete(BulkDeleteAnalyteRequest $request, AnalyteService $service): RedirectResponse
    {
        $data = $request->validated();

        // Excluir registros BLOQUEADOS (Lockable): no se borran en masa.
        [$allowedIds, $lockedIds] = $this->splitLockedIds(Analyte::class, $data['ids']);
        if (empty($allowedIds)) {
     ...
     */
    public function undoLastDelete(Request $request, AnalyteService $service): RedirectResponse
    {
        $claim = session('analytes.recent_delete');
        if (!$claim || !is_array($claim) || empty($claim['ids']) || empty($claim['expires_at'])) {
            return back()->with('error', __('global.undo_failed'));
        }
        if (now()->isAfter($claim['expires_at'])) {
            session()->forget('analytes.recent_delete');
            return back()->with('error', __('global.undo_failed'));
        }
    ...
    public function bulkSetActive(BulkSetActiveAnalyteRequest $request, AnalyteService $service): RedirectResponse
    {
        $data = $request->validated();
    ...
    protected function buildExportOptions(Request $request, string $format): array
    {
        ...
        return [
            ...
            'title'                   => $data['title']                   ?? __('analytes.export_title'),
    ...
    protected function recordExportAudit(string $format, array $options): void
    {
        AuditLog::create([
            ...
            'url'        => route('business_management.analytes.index'),
