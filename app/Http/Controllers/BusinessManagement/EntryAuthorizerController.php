...
use App\Http\Requests\BusinessManagement\EntryAuthorizer\BulkDeleteEntryAuthorizerRequest;
use App\Http\Requests\BusinessManagement\EntryAuthorizer\BulkRestoreEntryAuthorizerRequest;
use App\Http\Requests\BusinessManagement\EntryAuthorizer\BulkSetActiveEntryAuthorizerRequest;
use App\Http\Requests\BusinessManagement\EntryAuthorizer\DeleteEntryAuthorizerRequest;
use App\Http\Requests\BusinessManagement\EntryAuthorizer\EditAllUpdateEntryAuthorizerRequest;
use App\Http\Requests\BusinessManagement\EntryAuthorizer\ForceDeleteEntryAuthorizerRequest;
use App\Http\Requests\BusinessManagement\EntryAuthorizer\ImportEntryAuthorizerRequest;
use App\Http\Requests\BusinessManagement\EntryAuthorizer\StoreEntryAuthorizerRequest;
use App\Http\Requests\BusinessManagement\EntryAuthorizer\UpdateEntryAuthorizerRequest;
use App\Http\Resources\AuditLogResource;
use App\Jobs\BusinessManagement\EntryAuthorizers\GenerateEntryAuthorizersCsvJob;
use App\Jobs\BusinessManagement\EntryAuthorizers\GenerateEntryAuthorizersExcelJob;
use App\Jobs\BusinessManagement\EntryAuthorizers\GenerateEntryAuthorizersPdfJob;
use App\Jobs\BusinessManagement\EntryAuthorizers\GenerateEntryAuthorizersWordJob;
use App\Models\AuditLog;
use App\Models\EntryAuthorizer;
use App\Services\BusinessManagement\EntryAuthorizerService;
use Illuminate\Http\RedirectResponse;
use Illuminate\Http\Request;

class EntryAuthorizerController extends Controller
{
    ...
    public function lock(Request $request, EntryAuthorizer $entryAuthorizer): RedirectResponse
    {
        return $this->applyLock($entryAuthorizer, $request);
    }

    /** Saca el candado (un admin no puede quitar un candado del super). */
    public function unlock(Request $request, EntryAuthorizer $entryAuthorizer): RedirectResponse
    {
        return $this->applyUnlock($entryAuthorizer, $request);
    }
    ...
    public function index(Request $request)
    {
        ...
        // entry_authorizers es per-tenant (BelongsToTenant lo scopea solo) — eager-load creator.
        ...
        $entry_authorizers = EntryAuthorizer::query()
            ->select('entry_authorizers.*')
            ->with($with)
        ...
        $totalUnfiltered = EntryAuthorizer::count();

        $names = $request->get('name', []);
        if (is_string($names)) $names = $names === '' ? [] : [$names];

        return inertia('EntryAuthorizers/Index', [
            'entry_authorizers' => array_merge($entry_authorizers->toArray(), [
            ...
            'exportLimits' => \App\Models\Setting::getExportLimits('entry_authorizers'),
            'filters' => [
            ...
            'filterSchema'   => EntryAuthorizer::filterSchema(),
        ]);
    }
    ...
    public function show(Request $request, EntryAuthorizer $entryAuthorizer)
    {
        $entryAuthorizer->load(['creator:id,name,email', 'deleter:id,name,email', 'locker:id,name']);

        $canSeeAudit = $request->user()?->hasAnyRole(['super', 'admin']) ?? false;
        $activity = $canSeeAudit
            ? AuditLogResource::collection(
                AuditLog::query()
                    ->where('auditable_type', EntryAuthorizer::class)
                    ->where('auditable_id', $entryAuthorizer->id)
        ...
        return inertia('EntryAuthorizers/Show', [
            'entryAuthorizer' => array_merge(
                $this->payload($entryAuthorizer, withAudit: true),
                ['lock' => $this->lockMeta($entryAuthorizer, $request)],
            ),
            'recordAudit'  => $this->recordAuditMeta($entryAuthorizer),
            'activity'     => $activity,
        ]);
    }
    ...
    {
        return inertia('EntryAuthorizers/Form', [
            'entryAuthorizer'        => null,
        ]);
    }

    public function store(StoreEntryAuthorizerRequest $request, EntryAuthorizerService $service): RedirectResponse
    {
        ...
        if ($tenant) {
            ...
            if ($max > 0 && EntryAuthorizer::count() >= $max) {
        ...
        return redirect()
            ->route('business_management.entry_authorizers.index')
            ->with('success', __('entry_authorizers.created'));
    }
     ...
     * Gated por permission:entry_authorizers.create (super/admin pasan por sus permisos).
     */
    public function quickStore(StoreEntryAuthorizerRequest $request, EntryAuthorizerService $service): \Illuminate\Http\JsonResponse
    {
        $tenant = $request->user()?->tenant;
        if ($tenant) {
            $max = $tenant->maxRecordsPerModule();
        ...
        $entryAuthorizer = $service->create($request->validated());

        return response()->json(['id' => $entryAuthorizer->id, 'name' => $entryAuthorizer->name], 201);
    }

    public function edit(EntryAuthorizer $entryAuthorizer)
    {
        // Registro bloqueado (Lockable): ni se abre el formulario de edición.
        abort_if($entryAuthorizer->is_locked, 403, __('locks.cannot_edit_locked'));

        return inertia('EntryAuthorizers/Form', [
            'entryAuthorizer'        => $this->payload($entryAuthorizer),
        ]);
    }


    public function update(UpdateEntryAuthorizerRequest $request, EntryAuthorizer $entryAuthorizer, EntryAuthorizerService $service): RedirectResponse
    {
        $service->update($entryAuthorizer, $request->validated());

        return redirect()
            ->route('business_management.entry_authorizers.index')
            ->with('success', __('entry_authorizers.saved'));
    }

    public function delete(EntryAuthorizer $entryAuthorizer)
    {
        // Registro bloqueado (Lockable): ni se abre la confirmación de borrado.
        abort_if($entryAuthorizer->is_locked, 403, __('locks.cannot_delete_locked'));

        return inertia('EntryAuthorizers/Delete', [
            'entryAuthorizer' => $this->payload($entryAuthorizer),
        ]);
    }

    public function deleteSave(DeleteEntryAuthorizerRequest $request, EntryAuthorizer $entryAuthorizer, EntryAuthorizerService $service): RedirectResponse
    {
        $service->delete($entryAuthorizer, $request->validated()['deleted_description']);

        $this->storeUndoableDelete([$entryAuthorizer->id]);

        return redirect()
            ->route('business_management.entry_authorizers.index')
            ->with('success', __('global.deleted_success'))
            ->with('recentDelete', $this->buildRecentDeletePayload([$entryAuthorizer->id]));
    }

    /** Persiste el claim en sesion por el window de undo (60s). */
    protected function storeUndoableDelete(array $ids): void
    {
        session(['entry_authorizers.recent_delete' => [
    ...
    public function trash(Request $request)
    {
        ...
        return inertia('EntryAuthorizers/Trash', [
    ...
    public function restore(Request $request, $slug, EntryAuthorizerService $service): RedirectResponse
    {
        abort_unless($request->user()?->hasRole('super'), 403);
        $model = EntryAuthorizer::onlyTrashed()->where('slug', $slug)->firstOrFail();
        $service->restore($model);

        return redirect()
            ->route('business_management.entry_authorizers.trash')
    ...
    public function editAll(Request $request)
    {
        ...
        $entry_authorizers = EntryAuthorizer::query()
            ->filter($request)
            ->select('entry_authorizers.id', 'entry_authorizers.slug', 'entry_authorizers.name', 'entry_authorizers.code', 'entry_authorizers.is_active')
            ->paginate($perPage)
            ->withQueryString();

        return inertia('EntryAuthorizers/EditAll', [
            'entry_authorizers' => $entry_authorizers,
            'filters'   => [
                'name'      => $request->get('name', ''),
                'is_active' => $request->filled('is_active')
    ...
    public function editAllUpdate(EditAllUpdateEntryAuthorizerRequest $request, EntryAuthorizerService $service): RedirectResponse
    {
        $changes = $request->validated()['changes'];

        // Excluir registros BLOQUEADOS (Lockable) de la edición masiva.
        $ids = array_column($changes, 'id');
        [, $lockedIds] = $this->splitLockedIds(EntryAuthorizer::class, $ids);
        if (!empty($lockedIds)) {
        ...
        return redirect()
            ->route('business_management.entry_authorizers.edit_all')
            ->with('success', $msg);
    }
     ...
     */
    public function duplicate(Request $request, EntryAuthorizer $entryAuthorizer, EntryAuthorizerService $service): RedirectResponse
    {
        ...
        $clone = $service->duplicate($entryAuthorizer);
    ...
    public function bulkRestore(BulkRestoreEntryAuthorizerRequest $request, EntryAuthorizerService $service): RedirectResponse
    {
    ...
    public function forceDelete(ForceDeleteEntryAuthorizerRequest $request, $slug, EntryAuthorizerService $service): RedirectResponse
    {
        $model = EntryAuthorizer::onlyTrashed()->where('slug', $slug)->firstOrFail();
    ...
    protected function payload(EntryAuthorizer $m, bool $withAudit = false): array
    {
        $base = [
            'id'         => $m->id,
            'slug'       => $m->slug,
            'name'       => $m->name,
            'code'       => $m->code,
            // La firma escaneada, como URL pública: la ficha y el formulario
            // la muestran, y el alta de recepciones no la necesita.
            'image_url'  => $m->image
                ? \Illuminate\Support\Facades\Storage::disk('public')->url($m->image)
                : null,
    ...
    {
        return $this->dispatchExport($request, 'csv', GenerateEntryAuthorizersCsvJob::class);
    }

    public function exportExcel(Request $request)
    {
        return $this->dispatchExport($request, 'excel', GenerateEntryAuthorizersExcelJob::class);
    }

    public function exportPdf(Request $request)
    {
        return $this->dispatchExport($request, 'pdf', GenerateEntryAuthorizersPdfJob::class);
    }

    public function exportWord(Request $request)
    {
        return $this->dispatchExport($request, 'word', GenerateEntryAuthorizersWordJob::class);
    }
    ...
    protected function assertExportLimit(string $format, array $options): void
    {
        ...
        $limit = \App\Models\Setting::getExportLimit('entry_authorizers', $format);
        if ($limit === 0) return; // CSV streaming, sin limite

        $count = $this->countForExport($options);
        if ($count > $limit) {
            abort(422, __('entry_authorizers.export_limit_exceeded', [
    ...
    protected function countForExport(array $options): int
    {
        ...
        if ($scope === 'all') {
            return EntryAuthorizer::query()->count();
        }
        // Filters como Request para reusar scopeFilter.
        $fakeReq = new Request($options['filters'] ?? []);
        return EntryAuthorizer::query()->filter($fakeReq)->count();
    }
    ...
    public function importTemplate()
    {
        return \Maatwebsite\Excel\Facades\Excel::download(
            new \App\Exports\BusinessManagement\EntryAuthorizers\EntryAuthorizersImportTemplate(),
            __('entry_authorizers.import_template_filename')
        );
    }

    public function import(ImportEntryAuthorizerRequest $request)
    {
        $data    = $request->validated();
        ...
        // el lookup por nombre case-insensitive matchearÃ­a entry_authorizers de cualquier
        ...
        if ($user && $user->hasRole('super') && empty($user->tenant_id)) {
            return response()->json([
                'ok'      => false,
                'dry_run' => $dryRun,
                'message' => __('entry_authorizers.import_super_blocked', [], 'Super sin workspace asignado no puede importar â€” el match por nombre puede actualizar registros de otro tenant.'),
            ], 422);
        }

        $importer = new \App\Imports\BusinessManagement\EntryAuthorizers\EntryAuthorizersImport(
            mode:   $mode,
            dryRun: $dryRun,
        );
        ...
        try {
            ...
            \Log::error('EntryAuthorizersImport failed', [
                'error' => $e->getMessage(),
    ...
    public function bulkDelete(BulkDeleteEntryAuthorizerRequest $request, EntryAuthorizerService $service): RedirectResponse
    {
        $data = $request->validated();

        // Excluir registros BLOQUEADOS (Lockable): no se borran en masa.
        [$allowedIds, $lockedIds] = $this->splitLockedIds(EntryAuthorizer::class, $data['ids']);
        if (empty($allowedIds)) {
     ...
     */
    public function undoLastDelete(Request $request, EntryAuthorizerService $service): RedirectResponse
    {
        $claim = session('entry_authorizers.recent_delete');
        if (!$claim || !is_array($claim) || empty($claim['ids']) || empty($claim['expires_at'])) {
            return back()->with('error', __('global.undo_failed'));
        }
        if (now()->isAfter($claim['expires_at'])) {
            session()->forget('entry_authorizers.recent_delete');
            return back()->with('error', __('global.undo_failed'));
        }
    ...
    public function bulkSetActive(BulkSetActiveEntryAuthorizerRequest $request, EntryAuthorizerService $service): RedirectResponse
    {
        $data = $request->validated();
    ...
    protected function buildExportOptions(Request $request, string $format): array
    {
        ...
        return [
            ...
            'title'                   => $data['title']                   ?? __('entry_authorizers.export_title'),
    ...
    protected function recordExportAudit(string $format, array $options): void
    {
        AuditLog::create([
            ...
            'url'        => route('business_management.entry_authorizers.index'),
