...
use App\Http\Requests\BusinessManagement\EquipmentType\BulkDeleteEquipmentTypeRequest;
use App\Http\Requests\BusinessManagement\EquipmentType\BulkRestoreEquipmentTypeRequest;
use App\Http\Requests\BusinessManagement\EquipmentType\BulkSetActiveEquipmentTypeRequest;
use App\Http\Requests\BusinessManagement\EquipmentType\DeleteEquipmentTypeRequest;
use App\Http\Requests\BusinessManagement\EquipmentType\EditAllUpdateEquipmentTypeRequest;
use App\Http\Requests\BusinessManagement\EquipmentType\ForceDeleteEquipmentTypeRequest;
use App\Http\Requests\BusinessManagement\EquipmentType\ImportEquipmentTypeRequest;
use App\Http\Requests\BusinessManagement\EquipmentType\StoreEquipmentTypeRequest;
use App\Http\Requests\BusinessManagement\EquipmentType\UpdateEquipmentTypeRequest;
use App\Http\Resources\AuditLogResource;
use App\Jobs\BusinessManagement\EquipmentTypes\GenerateEquipmentTypesCsvJob;
use App\Jobs\BusinessManagement\EquipmentTypes\GenerateEquipmentTypesExcelJob;
use App\Jobs\BusinessManagement\EquipmentTypes\GenerateEquipmentTypesPdfJob;
use App\Jobs\BusinessManagement\EquipmentTypes\GenerateEquipmentTypesWordJob;
use App\Models\AuditLog;
use App\Models\EquipmentType;
use App\Services\BusinessManagement\EquipmentTypeService;
...
class EquipmentTypeController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        // equipment_types es catalogo global (sin tenant) — solo eager-load creator.
        ...
        $totalUnfiltered = EquipmentType::count();

        $names = $request->get('name', []);
        if (is_string($names)) $names = $names === '' ? [] : [$names];

        return inertia('EquipmentTypes/Index', [
            'equipmentTypes' => array_merge($equipmentTypes->toArray(), [
            ...
            'exportLimits' => \App\Models\Setting::getExportLimits('equipment_types'),
            'filters' => [
            ...
            'filterSchema'   => EquipmentType::filterSchema(),
        ]);
    }
    ...
    {
        $equipmentType->load(['creator:id,name,email', 'deleter:id,name,email']);

        $canSeeAudit = $request->user()?->hasAnyRole(['super', 'admin']) ?? false;
        $activity = $canSeeAudit
            ? AuditLogResource::collection(
                AuditLog::query()
                    ->where('auditable_type', EquipmentType::class)
                    ->where('auditable_id', $equipmentType->id)
        ...
        return inertia('EquipmentTypes/Show', [
            'equipmentType' => $this->payload($equipmentType, withAudit: true),
            'recordAudit'  => $this->recordAuditMeta($equipmentType),
            'activity'     => $activity,
        ]);
    }
    ...
    {
        return inertia('EquipmentTypes/Form', [
            'equipmentType'        => null,
        ]);
    }

    public function store(StoreEquipmentTypeRequest $request, EquipmentTypeService $service): RedirectResponse
    {
        ...
        if ($tenant) {
            ...
            if ($max > 0 && EquipmentType::count() >= $max) {
        ...
        return redirect()
            ->route('business_management.equipment_types.index')
            ->with('success', __('equipment_types.created'));
    }

    public function edit(EquipmentType $equipmentType)
    {
        return inertia('EquipmentTypes/Form', [
            'equipmentType'        => $this->payload($equipmentType),
        ]);
    }


    public function update(UpdateEquipmentTypeRequest $request, EquipmentType $equipmentType, EquipmentTypeService $service): RedirectResponse
    {
        $service->update($equipmentType, $request->validated());

        return redirect()
            ->route('business_management.equipment_types.index')
            ->with('success', __('equipment_types.saved'));
    }

    public function delete(EquipmentType $equipmentType)
    {
        return inertia('EquipmentTypes/Delete', [
            'equipmentType' => $this->payload($equipmentType),
        ]);
    }

    public function deleteSave(DeleteEquipmentTypeRequest $request, EquipmentType $equipmentType, EquipmentTypeService $service): RedirectResponse
    {
        $service->delete($equipmentType, $request->validated()['deleted_description']);

        $this->storeUndoableDelete([$equipmentType->id]);

        return redirect()
            ->route('business_management.equipment_types.index')
            ->with('success', __('global.deleted_success'))
            ->with('recentDelete', $this->buildRecentDeletePayload([$equipmentType->id]));
    }

    /** Persiste el claim en sesion por el window de undo (60s). */
    protected function storeUndoableDelete(array $ids): void
    {
        session(['equipment_types.recent_delete' => [
    ...
    public function trash(Request $request)
    {
        ...
        return inertia('EquipmentTypes/Trash', [
    ...
    public function restore(Request $request, $slug, EquipmentTypeService $service): RedirectResponse
    {
        abort_unless($request->user()?->hasRole('super'), 403);
        $model = EquipmentType::onlyTrashed()->where('slug', $slug)->firstOrFail();
        $service->restore($model);

        return redirect()
            ->route('business_management.equipment_types.trash')
    ...
    public function editAll(Request $request)
    {
        ...
        $equipmentTypes = EquipmentType::query()
            ->filter($request)
            ->select('equipment_types.id', 'equipment_types.slug', 'equipment_types.name', 'equipment_types.code', 'equipment_types.is_active')
            ->paginate($perPage)
            ->withQueryString();

        return inertia('EquipmentTypes/EditAll', [
            'equipmentTypes' => $equipmentTypes,
            'filters'   => [
                'name'      => $request->get('name', ''),
                'is_active' => $request->filled('is_active')
    ...
    public function editAllUpdate(EditAllUpdateEquipmentTypeRequest $request, EquipmentTypeService $service): RedirectResponse
    {
     ...
     */
    public function duplicate(Request $request, EquipmentType $equipmentType, EquipmentTypeService $service): RedirectResponse
    {
        ...
        $clone = $service->duplicate($equipmentType);
    ...
    public function bulkRestore(BulkRestoreEquipmentTypeRequest $request, EquipmentTypeService $service): RedirectResponse
    {
    ...
    public function forceDelete(ForceDeleteEquipmentTypeRequest $request, $slug, EquipmentTypeService $service): RedirectResponse
    {
        $model = EquipmentType::onlyTrashed()->where('slug', $slug)->firstOrFail();
    ...
    protected function payload(EquipmentType $m, bool $withAudit = false): array
    {
        $base = [
            'id'         => $m->id,
            'slug'       => $m->slug,
    ...
    {
        return $this->dispatchExport($request, 'csv', GenerateEquipmentTypesCsvJob::class);
    }

    public function exportExcel(Request $request)
    {
        return $this->dispatchExport($request, 'excel', GenerateEquipmentTypesExcelJob::class);
    }

    public function exportPdf(Request $request)
    {
        return $this->dispatchExport($request, 'pdf', GenerateEquipmentTypesPdfJob::class);
    }

    public function exportWord(Request $request)
    {
        return $this->dispatchExport($request, 'word', GenerateEquipmentTypesWordJob::class);
    }
    ...
    protected function assertExportLimit(string $format, array $options): void
    {
        ...
        $limit = \App\Models\Setting::getExportLimit('equipment_types', $format);
        if ($limit === 0) return; // CSV streaming, sin limite

        $count = $this->countForExport($options);
        if ($count > $limit) {
            abort(422, __('equipment_types.export_limit_exceeded', [
    ...
    protected function countForExport(array $options): int
    {
        ...
        if ($scope === 'all') {
            return EquipmentType::query()->count();
        }
        // Filters como Request para reusar scopeFilter.
        $fakeReq = new Request($options['filters'] ?? []);
        return EquipmentType::query()->filter($fakeReq)->count();
    }
    ...
    public function importTemplate()
    {
        return \Maatwebsite\Excel\Facades\Excel::download(
            new \App\Exports\BusinessManagement\EquipmentTypes\EquipmentTypesImportTemplate(),
            __('equipment_types.import_template_filename')
        );
    }

    public function import(ImportEquipmentTypeRequest $request)
    {
        $data    = $request->validated();
        ...
        // el lookup por nombre case-insensitive matchearÃ­a equipmentTypes de cualquier
        ...
        if ($user && $user->hasRole('super') && empty($user->tenant_id)) {
            return response()->json([
                'ok'      => false,
                'dry_run' => $dryRun,
                'message' => __('equipment_types.import_super_blocked', [], 'Super sin workspace asignado no puede importar â€” el match por nombre puede actualizar registros de otro tenant.'),
            ], 422);
        }

        $importer = new \App\Imports\BusinessManagement\EquipmentTypes\EquipmentTypesImport(
            mode:   $mode,
            dryRun: $dryRun,
        );
        ...
        try {
            ...
            \Log::error('EquipmentTypesImport failed', [
                'error' => $e->getMessage(),
    ...
    public function bulkDelete(BulkDeleteEquipmentTypeRequest $request, EquipmentTypeService $service): RedirectResponse
    {
        $data   = $request->validated();
     ...
     */
    public function undoLastDelete(Request $request, EquipmentTypeService $service): RedirectResponse
    {
        $claim = session('equipment_types.recent_delete');
        if (!$claim || !is_array($claim) || empty($claim['ids']) || empty($claim['expires_at'])) {
            return back()->with('error', __('global.undo_failed'));
        }
        if (now()->isAfter($claim['expires_at'])) {
            session()->forget('equipment_types.recent_delete');
            return back()->with('error', __('global.undo_failed'));
        }
    ...
    public function bulkSetActive(BulkSetActiveEquipmentTypeRequest $request, EquipmentTypeService $service): RedirectResponse
    {
        $data   = $request->validated();
    ...
    protected function buildExportOptions(Request $request, string $format): array
    {
        ...
        return [
            ...
            'title'                   => $data['title']                   ?? __('equipment_types.export_title'),
    ...
    protected function recordExportAudit(string $format, array $options): void
    {
        AuditLog::create([
            ...
            'url'        => route('business_management.equipment_types.index'),
