...
use App\Http\Requests\BusinessManagement\Instrument\BulkDeleteInstrumentRequest;
use App\Http\Requests\BusinessManagement\Instrument\BulkRestoreInstrumentRequest;
use App\Http\Requests\BusinessManagement\Instrument\BulkSetActiveInstrumentRequest;
use App\Http\Requests\BusinessManagement\Instrument\DeleteInstrumentRequest;
use App\Http\Requests\BusinessManagement\Instrument\EditAllUpdateInstrumentRequest;
use App\Http\Requests\BusinessManagement\Instrument\ForceDeleteInstrumentRequest;
use App\Http\Requests\BusinessManagement\Instrument\ImportInstrumentRequest;
use App\Http\Requests\BusinessManagement\Instrument\StoreInstrumentRequest;
use App\Http\Requests\BusinessManagement\Instrument\UpdateInstrumentRequest;
use App\Http\Resources\AuditLogResource;
use App\Jobs\BusinessManagement\Instruments\GenerateInstrumentsCsvJob;
use App\Jobs\BusinessManagement\Instruments\GenerateInstrumentsExcelJob;
use App\Jobs\BusinessManagement\Instruments\GenerateInstrumentsPdfJob;
use App\Jobs\BusinessManagement\Instruments\GenerateInstrumentsWordJob;
use App\Models\AuditLog;
use App\Models\Instrument;
use App\Services\BusinessManagement\InstrumentService;
...
class InstrumentController extends Controller
{
    ...
    /** Pone el candado al instrumento (super → nivel sistema; admin → nivel tenant). */
    public function lock(Request $request, Instrument $instrument): RedirectResponse
    {
        return $this->applyLock($instrument, $request);
    }

    /** Saca el candado (un admin no puede quitar un candado del super). */
    public function unlock(Request $request, Instrument $instrument): RedirectResponse
    {
        return $this->applyUnlock($instrument, $request);
    }
    ...
    public function index(Request $request)
    {
        ...
        return inertia('Instruments/Index', [
            'instruments' => array_merge($instruments->toArray(), [
            ...
            'filters' => [
                ...
                // justifica el módulo: ver de una qué equipos no se pueden usar.
                'calibration_status' => $request->get('calibration_status', null),
            ...
            'filterSchema'   => Instrument::filterSchema(),
        ]);
    }
    ...
    {
        $instrument->load(['creator:id,name,email', 'deleter:id,name,email', 'locker:id,name']);

        $canSeeAudit = $request->user()?->hasAnyRole(['super', 'admin']) ?? false;
        $activity = $canSeeAudit
            ? AuditLogResource::collection(
                AuditLog::query()
                    ->where('auditable_type', Instrument::class)
        ...
        return inertia('Instruments/Show', [
            'instrument' => array_merge(
                $this->payload($instrument, withAudit: true),
                ['lock' => $this->lockMeta($instrument, $request)],
            ),
            'recordAudit'  => $this->recordAuditMeta($instrument),
            'activity'     => $activity,
        ]);
    }
    ...
    {
        return inertia('Instruments/Form', [
            'instrument'        => null,
        ]);
    }

    public function store(StoreInstrumentRequest $request, InstrumentService $service): RedirectResponse
    {
        ...
        return redirect()
            ->route('business_management.instruments.index')
            ->with('success', __('instruments.created'));
    }

    /**
     * Alta rápida de instrumento desde un select de otra pantalla (la hoja
     * de trabajo, cuando el analista descubre que el equipo con el que midió
     * todavía no está dado de alta) sin salir de la página. Misma validación
     * que store() — incluye la unicidad del código — pero responde JSON con el
     * instrumento creado para inyectarlo en el select.
     * Gated por permission:instruments.create (super/admin pasan por sus permisos).
     */
    public function quickStore(StoreInstrumentRequest $request, InstrumentService $service): \Illuminate\Http\JsonResponse
    {
        $tenant = $request->user()?->tenant;
        if ($tenant) {
            $max = $tenant->maxRecordsPerModule();
        ...
        $instrument = $service->create($request->validated());

        return response()->json(['id' => $instrument->id, 'name' => $instrument->name], 201);
    }

    public function edit(Instrument $instrument)
    {
        // Registro bloqueado (Lockable): ni se abre el formulario de edición.
        abort_if($instrument->is_locked, 403, __('locks.cannot_edit_locked'));
    ...
    public function update(UpdateInstrumentRequest $request, Instrument $instrument, InstrumentService $service): RedirectResponse
    {
        $service->update($instrument, $request->validated());

        return redirect()
            ->route('business_management.instruments.index')
            ->with('success', __('instruments.saved'));
    }

    public function delete(Instrument $instrument)
    {
        // Registro bloqueado (Lockable): ni se abre la confirmación de borrado.
        abort_if($instrument->is_locked, 403, __('locks.cannot_delete_locked'));

        return inertia('Instruments/Delete', [
            'instrument' => $this->payload($instrument),
        ]);
    }

    public function deleteSave(DeleteInstrumentRequest $request, Instrument $instrument, InstrumentService $service): RedirectResponse
    {
        $service->delete($instrument, $request->validated()['deleted_description']);

        $this->storeUndoableDelete([$instrument->id]);

        return redirect()
            ->route('business_management.instruments.index')
            ->with('success', __('global.deleted_success'))
            ->with('recentDelete', $this->buildRecentDeletePayload([$instrument->id]));
    }

    /** Persiste el claim en sesion por el window de undo (60s). */
    protected function storeUndoableDelete(array $ids): void
    {
        session(['instruments.recent_delete' => [
    ...
    public function trash(Request $request)
    {
        ...
        return inertia('Instruments/Trash', [
    ...
    public function restore(Request $request, $slug, InstrumentService $service): RedirectResponse
    {
        abort_unless($request->user()?->hasRole('super'), 403);
        $model = Instrument::onlyTrashed()->where('slug', $slug)->firstOrFail();
        $service->restore($model);

        return redirect()
            ->route('business_management.instruments.trash')
    ...
    public function editAll(Request $request)
    {
        ...
        return inertia('Instruments/EditAll', [
            'instruments' => $instruments,
            'filters'   => [
                'name'      => $request->get('name', ''),
                'is_active' => $request->filled('is_active')
    ...
    public function editAllUpdate(EditAllUpdateInstrumentRequest $request, InstrumentService $service): RedirectResponse
    {
        $changes = $request->validated()['changes'];

        // Excluir registros BLOQUEADOS (Lockable) de la edición masiva.
        $ids = array_column($changes, 'id');
        [, $lockedIds] = $this->splitLockedIds(Instrument::class, $ids);
        if (!empty($lockedIds)) {
        ...
        return redirect()
            ->route('business_management.instruments.edit_all')
            ->with('success', $msg);
    }

    /**
     * Clona el instrument. Sufijo "(copia)" con sanity guard de 100 intentos.
     */
    public function duplicate(Request $request, Instrument $instrument, InstrumentService $service): RedirectResponse
    {
        ...
        $clone = $service->duplicate($instrument);
    ...
    public function bulkRestore(BulkRestoreInstrumentRequest $request, InstrumentService $service): RedirectResponse
    {
    ...
    public function forceDelete(ForceDeleteInstrumentRequest $request, $slug, InstrumentService $service): RedirectResponse
    {
        $model = Instrument::onlyTrashed()->where('slug', $slug)->firstOrFail();
    ...
    protected function payload(Instrument $m, bool $withAudit = false): array
    {
        $base = [
            'id'         => $m->id,
            'slug'       => $m->slug,
            ...
            'model'      => $m->model,
            'serial'     => $m->serial,
            // Fechas planas (Y-m-d): el DatePicker del form y la ficha no
            // quieren zona horaria, la calibración es un día de calendario.
            'calibrated_at'           => $m->calibrated_at?->toDateString(),
            'calibration_due_at'      => $m->calibration_due_at?->toDateString(),
            'calibration_certificate' => $m->calibration_certificate,
            'calibration_status'      => $m->calibration_status,
            'calibration_days_left'   => $m->calibration_days_left,
            'location'   => $m->location,
            'notes'      => $m->notes,
    ...
    {
        return $this->dispatchExport($request, 'csv', GenerateInstrumentsCsvJob::class);
    }

    public function exportExcel(Request $request)
    {
        return $this->dispatchExport($request, 'excel', GenerateInstrumentsExcelJob::class);
    }

    public function exportPdf(Request $request)
    {
        return $this->dispatchExport($request, 'pdf', GenerateInstrumentsPdfJob::class);
    }

    public function exportWord(Request $request)
    {
        return $this->dispatchExport($request, 'word', GenerateInstrumentsWordJob::class);
    }
    ...
    protected function assertExportLimit(string $format, array $options): void
    {
        ...
        $limit = \App\Models\Setting::getExportLimit('instruments', $format);
        if ($limit === 0) return; // CSV streaming, sin limite

        $count = $this->countForExport($options);
        if ($count > $limit) {
            abort(422, __('instruments.export_limit_exceeded', [
    ...
    public function importTemplate()
    {
        return \Maatwebsite\Excel\Facades\Excel::download(
            new \App\Exports\BusinessManagement\Instruments\InstrumentsImportTemplate(),
            __('instruments.import_template_filename')
        );
    }

    public function import(ImportInstrumentRequest $request)
    {
        $data    = $request->validated();
        ...
        // el lookup por nombre case-insensitive matchearía instruments de cualquier
        ...
        if ($user && $user->hasRole('super') && empty($user->tenant_id)) {
            return response()->json([
                'ok'      => false,
                'dry_run' => $dryRun,
                'message' => __('instruments.import_super_blocked', [], 'Super sin workspace asignado no puede importar — la busqueda por codigo puede actualizar registros de otro workspace.'),
            ], 422);
        }

        $importer = new \App\Imports\BusinessManagement\Instruments\InstrumentsImport(
            mode:   $mode,
            dryRun: $dryRun,
        );
        ...
        try {
            ...
            \Log::error('InstrumentsImport failed', [
                'error' => $e->getMessage(),
    ...
    // ── BULK OPERATIONS ─────────────────────────────────────────────────
    public function bulkDelete(BulkDeleteInstrumentRequest $request, InstrumentService $service): RedirectResponse
    {
        $data = $request->validated();

        // Excluir registros BLOQUEADOS (Lockable): no se borran en masa.
        [$allowedIds, $lockedIds] = $this->splitLockedIds(Instrument::class, $data['ids']);
        if (empty($allowedIds)) {
     ...
     */
    public function undoLastDelete(Request $request, InstrumentService $service): RedirectResponse
    {
        $claim = session('instruments.recent_delete');
        if (!$claim || !is_array($claim) || empty($claim['ids']) || empty($claim['expires_at'])) {
            return back()->with('error', __('global.undo_failed'));
        }
        if (now()->isAfter($claim['expires_at'])) {
            session()->forget('instruments.recent_delete');
            return back()->with('error', __('global.undo_failed'));
        }
    ...
    public function bulkSetActive(BulkSetActiveInstrumentRequest $request, InstrumentService $service): RedirectResponse
    {
        $data = $request->validated();
    ...
    protected function buildExportOptions(Request $request, string $format): array
    {
        ...
        // exportable por super: el resto ve únicamente instrumentos de su propio
        ...
        return [
            ...
            'title'                   => $data['title']                   ?? __('instruments.export_title'),
    ...
    protected function recordExportAudit(string $format, array $options): void
    {
        AuditLog::create([
            ...
            'url'        => route('business_management.instruments.index'),
