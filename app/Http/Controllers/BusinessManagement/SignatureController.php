...
use App\Http\Requests\BusinessManagement\Signature\BulkDeleteSignatureRequest;
use App\Http\Requests\BusinessManagement\Signature\BulkRestoreSignatureRequest;
use App\Http\Requests\BusinessManagement\Signature\BulkSetActiveSignatureRequest;
use App\Http\Requests\BusinessManagement\Signature\DeleteSignatureRequest;
use App\Http\Requests\BusinessManagement\Signature\EditAllUpdateSignatureRequest;
use App\Http\Requests\BusinessManagement\Signature\ForceDeleteSignatureRequest;
use App\Http\Requests\BusinessManagement\Signature\ImportSignatureRequest;
use App\Http\Requests\BusinessManagement\Signature\StoreSignatureRequest;
use App\Http\Requests\BusinessManagement\Signature\UpdateSignatureRequest;
use App\Http\Resources\AuditLogResource;
use App\Jobs\BusinessManagement\Signatures\GenerateSignaturesCsvJob;
use App\Jobs\BusinessManagement\Signatures\GenerateSignaturesExcelJob;
use App\Jobs\BusinessManagement\Signatures\GenerateSignaturesPdfJob;
use App\Jobs\BusinessManagement\Signatures\GenerateSignaturesWordJob;
use App\Models\AuditLog;
use App\Models\Signature;
use App\Services\BusinessManagement\SignatureService;
...
class SignatureController extends Controller
{
    ...
    public function lock(Request $request, Signature $signature): RedirectResponse
    {
        return $this->applyLock($signature, $request);
    }

    /** Saca el candado (un admin no puede quitar un candado del super). */
    public function unlock(Request $request, Signature $signature): RedirectResponse
    {
        return $this->applyUnlock($signature, $request);
    }
    ...
    public function index(Request $request)
    {
        ...
        // signatures es per-tenant (BelongsToTenant lo scopea solo) — eager-load creator.
        ...
        $totalUnfiltered = Signature::count();

        $names = $request->get('name', []);
        if (is_string($names)) $names = $names === '' ? [] : [$names];

        return inertia('Signatures/Index', [
            'signatures' => array_merge($signatures->toArray(), [
            ...
            'exportLimits' => \App\Models\Setting::getExportLimits('signatures'),
            'filters' => [
            ...
            'filterSchema'   => Signature::filterSchema(),
        ]);
    }
    ...
    public function show(Request $request, Signature $signature)
    {
        $signature->load(['creator:id,name,email', 'deleter:id,name,email', 'locker:id,name']);

        $canSeeAudit = $request->user()?->hasAnyRole(['super', 'admin']) ?? false;
        $activity = $canSeeAudit
            ? AuditLogResource::collection(
                AuditLog::query()
                    ->where('auditable_type', Signature::class)
                    ->where('auditable_id', $signature->id)
        ...
        return inertia('Signatures/Show', [
            'signature' => array_merge(
                $this->payload($signature, withAudit: true),
                ['lock' => $this->lockMeta($signature, $request)],
            ),
            'recordAudit'  => $this->recordAuditMeta($signature),
            'activity'     => $activity,
        ]);
    }
    ...
    public function store(StoreSignatureRequest $request, SignatureService $service): RedirectResponse
    {
        ...
        return redirect()
            ->route('business_management.signatures.index')
            ->with('success', __('signatures.created'));
    }
     ...
     * Gated por permission:signatures.create (super/admin pasan por sus permisos).
     */
    public function quickStore(StoreSignatureRequest $request, SignatureService $service): \Illuminate\Http\JsonResponse
    {
        $tenant = $request->user()?->tenant;
        if ($tenant) {
            $max = $tenant->maxRecordsPerModule();
        ...
        $signature = $service->create($request->validated());

        return response()->json(['id' => $signature->id, 'name' => $signature->name], 201);
    }
    ...
    public function update(UpdateSignatureRequest $request, Signature $signature, SignatureService $service): RedirectResponse
    {
        $service->update($signature, $request->validated());

        return redirect()
            ->route('business_management.signatures.index')
            ->with('success', __('signatures.saved'));
    }

    public function delete(Signature $signature)
    {
        // Registro bloqueado (Lockable): ni se abre la confirmación de borrado.
        abort_if($signature->is_locked, 403, __('locks.cannot_delete_locked'));

        return inertia('Signatures/Delete', [
            'signature' => $this->payload($signature),
        ]);
    }

    public function deleteSave(DeleteSignatureRequest $request, Signature $signature, SignatureService $service): RedirectResponse
    {
        $service->delete($signature, $request->validated()['deleted_description']);

        $this->storeUndoableDelete([$signature->id]);

        return redirect()
            ->route('business_management.signatures.index')
            ->with('success', __('global.deleted_success'))
            ->with('recentDelete', $this->buildRecentDeletePayload([$signature->id]));
    }

    /** Persiste el claim en sesion por el window de undo (60s). */
    protected function storeUndoableDelete(array $ids): void
    {
        session(['signatures.recent_delete' => [
    ...
    public function trash(Request $request)
    {
        ...
        return inertia('Signatures/Trash', [
    ...
    public function restore(Request $request, $slug, SignatureService $service): RedirectResponse
    {
        abort_unless($request->user()?->hasRole('super'), 403);
        $model = Signature::onlyTrashed()->where('slug', $slug)->firstOrFail();
        $service->restore($model);

        return redirect()
            ->route('business_management.signatures.trash')
    ...
    public function editAll(Request $request)
    {
        ...
        $signatures = Signature::query()
            ->filter($request)
            ->select('signatures.id', 'signatures.slug', 'signatures.name', 'signatures.code', 'signatures.is_active')
            ->paginate($perPage)
            ->withQueryString();

        return inertia('Signatures/EditAll', [
            'signatures' => $signatures,
            'filters'   => [
                'name'      => $request->get('name', ''),
                'is_active' => $request->filled('is_active')
    ...
    public function editAllUpdate(EditAllUpdateSignatureRequest $request, SignatureService $service): RedirectResponse
    {
        $changes = $request->validated()['changes'];

        // Excluir registros BLOQUEADOS (Lockable) de la edición masiva.
        $ids = array_column($changes, 'id');
        [, $lockedIds] = $this->splitLockedIds(Signature::class, $ids);
        if (!empty($lockedIds)) {
     ...
     */
    public function duplicate(Request $request, Signature $signature, SignatureService $service): RedirectResponse
    {
        ...
        $clone = $service->duplicate($signature);
    ...
    public function bulkRestore(BulkRestoreSignatureRequest $request, SignatureService $service): RedirectResponse
    {
    ...
    public function forceDelete(ForceDeleteSignatureRequest $request, $slug, SignatureService $service): RedirectResponse
    {
        $model = Signature::onlyTrashed()->where('slug', $slug)->firstOrFail();
    ...
    protected function payload(Signature $m, bool $withAudit = false): array
    {
        $base = [
            'id'         => $m->id,
            'slug'       => $m->slug,
    ...
    {
        return $this->dispatchExport($request, 'csv', GenerateSignaturesCsvJob::class);
    }

    public function exportExcel(Request $request)
    {
        return $this->dispatchExport($request, 'excel', GenerateSignaturesExcelJob::class);
    }

    public function exportPdf(Request $request)
    {
        return $this->dispatchExport($request, 'pdf', GenerateSignaturesPdfJob::class);
    }

    public function exportWord(Request $request)
    {
        return $this->dispatchExport($request, 'word', GenerateSignaturesWordJob::class);
    }
    ...
    protected function assertExportLimit(string $format, array $options): void
    {
        ...
        $limit = \App\Models\Setting::getExportLimit('signatures', $format);
        if ($limit === 0) return; // CSV streaming, sin limite

        $count = $this->countForExport($options);
        if ($count > $limit) {
            abort(422, __('signatures.export_limit_exceeded', [
    ...
    public function importTemplate()
    {
        return \Maatwebsite\Excel\Facades\Excel::download(
            new \App\Exports\BusinessManagement\Signatures\SignaturesImportTemplate(),
            __('signatures.import_template_filename')
        );
    }

    public function import(ImportSignatureRequest $request)
    {
        $data    = $request->validated();
        ...
        // el lookup por nombre case-insensitive matchearÃ­a signatures de cualquier
        ...
        if ($user && $user->hasRole('super') && empty($user->tenant_id)) {
            return response()->json([
                'ok'      => false,
                'dry_run' => $dryRun,
                'message' => __('signatures.import_super_blocked', [], 'Super sin workspace asignado no puede importar â€” el match por nombre puede actualizar registros de otro tenant.'),
            ], 422);
        }

        $importer = new \App\Imports\BusinessManagement\Signatures\SignaturesImport(
            mode:   $mode,
            dryRun: $dryRun,
        );
        ...
        try {
            ...
            \Log::error('SignaturesImport failed', [
                'error' => $e->getMessage(),
    ...
    public function bulkDelete(BulkDeleteSignatureRequest $request, SignatureService $service): RedirectResponse
    {
        $data = $request->validated();

        // Excluir registros BLOQUEADOS (Lockable): no se borran en masa.
        [$allowedIds, $lockedIds] = $this->splitLockedIds(Signature::class, $data['ids']);
        if (empty($allowedIds)) {
     ...
     */
    public function undoLastDelete(Request $request, SignatureService $service): RedirectResponse
    {
        $claim = session('signatures.recent_delete');
        if (!$claim || !is_array($claim) || empty($claim['ids']) || empty($claim['expires_at'])) {
            return back()->with('error', __('global.undo_failed'));
        }
        if (now()->isAfter($claim['expires_at'])) {
            session()->forget('signatures.recent_delete');
            return back()->with('error', __('global.undo_failed'));
        }
    ...
    public function bulkSetActive(BulkSetActiveSignatureRequest $request, SignatureService $service): RedirectResponse
    {
        $data = $request->validated();
    ...
    protected function buildExportOptions(Request $request, string $format): array
    {
        ...
        return [
            ...
            'title'                   => $data['title']                   ?? __('signatures.export_title'),
    ...
    protected function recordExportAudit(string $format, array $options): void
    {
        AuditLog::create([
            ...
            'url'        => route('business_management.signatures.index'),
