...
namespace App\Imports\BusinessManagement\EntryAuthorizers;
...
/**
 * Imports EntryAuthorizers from .xlsx/.csv.
 *
 * Columns:
 ...
 *
 * entry_authorizers es PER-TENANT: el import scope-a por tenant_id via el global scope de
 * BelongsToTenant (EntryAuthorizer::create autorellena el tenant del actor).
 *
 ...
 *   - Si el plan del usuario tiene limite, contamos cuantos entry_authorizers hay HOY +
 ...
 */
class EntryAuthorizersImport implements ToCollection, WithHeadingRow
{
    ...
    /** Count de entry_authorizers del tenant del actor (pre-import). */
    ...
    public function __construct(
        ...
        $this->currentCount = EntryAuthorizer::count();
    }
    ...
    public function collection(Collection $rows): void
    {
        ...
        try {
            ...
            $seenInFileByName = [];
            $seenInFileByCode = [];
            $newRecordsCount = 0; // contador de filas que crearian un nuevo entryAuthorizer
     ...
     * Per-tenant: el global scope de BelongsToTenant limita al tenant del actor.
     */
    protected function findExistingByNameInsensitive(string $name): ?EntryAuthorizer
    {
        $isPgsql = DB::getDriverName() === 'pgsql';
        $query   = EntryAuthorizer::query();

        if ($isPgsql) {
            $query->whereRaw('unaccent(LOWER(entry_authorizers.name)) = unaccent(LOWER(?))', [$name]);
        } else {
            $query->whereRaw('LOWER(entry_authorizers.name) = LOWER(?)', [$name]);
        }
