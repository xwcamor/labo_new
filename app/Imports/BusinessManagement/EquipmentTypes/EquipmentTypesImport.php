...
namespace App\Imports\BusinessManagement\EquipmentTypes;
...
/**
 * Imports EquipmentTypes from .xlsx/.csv.
 *
 * Columns:
 ...
 *
 * equipment_types es un CATALOGO GLOBAL (sin tenant): el import NO scope-a por
 ...
 *   - Si el plan del usuario tiene limite, contamos cuantos equipmentTypes hay HOY +
 ...
 */
class EquipmentTypesImport implements ToCollection, WithHeadingRow
{
    ...
    /** Count global de equipmentTypes (pre-import). Catalogo unico, sin tenant. */
    ...
    public function __construct(
        ...
        // Snapshot del count actual. equipment_types es catalogo GLOBAL.
        $this->currentCount = EquipmentType::withoutGlobalScopes()->count();
    }
    ...
    public function collection(Collection $rows): void
    {
        ...
        try {
            ...
            $seenInFileByName = [];
            $seenInFileByCode = [];
            $newRecordsCount = 0; // contador de filas que crearian un nuevo equipmentType
            ...
            foreach ($rows as $i => $row) {
                ...
                if ($existing) {
                    ...
                    EquipmentType::create([
                        ...
                        'is_active'   => true,
                        'created_by'  => Auth::id(),
                        // Sin tenant_id: equipment_types es catalogo global. El slug
    ...
    {
        return EquipmentType::query()->withoutGlobalScopes()
     ...
     * Global: equipment_types es catalogo unico, sin scope por tenant.
     */
    protected function findExistingByNameInsensitive(string $name): ?EquipmentType
    {
        $isPgsql = DB::getDriverName() === 'pgsql';
        $query   = EquipmentType::query()->withoutGlobalScopes();

        if ($isPgsql) {
            $query->whereRaw('unaccent(LOWER(equipment_types.name)) = unaccent(LOWER(?))', [$name]);
        } else {
            $query->whereRaw('LOWER(equipment_types.name) = LOWER(?)', [$name]);
        }
