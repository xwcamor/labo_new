...
namespace App\Imports\BusinessManagement\Samplers;
...
/**
 * Imports Samplers from .xlsx/.csv.
 *
 * Columns:
 ...
 *
 * samplers es PER-TENANT: el import scope-a por tenant_id via el global scope de
 * BelongsToTenant (Sampler::create autorellena el tenant del actor).
 *
 ...
 *   - Si el plan del usuario tiene limite, contamos cuantos samplers hay HOY +
 ...
 */
class SamplersImport implements ToCollection, WithHeadingRow
{
    ...
    /** Count de samplers del tenant del actor (pre-import). */
    ...
    public function __construct(
        ...
        $this->currentCount = Sampler::count();
    }
    ...
    public function collection(Collection $rows): void
    {
        ...
        try {
            ...
            $seenInFileByName = [];
            $seenInFileByCode = [];
            $newRecordsCount = 0; // contador de filas que crearian un nuevo sampler
     ...
     * Per-tenant: el global scope de BelongsToTenant limita al tenant del actor.
     */
    protected function findExistingByNameInsensitive(string $name): ?Sampler
    {
        $isPgsql = DB::getDriverName() === 'pgsql';
        $query   = Sampler::query();

        if ($isPgsql) {
            $query->whereRaw('unaccent(LOWER(samplers.name)) = unaccent(LOWER(?))', [$name]);
        } else {
            $query->whereRaw('LOWER(samplers.name) = LOWER(?)', [$name]);
        }
