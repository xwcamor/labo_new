...
namespace App\Imports\BusinessManagement\Signatures;
...
/**
 * Imports Signatures from .xlsx/.csv.
 *
 * Columns:
 ...
 *
 * signatures es PER-TENANT: el import scope-a por tenant_id via el global scope de
 * BelongsToTenant (Signature::create autorellena el tenant del actor).
 *
 ...
 *   - Si el plan del usuario tiene limite, contamos cuantos signatures hay HOY +
 ...
 */
class SignaturesImport implements ToCollection, WithHeadingRow
{
    ...
    /** Count de signatures del tenant del actor (pre-import). */
    ...
    public function collection(Collection $rows): void
    {
        ...
        try {
            ...
            $seenInFileByName = [];
            $seenInFileByCode = [];
            $newRecordsCount = 0; // contador de filas que crearian un nuevo signature
     ...
     * Per-tenant: el global scope de BelongsToTenant limita al tenant del actor.
     */
    protected function findExistingByNameInsensitive(string $name): ?Signature
    {
        $isPgsql = DB::getDriverName() === 'pgsql';
        $query   = Signature::query();

        if ($isPgsql) {
            $query->whereRaw('unaccent(LOWER(signatures.name)) = unaccent(LOWER(?))', [$name]);
        } else {
            $query->whereRaw('LOWER(signatures.name) = LOWER(?)', [$name]);
        }
