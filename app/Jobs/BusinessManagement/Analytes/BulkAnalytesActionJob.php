 ...
 * dispatch viven en AnalyteService.
 *
 ...
 */
class BulkAnalytesActionJob implements ShouldQueue, ShouldBeUnique
{
    ...
    public function uniqueId(): string
    {
        ...
        return "bulk:analytes:{$this->userId}:{$this->action}:{$idsHash}";
    }

    /**
     * Umbral configurable. Prioridad: Setting global -> config/analytes.php -> 200.
    ...
    public static function asyncThreshold(): int
    {
        return \App\Models\Setting::getInt(
            ...
            (int) config('analytes.bulk_async_threshold', 200),
        );
    }
    ...
    public function handle(AnalyteService $service): void
    {
        ...
        if (!$user) {
            \Log::warning('BulkAnalytesActionJob: user not found, aborting', [
                'user_id' => $this->userId,
                'action'  => $this->action,
            ]);
        ...
        foreach (array_chunk($this->ids, 200) as $chunk) {
            $analytes = match ($this->action) {
                'restore' => Analyte::onlyTrashed()->whereIn('id', $chunk)->get(),
                default   => Analyte::whereIn('id', $chunk)->get(),
            };

            foreach ($analytes as $analyte) {
                try {
                    match ($this->action) {
                        'delete'     => $service->delete($analyte, $this->payload['reason'] ?? 'Bulk delete'),
                        'set_active' => $this->setActive($service, $analyte),
                        'restore'    => $service->restore($analyte),
                        default      => throw new \InvalidArgumentException("Unknown action: {$this->action}"),
                    };
                    $processed++;
                } catch (\Throwable $e) {
                    $errors++;
                    \Log::warning("BulkAnalytesActionJob: error on analyte {$analyte->id}", [
                        'error' => $e->getMessage(),
                    ]);
                }
            }
        ...
        \Log::info("BulkAnalytesActionJob completed", [
            'user_id'   => $this->userId,
            'action'    => $this->action,
            'processed' => $processed,
            'errors'    => $errors,
    ...
    {
        \Log::error("BulkAnalytesActionJob failed", [
            'user_id' => $this->userId,
            'action'  => $this->action,
            'total'   => count($this->ids),
    ...
    protected function notifyUser(string $status, ?string $error = null): void
    {
        try {
            ...
            \Log::warning('BulkAnalytesActionJob: notify failed', ['error' => $e->getMessage()]);
        }
    }

    protected function setActive(AnalyteService $service, Analyte $analyte): void
    {
        $target = (bool) ($this->payload['is_active'] ?? true);
        if ((bool) $analyte->is_active === $target) return;
        $service->update($analyte, ['is_active' => $target]);
    }
}
