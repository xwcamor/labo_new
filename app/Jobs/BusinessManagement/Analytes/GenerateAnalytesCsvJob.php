 ...
 */
class GenerateAnalytesCsvJob extends BaseAnalyteExportJob
{
    ...
    protected function executeExport(Download $download): void
    {
        ...
        $tempFile = tempnam(sys_get_temp_dir(), 'analytes_csv') . '.csv';
        ...
        try {
            ...
            $headings = [
                'id'         => __('analytes.id'),
                'name'       => __('analytes.name'),
                'code'       => __('analytes.code'),
                'sort_order' => __('analytes.sort_order'),
                'is_active'  => __('analytes.is_active'),
                'slug'       => 'Slug',
            ...
            $this->buildQuery()->chunkById(1000, function ($analytes) use ($handle, $columns, $tz) {
                foreach ($analytes as $analyte) {
                    $row = array_map(fn ($col) => match ($col) {
                        ...
                        'is_active'  => $analyte->is_active ? '1' : '0',
                        'slug'       => $analyte->slug,
                        'created_at' => $analyte->created_at?->copy()->setTimezone($tz)->format(\App\Support\Tz::DATETIME_FORMAT),
                        'updated_at' => $analyte->updated_at?->copy()->setTimezone($tz)->format(\App\Support\Tz::DATETIME_FORMAT),
                        'creator'    => $analyte->creator?->name ?? '',
                        default      => $analyte->{$col} ?? '',
                    }, $columns);
                    fputcsv($handle, $row);
                }
