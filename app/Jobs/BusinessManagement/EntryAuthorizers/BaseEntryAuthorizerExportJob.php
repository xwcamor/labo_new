 ...
 *
 * Clon de BaseRegionExportJob adaptado a EntryAuthorizers (per-tenant, con relacion
 ...
 */
abstract class BaseEntryAuthorizerExportJob implements ShouldQueue
{
    ...
    public function handle(): void
    {
        ...
        ini_set('memory_limit', config('entry_authorizers.export_job_memory_limit', '512M'));
     ...
     *
     * EntryAuthorizers es per-tenant: aplicamos `withoutGlobalScopes()` y manualmente
    ...
    protected function buildQuery()
    {
        ...
        if ($this->tenantId !== null) {
            $base->where(function ($q) {
                $q->where('entry_authorizers.tenant_id', $this->tenantId)
                  ->orWhereNull('entry_authorizers.tenant_id');
            });
        }
        ...
        if ($scope === 'selected' && !empty($this->options['selected_ids'])) {
            return $base->whereIn('entry_authorizers.id', $this->options['selected_ids']);
        }
        if ($scope === 'all') {
            return $base;
        }
    ...
    protected function buildFiltersSummary(): array
    {
        ...
        if (!empty($f['name'])) {
            ...
            $out[] = ['label' => __('entry_authorizers.name'), 'value' => implode(', ', $names)];
        }
        if (!empty($f['code'])) {
            $out[] = ['label' => __('entry_authorizers.code'), 'value' => (string) $f['code']];
        }
        if (isset($f['is_active']) && $f['is_active'] !== '' && $f['is_active'] !== null) {
            $bool = filter_var($f['is_active'], FILTER_VALIDATE_BOOLEAN);
            $out[] = ['label' => __('entry_authorizers.is_active'), 'value' => $bool ? __('global.active') : __('global.inactive')];
        }
    ...
    {
        $base = Str::slug($this->options['title'] ?? __('entry_authorizers.export_filename'));
