 ...
 * dispatch viven en EntryAuthorizerService.
 *
 ...
 */
class BulkEntryAuthorizersActionJob implements ShouldQueue, ShouldBeUnique
{
    ...
    public function uniqueId(): string
    {
        ...
        return "bulk:entry_authorizers:{$this->userId}:{$this->action}:{$idsHash}";
    }

    /**
     * Umbral configurable. Prioridad: Setting global -> config/entry_authorizers.php -> 200.
    ...
    public static function asyncThreshold(): int
    {
        return \App\Models\Setting::getInt(
            ...
            (int) config('entry_authorizers.bulk_async_threshold', 200),
        );
    }
    ...
    public function handle(EntryAuthorizerService $service): void
    {
        ...
        if (!$user) {
            \Log::warning('BulkEntryAuthorizersActionJob: user not found, aborting', [
                'user_id' => $this->userId,
                'action'  => $this->action,
            ]);
        ...
        foreach (array_chunk($this->ids, 200) as $chunk) {
            $entry_authorizers = match ($this->action) {
                'restore' => EntryAuthorizer::onlyTrashed()->whereIn('id', $chunk)->get(),
                default   => EntryAuthorizer::whereIn('id', $chunk)->get(),
            };

            foreach ($entry_authorizers as $entryAuthorizer) {
                try {
                    match ($this->action) {
                        'delete'     => $service->delete($entryAuthorizer, $this->payload['reason'] ?? 'Bulk delete'),
                        'set_active' => $this->setActive($service, $entryAuthorizer),
                        'restore'    => $service->restore($entryAuthorizer),
                        default      => throw new \InvalidArgumentException("Unknown action: {$this->action}"),
                    };
                    $processed++;
                } catch (\Throwable $e) {
                    $errors++;
                    \Log::warning("BulkEntryAuthorizersActionJob: error on entryAuthorizer {$entryAuthorizer->id}", [
                        'error' => $e->getMessage(),
                    ]);
                }
            }
        ...
        \Log::info("BulkEntryAuthorizersActionJob completed", [
            'user_id'   => $this->userId,
            'action'    => $this->action,
            'processed' => $processed,
            'errors'    => $errors,
    ...
    {
        \Log::error("BulkEntryAuthorizersActionJob failed", [
            'user_id' => $this->userId,
            'action'  => $this->action,
            'total'   => count($this->ids),
    ...
    protected function notifyUser(string $status, ?string $error = null): void
    {
        try {
            ...
            \Log::warning('BulkEntryAuthorizersActionJob: notify failed', ['error' => $e->getMessage()]);
        }
    }

    protected function setActive(EntryAuthorizerService $service, EntryAuthorizer $entryAuthorizer): void
    {
        $target = (bool) ($this->payload['is_active'] ?? true);
        if ((bool) $entryAuthorizer->is_active === $target) return;
        $service->update($entryAuthorizer, ['is_active' => $target]);
    }
}
