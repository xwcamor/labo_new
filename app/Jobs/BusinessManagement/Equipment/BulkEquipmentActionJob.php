 ...
 * dispatch viven en EquipmentService.
 *
 ...
 */
class BulkEquipmentActionJob implements ShouldQueue, ShouldBeUnique
{
    ...
    public function uniqueId(): string
    {
        ...
        return "bulk:equipment:{$this->userId}:{$this->action}:{$idsHash}";
    }

    /**
     * Umbral configurable. Prioridad: Setting global -> config/equipment.php -> 200.
    ...
    public static function asyncThreshold(): int
    {
        return \App\Models\Setting::getInt(
            ...
            (int) config('equipment.bulk_async_threshold', 200),
        );
    }
    ...
    public function handle(EquipmentService $service): void
    {
        ...
        if (!$user) {
            \Log::warning('BulkEquipmentActionJob: user not found, aborting', [
                'user_id' => $this->userId,
                'action'  => $this->action,
            ]);
        ...
        foreach (array_chunk($this->ids, 200) as $chunk) {
            $equipment = match ($this->action) {
                'restore' => Equipment::onlyTrashed()->whereIn('id', $chunk)->get(),
                default   => Equipment::whereIn('id', $chunk)->get(),
            };

            foreach ($equipment as $equipment) {
                try {
                    match ($this->action) {
                        'delete'     => $service->delete($equipment, $this->payload['reason'] ?? 'Bulk delete'),
                        'set_active' => $this->setActive($service, $equipment),
                        'restore'    => $service->restore($equipment),
                        default      => throw new \InvalidArgumentException("Unknown action: {$this->action}"),
                    };
                    $processed++;
                } catch (\Throwable $e) {
                    $errors++;
                    \Log::warning("BulkEquipmentActionJob: error on equipment {$equipment->id}", [
                        'error' => $e->getMessage(),
                    ]);
                }
            }
        ...
        \Log::info("BulkEquipmentActionJob completed", [
            'user_id'   => $this->userId,
            'action'    => $this->action,
            'processed' => $processed,
            'errors'    => $errors,
    ...
    {
        \Log::error("BulkEquipmentActionJob failed", [
            'user_id' => $this->userId,
            'action'  => $this->action,
            'total'   => count($this->ids),
    ...
    protected function notifyUser(string $status, ?string $error = null): void
    {
        try {
            ...
            \Log::warning('BulkEquipmentActionJob: notify failed', ['error' => $e->getMessage()]);
        }
    }

    protected function setActive(EquipmentService $service, Equipment $equipment): void
    {
        $target = (bool) ($this->payload['is_active'] ?? true);
        if ((bool) $equipment->is_active === $target) return;
        $service->update($equipment, ['is_active' => $target]);
    }
}
