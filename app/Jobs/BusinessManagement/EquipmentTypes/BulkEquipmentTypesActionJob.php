...
namespace App\Jobs\BusinessManagement\EquipmentTypes;
 ...
 * dispatch viven en EquipmentTypeService.
 *
 ...
 */
class BulkEquipmentTypesActionJob implements ShouldQueue, ShouldBeUnique
{
    ...
    public function uniqueId(): string
    {
        ...
        return "bulk:equipmentTypes:{$this->userId}:{$this->action}:{$idsHash}";
    }

    /**
     * Umbral configurable. Prioridad: Setting global -> config/equipment_types.php -> 200.
    ...
    public static function asyncThreshold(): int
    {
        return \App\Models\Setting::getInt(
            ...
            (int) config('equipment_types.bulk_async_threshold', 200),
        );
    }
    ...
    public function handle(EquipmentTypeService $service): void
    {
        ...
        if (!$user) {
            \Log::warning('BulkEquipmentTypesActionJob: user not found, aborting', [
                'user_id' => $this->userId,
                'action'  => $this->action,
            ]);
        ...
        foreach (array_chunk($this->ids, 200) as $chunk) {
            $equipmentTypes = match ($this->action) {
                'restore' => EquipmentType::onlyTrashed()->whereIn('id', $chunk)->get(),
                default   => EquipmentType::whereIn('id', $chunk)->get(),
            };

            foreach ($equipmentTypes as $equipmentType) {
                try {
                    match ($this->action) {
                        'delete'     => $service->delete($equipmentType, $this->payload['reason'] ?? 'Bulk delete'),
                        'set_active' => $this->setActive($service, $equipmentType),
                        'restore'    => $service->restore($equipmentType),
                        default      => throw new \InvalidArgumentException("Unknown action: {$this->action}"),
                    };
                    $processed++;
                } catch (\Throwable $e) {
                    $errors++;
                    \Log::warning("BulkEquipmentTypesActionJob: error on equipmentType {$equipmentType->id}", [
                        'error' => $e->getMessage(),
                    ]);
                }
            }
        ...
        \Log::info("BulkEquipmentTypesActionJob completed", [
            'user_id'   => $this->userId,
            'action'    => $this->action,
            'processed' => $processed,
            'errors'    => $errors,
    ...
    {
        \Log::error("BulkEquipmentTypesActionJob failed", [
            'user_id' => $this->userId,
            'action'  => $this->action,
            'total'   => count($this->ids),
    ...
    protected function notifyUser(string $status, ?string $error = null): void
    {
        try {
            ...
            \Log::warning('BulkEquipmentTypesActionJob: notify failed', ['error' => $e->getMessage()]);
        }
    }

    protected function setActive(EquipmentTypeService $service, EquipmentType $equipmentType): void
    {
        $target = (bool) ($this->payload['is_active'] ?? true);
        if ((bool) $equipmentType->is_active === $target) return;
        $service->update($equipmentType, ['is_active' => $target]);
    }
}
