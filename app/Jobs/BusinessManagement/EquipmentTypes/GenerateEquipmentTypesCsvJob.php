 ...
 */
class GenerateEquipmentTypesCsvJob extends BaseEquipmentTypeExportJob
{
    ...
    protected function executeExport(Download $download): void
    {
        ...
        $tempFile = tempnam(sys_get_temp_dir(), 'equipmentTypes_csv') . '.csv';
        ...
        try {
            ...
            $this->buildQuery()->chunkById(1000, function ($equipmentTypes) use ($handle, $columns, $tz) {
                foreach ($equipmentTypes as $equipmentType) {
                    $row = array_map(fn ($col) => match ($col) {
                        ...
                        'is_active'  => $equipmentType->is_active ? '1' : '0',
                        'slug'       => $equipmentType->slug,
                        'created_at' => $equipmentType->created_at?->copy()->setTimezone($tz)->format(\App\Support\Tz::DATETIME_FORMAT),
                        'updated_at' => $equipmentType->updated_at?->copy()->setTimezone($tz)->format(\App\Support\Tz::DATETIME_FORMAT),
                        'creator'    => $equipmentType->creator?->name ?? '',
                        default      => $equipmentType->{$col} ?? '',
                    }, $columns);
                    fputcsv($handle, $row);
                }
            }, 'equipment_types.id', 'id');
