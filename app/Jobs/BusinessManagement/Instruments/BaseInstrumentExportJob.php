 ...
 *
 * Clon de BaseRegionExportJob adaptado a Instruments (per-tenant, con relacion
 ...
 */
abstract class BaseInstrumentExportJob implements ShouldQueue
{
    ...
    public function handle(): void
    {
        ...
        ini_set('memory_limit', config('instruments.export_job_memory_limit', '512M'));
     ...
     *
     * Instruments es per-tenant: aplicamos `withoutGlobalScopes()` y manualmente
    ...
    protected function buildQuery()
    {
        ...
        // instruments de TODOS los workspaces. Un super no tiene tenant y ve
        // todo, que es lo esperado.
        if ($this->tenantId !== null) {
            $base->where(function ($q) {
                $q->where('instruments.tenant_id', $this->tenantId)
                  ->orWhereNull('instruments.tenant_id');
            });
        }
        ...
        if ($scope === 'selected' && !empty($this->options['selected_ids'])) {
            return $base->whereIn('instruments.id', $this->options['selected_ids']);
        }
        if ($scope === 'all') {
            return $base;
        }
    ...
    protected function buildFiltersSummary(): array
    {
        ...
        if (isset($f['is_active']) && $f['is_active'] !== '' && $f['is_active'] !== null) {
            ...
            $out[] = ['label' => __('instruments.is_active'), 'value' => $bool ? __('global.active') : __('global.inactive')];
        }
    ...
    {
        $base = Str::slug($this->options['title'] ?? __('instruments.export_filename'));
