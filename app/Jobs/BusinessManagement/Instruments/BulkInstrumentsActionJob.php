 ...
 * dispatch viven en InstrumentService.
 *
 ...
 */
class BulkInstrumentsActionJob implements ShouldQueue, ShouldBeUnique
{
    ...
    public function uniqueId(): string
    {
        ...
        return "bulk:instruments:{$this->userId}:{$this->action}:{$idsHash}";
    }

    /**
     * Umbral configurable. Prioridad: Setting global -> config/instruments.php -> 200.
    ...
    public static function asyncThreshold(): int
    {
        return \App\Models\Setting::getInt(
            ...
            (int) config('instruments.bulk_async_threshold', 200),
        );
    }
    ...
    public function handle(InstrumentService $service): void
    {
        ...
        if (!$user) {
            \Log::warning('BulkInstrumentsActionJob: user not found, aborting', [
                'user_id' => $this->userId,
                'action'  => $this->action,
            ]);
        ...
        foreach (array_chunk($this->ids, 200) as $chunk) {
            $instruments = match ($this->action) {
                'restore' => Instrument::onlyTrashed()->whereIn('id', $chunk)->get(),
                default   => Instrument::whereIn('id', $chunk)->get(),
            };

            foreach ($instruments as $instrument) {
                try {
                    match ($this->action) {
                        'delete'     => $service->delete($instrument, $this->payload['reason'] ?? 'Bulk delete'),
                        'set_active' => $this->setActive($service, $instrument),
                        'restore'    => $service->restore($instrument),
                        default      => throw new \InvalidArgumentException("Unknown action: {$this->action}"),
                    };
                    $processed++;
                } catch (\Throwable $e) {
                    $errors++;
                    \Log::warning("BulkInstrumentsActionJob: error on instrument {$instrument->id}", [
                        'error' => $e->getMessage(),
                    ]);
                }
            }
        ...
        \Log::info("BulkInstrumentsActionJob completed", [
            'user_id'   => $this->userId,
            'action'    => $this->action,
            'processed' => $processed,
            'errors'    => $errors,
    ...
    {
        \Log::error("BulkInstrumentsActionJob failed", [
            'user_id' => $this->userId,
            'action'  => $this->action,
            'total'   => count($this->ids),
    ...
    protected function notifyUser(string $status, ?string $error = null): void
    {
        try {
            ...
            \Log::warning('BulkInstrumentsActionJob: notify failed', ['error' => $e->getMessage()]);
        }
    }

    protected function setActive(InstrumentService $service, Instrument $instrument): void
    {
        $target = (bool) ($this->payload['is_active'] ?? true);
        if ((bool) $instrument->is_active === $target) return;
        $service->update($instrument, ['is_active' => $target]);
    }
}
