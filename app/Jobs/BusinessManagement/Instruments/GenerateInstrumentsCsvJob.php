 ...
 */
class GenerateInstrumentsCsvJob extends BaseInstrumentExportJob
{
    ...
    protected function executeExport(Download $download): void
    {
        ...
        $tempFile = tempnam(sys_get_temp_dir(), 'instruments_csv') . '.csv';
        ...
        try {
            ...
            $this->buildQuery()->chunkById(1000, function ($instruments) use ($handle, $columns, $tz) {
                foreach ($instruments as $instrument) {
                    $row = array_map(fn ($col) => match ($col) {
                        ...
                        'created_at' => $instrument->created_at?->copy()->setTimezone($tz)->format(\App\Support\Tz::DATETIME_FORMAT),
                        'updated_at' => $instrument->updated_at?->copy()->setTimezone($tz)->format(\App\Support\Tz::DATETIME_FORMAT),
                        'creator'    => $instrument->creator?->name ?? '',
                        default      => $instrument->{$col} ?? '',
                    }, $columns);
                    fputcsv($handle, $row);
                }
            }, 'instruments.id', 'id');
