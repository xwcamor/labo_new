 ...
 * dispatch viven en SamplerService.
 *
 ...
 */
class BulkSamplersActionJob implements ShouldQueue, ShouldBeUnique
{
    ...
    public function uniqueId(): string
    {
        ...
        return "bulk:samplers:{$this->userId}:{$this->action}:{$idsHash}";
    }

    /**
     * Umbral configurable. Prioridad: Setting global -> config/samplers.php -> 200.
    ...
    public static function asyncThreshold(): int
    {
        return \App\Models\Setting::getInt(
            ...
            (int) config('samplers.bulk_async_threshold', 200),
        );
    }
    ...
    public function handle(SamplerService $service): void
    {
        ...
        if (!$user) {
            \Log::warning('BulkSamplersActionJob: user not found, aborting', [
                'user_id' => $this->userId,
                'action'  => $this->action,
            ]);
        ...
        foreach (array_chunk($this->ids, 200) as $chunk) {
            $samplers = match ($this->action) {
                'restore' => Sampler::onlyTrashed()->whereIn('id', $chunk)->get(),
                default   => Sampler::whereIn('id', $chunk)->get(),
            };

            foreach ($samplers as $sampler) {
                try {
                    match ($this->action) {
                        'delete'     => $service->delete($sampler, $this->payload['reason'] ?? 'Bulk delete'),
                        'set_active' => $this->setActive($service, $sampler),
                        'restore'    => $service->restore($sampler),
                        default      => throw new \InvalidArgumentException("Unknown action: {$this->action}"),
                    };
                    $processed++;
                } catch (\Throwable $e) {
                    $errors++;
                    \Log::warning("BulkSamplersActionJob: error on sampler {$sampler->id}", [
                        'error' => $e->getMessage(),
                    ]);
                }
            }
        ...
        \Log::info("BulkSamplersActionJob completed", [
            'user_id'   => $this->userId,
            'action'    => $this->action,
            'processed' => $processed,
            'errors'    => $errors,
    ...
    {
        \Log::error("BulkSamplersActionJob failed", [
            'user_id' => $this->userId,
            'action'  => $this->action,
            'total'   => count($this->ids),
    ...
    protected function notifyUser(string $status, ?string $error = null): void
    {
        try {
            ...
            \Log::warning('BulkSamplersActionJob: notify failed', ['error' => $e->getMessage()]);
        }
    }

    protected function setActive(SamplerService $service, Sampler $sampler): void
    {
        $target = (bool) ($this->payload['is_active'] ?? true);
        if ((bool) $sampler->is_active === $target) return;
        $service->update($sampler, ['is_active' => $target]);
    }
}
