 ...
 * dispatch viven en SignatureService.
 *
 ...
 */
class BulkSignaturesActionJob implements ShouldQueue, ShouldBeUnique
{
    ...
    public function uniqueId(): string
    {
        ...
        return "bulk:signatures:{$this->userId}:{$this->action}:{$idsHash}";
    }

    /**
     * Umbral configurable. Prioridad: Setting global -> config/signatures.php -> 200.
    ...
    public static function asyncThreshold(): int
    {
        return \App\Models\Setting::getInt(
            ...
            (int) config('signatures.bulk_async_threshold', 200),
        );
    }
    ...
    public function handle(SignatureService $service): void
    {
        ...
        if (!$user) {
            \Log::warning('BulkSignaturesActionJob: user not found, aborting', [
                'user_id' => $this->userId,
                'action'  => $this->action,
            ]);
        ...
        foreach (array_chunk($this->ids, 200) as $chunk) {
            $signatures = match ($this->action) {
                'restore' => Signature::onlyTrashed()->whereIn('id', $chunk)->get(),
                default   => Signature::whereIn('id', $chunk)->get(),
            };

            foreach ($signatures as $signature) {
                try {
                    match ($this->action) {
                        'delete'     => $service->delete($signature, $this->payload['reason'] ?? 'Bulk delete'),
                        'set_active' => $this->setActive($service, $signature),
                        'restore'    => $service->restore($signature),
                        default      => throw new \InvalidArgumentException("Unknown action: {$this->action}"),
                    };
                    $processed++;
                } catch (\Throwable $e) {
                    $errors++;
                    \Log::warning("BulkSignaturesActionJob: error on signature {$signature->id}", [
                        'error' => $e->getMessage(),
                    ]);
                }
            }
        ...
        \Log::info("BulkSignaturesActionJob completed", [
            'user_id'   => $this->userId,
            'action'    => $this->action,
            'processed' => $processed,
            'errors'    => $errors,
    ...
    {
        \Log::error("BulkSignaturesActionJob failed", [
            'user_id' => $this->userId,
            'action'  => $this->action,
            'total'   => count($this->ids),
    ...
    protected function notifyUser(string $status, ?string $error = null): void
    {
        try {
            ...
            \Log::warning('BulkSignaturesActionJob: notify failed', ['error' => $e->getMessage()]);
        }
    }

    protected function setActive(SignatureService $service, Signature $signature): void
    {
        $target = (bool) ($this->payload['is_active'] ?? true);
        if ((bool) $signature->is_active === $target) return;
        $service->update($signature, ['is_active' => $target]);
    }
}
