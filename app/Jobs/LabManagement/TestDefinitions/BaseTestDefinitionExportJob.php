 ...
 *
 * Clon de BaseRegionExportJob adaptado a TestDefinitions (per-tenant, con relacion
 ...
 */
abstract class BaseTestDefinitionExportJob implements ShouldQueue
{
    ...
    public function handle(): void
    {
        ...
        ini_set('memory_limit', config('test_definitions.export_job_memory_limit', '512M'));
     ...
     *
     * TestDefinitions es per-tenant: aplicamos `withoutGlobalScopes()` y manualmente
    ...
    protected function buildQuery()
    {
        ...
        $base = TestDefinition::query();

        // El worker NO tiene sesión, así que el global scope de
        // BelongsToTenantOrGlobal no se aplica solo: hay que repetir su
        // regla a mano (propios + globales) con el tenant que se capturó
        // al encolar. Sin esto un admin se lleva en su exportación los
        // test_definitions de TODOS los workspaces. Un super no tiene tenant y ve
        // todo, que es lo esperado.
        if ($this->tenantId !== null) {
            $base->where(function ($q) {
                $q->where('test_definitions.tenant_id', $this->tenantId)
                  ->orWhereNull('test_definitions.tenant_id');
            });
        }
        ...
        // Sin este eager-load la columna Grupo dispara una consulta por fila.
        if (in_array('group', $columns)) {
            $base->with('group:id,name,code');
        }

        if ($scope === 'selected' && !empty($this->options['selected_ids'])) {
            return $base->whereIn('test_definitions.id', $this->options['selected_ids']);
        }
        if ($scope === 'all') {
            return $base;
        }
    ...
    protected function buildFiltersSummary(): array
    {
        ...
        if (!empty($f['name'])) {
            ...
            $out[] = ['label' => __('test_definitions.name'), 'value' => implode(', ', $names)];
        }
        if (!empty($f['code'])) {
            $out[] = ['label' => __('test_definitions.code'), 'value' => (string) $f['code']];
        }
        if (isset($f['is_active']) && $f['is_active'] !== '' && $f['is_active'] !== null) {
            $bool = filter_var($f['is_active'], FILTER_VALIDATE_BOOLEAN);
            $out[] = ['label' => __('test_definitions.is_active'), 'value' => $bool ? __('global.active') : __('global.inactive')];
        }
    ...
    {
        $base = Str::slug($this->options['title'] ?? __('test_definitions.export_filename'));
