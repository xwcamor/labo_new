 ...
 *
 * Clon de BaseRegionExportJob adaptado a TestGroups (per-tenant, con relacion
 ...
 */
abstract class BaseTestGroupExportJob implements ShouldQueue
{
    ...
    public function handle(): void
    {
        ...
        ini_set('memory_limit', config('test_groups.export_job_memory_limit', '512M'));
     ...
     *
     * TestGroups es per-tenant: aplicamos `withoutGlobalScopes()` y manualmente
    ...
    protected function buildQuery()
    {
        ...
        // test_groups de TODOS los workspaces. Un super no tiene tenant y ve
        // todo, que es lo esperado.
        if ($this->tenantId !== null) {
            $base->where(function ($q) {
                $q->where('test_groups.tenant_id', $this->tenantId)
                  ->orWhereNull('test_groups.tenant_id');
            });
        }
        ...
        // Solo si la pidieron: es un subquery por fila y la mayoría de las
        // exportaciones no la necesita.
        if (in_array('tests_count', $columns)) {
            $base->withCount('tests');
        }

        if ($scope === 'selected' && !empty($this->options['selected_ids'])) {
            return $base->whereIn('test_groups.id', $this->options['selected_ids']);
        }
        if ($scope === 'all') {
            return $base;
        }
    ...
    protected function buildFiltersSummary(): array
    {
        ...
        if (!empty($f['name'])) {
            ...
            $out[] = ['label' => __('test_groups.name'), 'value' => implode(', ', $names)];
        }
        if (!empty($f['code'])) {
            $out[] = ['label' => __('test_groups.code'), 'value' => (string) $f['code']];
        }
        if (isset($f['is_active']) && $f['is_active'] !== '' && $f['is_active'] !== null) {
            $bool = filter_var($f['is_active'], FILTER_VALIDATE_BOOLEAN);
            $out[] = ['label' => __('test_groups.is_active'), 'value' => $bool ? __('global.active') : __('global.inactive')];
        }
    ...
    {
        $base = Str::slug($this->options['title'] ?? __('test_groups.export_filename'));
