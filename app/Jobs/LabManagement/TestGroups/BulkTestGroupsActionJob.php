 ...
 * dispatch viven en TestGroupService.
 *
 ...
 */
class BulkTestGroupsActionJob implements ShouldQueue, ShouldBeUnique
{
    ...
    public function uniqueId(): string
    {
        ...
        return "bulk:test_groups:{$this->userId}:{$this->action}:{$idsHash}";
    }

    /**
     * Umbral configurable. Prioridad: Setting global -> config/test_groups.php -> 200.
    ...
    public static function asyncThreshold(): int
    {
        return \App\Models\Setting::getInt(
            ...
            (int) config('test_groups.bulk_async_threshold', 200),
        );
    }
    ...
    public function handle(TestGroupService $service): void
    {
        ...
        if (!$user) {
            \Log::warning('BulkTestGroupsActionJob: user not found, aborting', [
                'user_id' => $this->userId,
                'action'  => $this->action,
            ]);
        ...
        foreach (array_chunk($this->ids, 200) as $chunk) {
            $test_groups = match ($this->action) {
                'restore' => TestGroup::onlyTrashed()->whereIn('id', $chunk)->get(),
                default   => TestGroup::whereIn('id', $chunk)->get(),
            };

            foreach ($test_groups as $testGroup) {
                try {
                    match ($this->action) {
                        'delete'     => $service->delete($testGroup, $this->payload['reason'] ?? 'Bulk delete'),
                        'set_active' => $this->setActive($service, $testGroup),
                        'restore'    => $service->restore($testGroup),
                        default      => throw new \InvalidArgumentException("Unknown action: {$this->action}"),
                    };
                    $processed++;
                } catch (\Throwable $e) {
                    $errors++;
                    \Log::warning("BulkTestGroupsActionJob: error on testGroup {$testGroup->id}", [
                        'error' => $e->getMessage(),
                    ]);
                }
            }
        ...
        \Log::info("BulkTestGroupsActionJob completed", [
            'user_id'   => $this->userId,
            'action'    => $this->action,
            'processed' => $processed,
            'errors'    => $errors,
    ...
    {
        \Log::error("BulkTestGroupsActionJob failed", [
            'user_id' => $this->userId,
            'action'  => $this->action,
            'total'   => count($this->ids),
    ...
    protected function notifyUser(string $status, ?string $error = null): void
    {
        try {
            ...
            \Log::warning('BulkTestGroupsActionJob: notify failed', ['error' => $e->getMessage()]);
        }
    }

    protected function setActive(TestGroupService $service, TestGroup $testGroup): void
    {
        $target = (bool) ($this->payload['is_active'] ?? true);
        if ((bool) $testGroup->is_active === $target) return;
        $service->update($testGroup, ['is_active' => $target]);
    }
}
