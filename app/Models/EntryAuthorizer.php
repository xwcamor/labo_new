...
/**
 * EntryAuthorizer — catálogo de marcas/fabricantes de transformadores (ABB, Siemens…).
 *
 ...
 * `tenant_id`.
 */
class EntryAuthorizer extends Model
{
    use HasFactory, SoftDeletes, Auditable, BelongsToTenantOrGlobal, HasFavorites, \App\Traits\Lockable;

    protected string $auditModule = 'entry_authorizers';

    protected $fillable = [
        'slug', 'name', 'code', 'image', 'is_active', 'sort_order', 'tenant_id',
    ...
    /**
     * scopeFilter — mismo patrón que Customer, sobre la tabla entry_authorizers.
    ...
    {
        return [
            ['key' => 'name',       'label' => __('entry_authorizers.name'),     'type' => 'string',  'operators' => ['=', '!=', 'contains']],
            ['key' => 'code',       'label' => __('entry_authorizers.code'),     'type' => 'string',  'operators' => ['=', '!=', 'contains']],
            ['key' => 'is_active',  'label' => __('entry_authorizers.is_active'), 'type' => 'boolean', 'operators' => ['=']],
