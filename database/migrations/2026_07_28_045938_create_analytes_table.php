...
/**
 * analytes — tabla base generada por make:module.
...
return new class extends Migration {
    public function up(): void
    {
        Schema::create('analytes', function (Blueprint $table) {
            ...
            $table->index(['is_active', 'created_at'], 'idx_analytes_tenant_active_created');
            $table->index('created_at', 'idx_analytes_created_at');
            $table->index('updated_at', 'idx_analytes_updated_at');
            $table->index('deleted_at', 'idx_analytes_deleted_at');
            $table->index('created_by', 'idx_analytes_created_by');
            $table->index('is_active',  'idx_analytes_is_active');
        });
        ...
        if ($driver === 'pgsql') {
            DB::statement(
                "CREATE UNIQUE INDEX analytes_name_unique_active " .
                "ON analytes (tenant_id, unaccent_immutable(LOWER(name))) " .
                "WHERE deleted_at IS NULL"
            );
            // varchar_pattern_ops para `WHERE name LIKE 'X%'` eficiente.
            DB::statement('CREATE INDEX idx_analytes_name_pattern ON analytes (name varchar_pattern_ops)');
        } elseif ($driver === 'sqlite') {
            DB::statement(
                "CREATE UNIQUE INDEX analytes_name_unique_active " .
                "ON analytes (tenant_id, LOWER(name)) " .
                "WHERE deleted_at IS NULL"
            );
        }
    }
