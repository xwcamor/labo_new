# Lo que TrafoDex tiene que construir

> Este documento vive acá porque el repo de TrafoDex quedó sin tocar a pedido
> del dueño. Cuando llegue el momento de implementar la fase 7, copiarlo a
> `trafodex/docs/` o abrir el trabajo desde acá.
...
> [`04-INTEGRACION-TRAFODEX.md`](04-INTEGRACION-TRAFODEX.md).
>
...
## Dos cosas más que hay que arreglar en TrafoDex

Salieron al construir TR LAB sobre una copia de su núcleo. No tienen que ver
con la integración, pero afectan su despliegue.

### 1. `npm run build` falla en Linux por mayúsculas — BLOQUEA EL DEPLOY

La carpeta es `resources/js/Utils/` (con U mayúscula), pero dos archivos la
importan en minúscula:

- `resources/js/app.js:10` → `@/utils/severity`
- `resources/js/Pages/Customers/Index.vue:22` → `@/utils/filterTree`

Windows no distingue mayúsculas y por eso no se nota en desarrollo. En Linux
Vite corta con `Could not load .../resources/js/utils/severity`. **El droplet
es Linux**, así que revienta el primer build de producción. Arreglo: cambiar
las dos importaciones a `@/Utils/...`.

### 2. Dependencia de despliegue con el laboratorio

El sistema Rails del laboratorio comparte **14 tablas** con `tr_app_development`
(la base del TRAPP viejo): no solo muestras, también `customers`, `countries`,
`oil_types`, `marks`, `conmutation_types` y la jerarquía de sedes. Diez de esos
modelos heredan de una clase abstracta `Primary2` y no lo declaran de forma
visible.

**Cuando TrafoDex reemplace al TRAPP viejo en producción, el laboratorio se
queda sin clientes y sin catálogos**, porque nunca los tuvo propios. El
desacople del laboratorio (fase 1 de este plan) tiene que estar hecho antes de
ese corte. Ver [`01-ANALISIS-SISTEMA-ACTUAL.md`](01-ANALISIS-SISTEMA-ACTUAL.md) §4.
