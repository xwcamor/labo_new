<script setup>
/**
 * Tabla editable in-line del flujo Edit-All de EntryAuthorizers. Recibe `draft` por
...
<template>
    <table v-if="draft.length > 0" class="edit-table">
        <thead>
            <tr>
                ...
                <th class="col-name">{{ $t('entry_authorizers.table_headers.editable_name') }}</th>
                <th class="col-cod">{{ $t('entry_authorizers.code') }}</th>
                <th class="col-status">{{ $t('entry_authorizers.table_headers.editable_status') }}</th>
            </tr>
        </thead>
        <tbody>
            <tr
    ...
    <div v-else class="empty">
        {{ $t('entry_authorizers.edit_all_no_results') }}
    </div>
</template>
