<script setup>
/**
 * Tabla editable in-line del flujo Edit-All de TestDefinitions. Recibe `draft` por
...
<template>
    <table v-if="draft.length > 0" class="edit-table">
        <thead>
            <tr>
                ...
                <th class="col-name">{{ $t('test_definitions.table_headers.editable_name') }}</th>
                <th class="col-cod">{{ $t('test_definitions.code') }}</th>
                <th class="col-status">{{ $t('test_definitions.table_headers.editable_status') }}</th>
            </tr>
        </thead>
        <tbody>
            <tr
    ...
    <div v-else class="empty">
        {{ $t('test_definitions.edit_all_no_results') }}
    </div>
</template>
