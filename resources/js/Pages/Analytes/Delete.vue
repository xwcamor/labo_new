...
const submit = () => {
    form.delete(route('business_management.analytes.deleteSave', props.analyte.slug), {
        preserveScroll: true,
    });
};
</script>
...
<template>
    <Head :title="$t('global.delete') + ' — ' + $t('analytes.singular')" />

    <DeletePage
        :back-href="route('business_management.analytes.index')"
        :title="$t('global.delete') + ' ' + $t('analytes.record')"
        :subtitle="analyte.name"
    ...
    >
        ...
        <template #summary>
            <DeleteSummaryRow :label="$t('analytes.name')">{{ analyte.name }}</DeleteSummaryRow>
            <DeleteSummaryRow v-if="analyte.code" :label="$t('analytes.code')">
                <code>{{ analyte.code }}</code>
            </DeleteSummaryRow>
            <DeleteSummaryRow :label="$t('analytes.is_active')">
                <Tag :color="analyte.is_active ? 'success' : 'error'" :bordered="false">
