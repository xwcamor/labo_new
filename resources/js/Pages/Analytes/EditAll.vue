...
import { SaveOutlined, UndoOutlined, EditOutlined, RadarChartOutlined } from '@ant-design/icons-vue';

import AppLayout from '@/Layouts/AppLayout.vue';
import SectionHeader from '@/Components/Common/SectionHeader.vue';
import EditAllFooter from '@/Components/Common/EditAllFooter.vue';
import AnalytesEditAllTable from '@/Components/Analytes/AnalytesEditAllTable.vue';
...
/**
 * Edit-All de Analytes. Mantenemos snapshot `original` + `draft` mutable
...
const source = computed(() => props.analytes.data);
...
const saveAll = () => {
    ...
    router.post(
        route('business_management.analytes.edit_all.update'),
...
<template>
    <Head :title="$t('analytes.edit_all_title')" />

    <div class="edit-all sap-form">
        <SectionHeader
            :back-href="route('business_management.analytes.index')"
            :title="$t('global.edit_all') + ' — ' + $t('analytes.plural')"
            :subtitle="$t('analytes.edit_all_subtitle')"
        >
        ...
        <Alert
            ...
            type="error"
            show-icon
            :message="$t('analytes.name_unique')"
            class="status-bar"
        />

        <Card :bodyStyle="{ padding: 0 }" class="edit-table-card">
            <AnalytesEditAllTable
                v-model:draft="draft"
                :is-dirty="isDirty"
                :duplicate-rows="duplicateRows"
            />

        <div v-if="analytes.total > analytes.per_page" class="edit-pagination">
            <Pagination
                :current="analytes.current_page"
                :pageSize="analytes.per_page"
                :total="analytes.total"
        ...
        <EditAllFooter
            :discard-label="$t('analytes.edit_all_discard')"
            :save-label="$t('analytes.edit_all_save_all')"
            :discard-disabled="dirtyCount === 0"
            :save-disabled="dirtyCount === 0 || duplicateRows.size > 0"
            :submitting="submitting"
            :status-text="dirtyCount > 0 ? $tc('analytes.edit_all_changes', dirtyCount) : ''"
            @discard="discardAll"
            @save="saveAll"
        />
