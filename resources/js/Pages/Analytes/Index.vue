...
import AnalytesBulkBar from '@/Components/Analytes/AnalytesBulkBar.vue';
import AnalytesBulkDeleteModal from '@/Components/Analytes/AnalytesBulkDeleteModal.vue';
import AnalytesFavoriteCell from '@/Components/Analytes/AnalytesFavoriteCell.vue';
...
// El toolbar inline de Analytes (no usa ModuleToolbar) repite manualmente
// estos gates para no mostrar botones que no funcionan al user free/basic.
const { canUse: canUsePlanFeature } = usePlanFeatures();

import {
    analytesFilterFields, analytesEmptyFilters, hydrateAnalytesFilters,
    analytesFiltersToQuery, analytesFiltersSummary,
    serializeSavedFilters, deserializeSavedFilters,
} from './config/filters';
import { analytesTableColumns } from './config/columns';
import { analytesExportableColumns, analytesExportEndpoints } from './config/exports';
...
const {
    ...
    hydrate:       hydrateAnalytesFilters,
    toQuery:       analytesFiltersToQuery,
    summary:       analytesFiltersSummary,
    empty:         analytesEmptyFilters,
    only:          ['analytes', 'filters'],
    t,
});
...
const { counterLabel } = useModuleListMeta({
    pagination: computed(() => props.analytes),
    hasActiveFilters,
    t,
});
...
const allColumns = computed(() =>
    analytesTableColumns(t, { isSuper: isSuper.value, isMobile: isMobileScreen.value }),
);
const { visibleColumnKeys, columns } = useColumnPreferences(allColumns);

// ─── Paginacion + sort ──────────────────────────────────────────────────────
const tablePagination = computed(() => ({
    current:  props.analytes.current_page,
    pageSize: props.analytes.per_page,
    total:    props.analytes.total,
...
const applyInlineFilters = (cleaned) => {
    ...
    inlineTimer = setTimeout(() => {
        ...
        router.get(route('business_management.analytes.index'), data, {
            preserveScroll: true,
            preserveState: true,
...
const VIEW_KEY = 'analytes_view_mode';
...
useModuleUndoToast('business_management.analytes.undo_last_delete');

// ─── Favoritos polimorficos ────────────────────────────────────────────────
const { submitting: favoriteSubmitting, toggle: toggleFavorite } = useModuleFavorites('analytes', 'analytes');
...
const {
    ...
    bulkSetActiveRoute: 'business_management.analytes.bulk_set_active',
    bulkDeleteRoute:    'business_management.analytes.bulk_delete',
    resourceLabel:      t('analytes.records'),
...
const duplicate = (record) => {
    ...
    router.post(route('business_management.analytes.duplicate', record.slug), {}, {
        preserveScroll: true,
...
const exportableColumns = computed(() => analytesExportableColumns(t, { isSuper: isSuper.value }));
const exportEndpoints   = computed(() => analytesExportEndpoints());
...
<template>
    ...
    <div class="sap-index">
        ...
        <Drawer v-model:open="filtersOpen" :title="$t('global.filters')" placement="right" :width="380">
            <FilterBar :fields="filterFields" v-model="filters" storage-key="analytes" />
        </Drawer>
        ...
        <div class="mi-tabletoolbar">
            ...
            <div class="mi-tabletoolbar__right">
                <label class="mi-bar mi-bar--toolbar" :class="{ 'is-active': quickSearch }">
                    ...
                    <input v-model="quickSearch" class="mi-bar__input" :placeholder="$t('global.search_in', { item: $t('analytes.singular').toLowerCase() })" autocomplete="off" spellcheck="false" type="text" />
                ...
                <Tooltip v-if="can('analytes.export')" :title="$t('global.export_hint')" data-tour="export-import">
                ...
                <Dropdown :trigger="['click']" placement="bottomRight">
                    ...
                    <template #overlay>
                        <Menu>
                            <MenuItem v-if="can('analytes.create') && can('analytes.import') && canUsePlanFeature('imports')" key="import" @click="importOpen = true"><UploadOutlined /> {{ $t('global.import') }}</MenuItem>
                            <MenuItem v-if="can('analytes.edit') && canUsePlanFeature('edit_all')" key="editall" @click="router.visit(route('business_management.analytes.edit_all'))"><FormOutlined /> {{ $t('global.edit_all') }}</MenuItem>
                            <MenuItem v-if="isSuper" key="trash" @click="router.visit(route('business_management.analytes.trash'))"><InboxOutlined /> {{ $t('global.view_deleted') }}</MenuItem>
        ...
        <Card :bodyStyle="{ padding: 0 }" class="grid-card">
            ...
            <ResponsiveTable
                :loading="isFetching"
                :dataSource="analytes.data"
                :columns="columns"
                :pagination="tablePagination"
                :row-selection="(can('analytes.delete') || can('analytes.edit')) ? rowSelection : null"
            ...
            >
                ...
                <template #bodyCell="{ column, record, text, isMobile, compact }">
                    <AnalytesFavoriteCell
                        v-if="column.key === 'favorite'"
                        :record="record"
                        :submitting="favoriteSubmitting"
                        :data-tour="record === analytes.data[0] ? 'favorites' : null"
                    ...
                    <template v-else-if="column.key === 'name'">
                        <div class="lead">
                            <div class="lead__txt">
                                <Link :href="route('business_management.analytes.show', record.slug)" class="lead__name lead__link">{{ record.name }}</Link>
        ...
        <AnalytesBulkDeleteModal
            v-model:open="bulkOpen"
            v-model:reason="bulkReason"
            :count="selectedRowKeys.length"
            :submitting="bulkSubmitting"
            :error-msg="bulkError"
            :resource-label="selectedRowKeys.length === 1 ? $t('analytes.record') : $t('analytes.records')"
        ...
        <ExportDialog
            ...
            :default-title="$t('analytes.export_title')"
            :endpoints="exportEndpoints"
            :limits="exportLimits"
            :total-rows="analytes.total ?? 0"
            :total-unfiltered="analytes.total_unfiltered ?? analytes.total ?? 0"
        />
