...
import AnalytesTrashBulkBar from '@/Components/Analytes/AnalytesTrashBulkBar.vue';
import AnalytesForceDeleteModal from '@/Components/Analytes/AnalytesForceDeleteModal.vue';

import { useModuleRestore } from '@/Composables/useModuleRestore';
import { useDateFormat } from '@/Composables/useDateFormat';
import { useI18n } from '@/Plugins/i18n';
import { analytesTrashColumns } from './config/trashColumns';
...
if (!isSuper) {
    router.visit(route('business_management.analytes.index'));
}
...
const {
    ...
    restoreRouteName:     'business_management.analytes.restore',
    bulkRestoreRouteName: 'business_management.analytes.bulk_restore',
});
...
const openForceDelete = (analyte) => {
    forceDeleteTarget.value = analyte;
...
const submitForceDelete = () => {
    ...
    forceDeleteErrors.value = {};
    router.delete(
        route('business_management.analytes.force_delete', forceDeleteTarget.value.slug),
        {
...
const columns = computed(() => analytesTrashColumns(t));
...
const subtitle = computed(() => {
    const word = props.analytes.total === 1 ? t('global.record') : t('global.records');
    return `${props.analytes.total} ${word} · ${t('global.super_only')}`;
});
</script>

<template>
    <Head :title="$t('global.view_deleted') + ' — ' + $t('analytes.plural')" />

    <div v-if="isSuper" class="sap-form trash-page">
        <SectionHeader
            :back-href="route('business_management.analytes.index')"
            :title="$t('global.view_deleted') + ' — ' + $t('analytes.plural')"
            :subtitle="subtitle"
        ...
        <Card :bodyStyle="{ padding: 0 }" class="grid-card">
                    ...
                    <template v-else-if="column.key === 'actions'">
                        <Space :size="4">
                            <Popconfirm
                                ...
                                :description="$t('analytes.restore_hint')"
            ...
            <Empty
                v-if="analytes.data.length === 0"
                :description="$t('global.no_deleted_records')"
                style="padding: 48px 16px"
            />
        </Card>

        <AnalytesTrashBulkBar
