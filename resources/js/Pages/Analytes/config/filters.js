...
/**
 * Schema de filtros del módulo Analytes (catálogo global de marcas).
 * Campos de dominio: name + code + is_active. Mismo patrón que Regions/Tenants.
 */
export const analytesFilterFields = (t) => [
    { key: 'name',           label: t('analytes.filter_name'), type: 'tags' },
    { key: 'code',           label: t('analytes.code'),        type: 'text' },
    { key: 'is_active',      label: t('analytes.is_active'),   type: 'select', options: [
...
export const analytesEmptyFilters = () => ({
    name: [],
    code: '',
    is_active: null,
    created_at: null,
...
export const hydrateAnalytesFilters = (server) => ({
...
export const analytesFiltersToQuery = (f) => ({
...
export const analytesFiltersSummary = (f, t) => {
    const parts = [];
    if (f.name?.length)        parts.push(`${t('analytes.filter_name')}: ${f.name.join(', ')}`);
    if (f.code)                parts.push(`${t('analytes.code')}: ${f.code}`);
    if (f.is_active !== null && f.is_active !== undefined) {
        parts.push(`${t('analytes.is_active')}: ${f.is_active ? t('global.active') : t('global.inactive')}`);
    }
