<script setup>
/**
 * Alta y edición de la marca, en diálogo (regla Fiori de la casa: menos de
 * 7 campos no merece página completa — se abre sobre el listado o la ficha).
 *
 * A diferencia del piloto (TestGroups), el código aquí es texto libre y
 * opcional: es la abreviatura comercial de la marca, no un identificador
 * técnico que otras piezas referencien, así que no se deriva del nombre ni
 * queda de solo lectura al editar.
 */
...
const submit = () => {
    ...
    if (isEdit.value) {
        form.put(route('business_management.brands.update', props.record.slug), opciones);
    } else {
        form.post(route('business_management.brands.store'), opciones);
    }
};
</script>
