...
const submit = () => {
    form.delete(route('business_management.entry_authorizers.deleteSave', props.entryAuthorizer.slug), {
        preserveScroll: true,
    });
};
</script>
...
<template>
    <Head :title="$t('global.delete') + ' — ' + $t('entry_authorizers.singular')" />

    <DeletePage
        :back-href="route('business_management.entry_authorizers.index')"
        :title="$t('global.delete') + ' ' + $t('entry_authorizers.record')"
        :subtitle="entryAuthorizer.name"
    ...
    >
        ...
        <template #summary>
            <DeleteSummaryRow :label="$t('entry_authorizers.name')">{{ entryAuthorizer.name }}</DeleteSummaryRow>
            <DeleteSummaryRow v-if="entryAuthorizer.code" :label="$t('entry_authorizers.code')">
                <code>{{ entryAuthorizer.code }}</code>
            </DeleteSummaryRow>
            <DeleteSummaryRow :label="$t('entry_authorizers.is_active')">
                <Tag :color="entryAuthorizer.is_active ? 'success' : 'error'" :bordered="false">
