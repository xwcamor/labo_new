...
import EntryAuthorizersEditAllTable from '@/Components/EntryAuthorizers/EntryAuthorizersEditAllTable.vue';
...
/**
 * Edit-All de EntryAuthorizers. Mantenemos snapshot `original` + `draft` mutable
...
const source = computed(() => props.entry_authorizers.data);
...
const saveAll = () => {
    ...
    router.post(
        route('business_management.entry_authorizers.edit_all.update'),
...
<template>
    <Head :title="$t('entry_authorizers.edit_all_title')" />

    <div class="edit-all sap-form">
        <SectionHeader
            :back-href="route('business_management.entry_authorizers.index')"
            :title="$t('global.edit_all') + ' — ' + $t('entry_authorizers.plural')"
            :subtitle="$t('entry_authorizers.edit_all_subtitle')"
        >
        ...
        <Alert
            ...
            type="error"
            show-icon
            :message="$t('entry_authorizers.name_unique')"
            class="status-bar"
        />

        <Card :bodyStyle="{ padding: 0 }" class="edit-table-card">
            <EntryAuthorizersEditAllTable
                v-model:draft="draft"
                :is-dirty="isDirty"
                :duplicate-rows="duplicateRows"
            />

        <div v-if="entry_authorizers.total > entry_authorizers.per_page" class="edit-pagination">
            <Pagination
                :current="entry_authorizers.current_page"
                :pageSize="entry_authorizers.per_page"
                :total="entry_authorizers.total"
        ...
        <EditAllFooter
            :discard-label="$t('entry_authorizers.edit_all_discard')"
            :save-label="$t('entry_authorizers.edit_all_save_all')"
            :discard-disabled="dirtyCount === 0"
            :save-disabled="dirtyCount === 0 || duplicateRows.size > 0"
            :submitting="submitting"
            :status-text="dirtyCount > 0 ? $tc('entry_authorizers.edit_all_changes', dirtyCount) : ''"
            @discard="discardAll"
            @save="saveAll"
        />
