...
import EntryAuthorizersBulkBar from '@/Components/EntryAuthorizers/EntryAuthorizersBulkBar.vue';
import EntryAuthorizersBulkDeleteModal from '@/Components/EntryAuthorizers/EntryAuthorizersBulkDeleteModal.vue';
import EntryAuthorizersFavoriteCell from '@/Components/EntryAuthorizers/EntryAuthorizersFavoriteCell.vue';
import EntryAuthorizersPageHeader from '@/Components/EntryAuthorizers/EntryAuthorizersPageHeader.vue';
import EntryAuthorizersActionsCell from '@/Components/EntryAuthorizers/EntryAuthorizersActionsCell.vue';
import EntryAuthorizersEmptyState from '@/Components/EntryAuthorizers/EntryAuthorizersEmptyState.vue';
...
// El toolbar inline de EntryAuthorizers (no usa ModuleToolbar) repite manualmente
// estos gates para no mostrar botones que no funcionan al user free/basic.
const { canUse: canUsePlanFeature } = usePlanFeatures();

import {
    entryAuthorizersFilterFields, entryAuthorizersEmptyFilters, hydrateEntryAuthorizersFilters,
    entryAuthorizersFiltersToQuery, entryAuthorizersFiltersSummary,
    serializeSavedFilters, deserializeSavedFilters,
} from './config/filters';
import { entryAuthorizersTableColumns } from './config/columns';
import { entryAuthorizersExportableColumns, entryAuthorizersExportEndpoints } from './config/exports';
...
const filterFields = computed(() =>
    entryAuthorizersFilterFields(t),
);
...
const {
    ...
    hydrate:       hydrateEntryAuthorizersFilters,
    toQuery:       entryAuthorizersFiltersToQuery,
    summary:       entryAuthorizersFiltersSummary,
    empty:         entryAuthorizersEmptyFilters,
...
const { counterLabel } = useModuleListMeta({
    pagination: computed(() => props.entry_authorizers),
    hasActiveFilters,
    t,
});
...
const allColumns = computed(() =>
    entryAuthorizersTableColumns(t, { isSuper: isSuper.value, isMobile: isMobileScreen.value }),
);
...
const applyInlineFilters = (cleaned) => {
    ...
    inlineTimer = setTimeout(() => {
        ...
        router.get(route('business_management.entry_authorizers.index'), data, {
            preserveScroll: true,
            preserveState: true,
...
const VIEW_KEY = 'entry_authorizers_view_mode';
...
useModuleUndoToast('business_management.entry_authorizers.undo_last_delete');

// ─── Favoritos polimorficos ────────────────────────────────────────────────
const { submitting: favoriteSubmitting, toggle: toggleFavorite } = useModuleFavorites('entry_authorizers', 'entry_authorizers');
...
const {
    ...
    bulkSetActiveRoute: 'business_management.entry_authorizers.bulk_set_active',
    bulkDeleteRoute:    'business_management.entry_authorizers.bulk_delete',
    resourceLabel:      t('entry_authorizers.records'),
...
const duplicate = (record) => {
    ...
    router.post(route('business_management.entry_authorizers.duplicate', record.slug), {}, {
        preserveScroll: true,
...
const exportableColumns = computed(() => entryAuthorizersExportableColumns(t, { isSuper: isSuper.value }));
const exportEndpoints   = computed(() => entryAuthorizersExportEndpoints());
...
const tour = useModuleTour({ module: 'entry_authorizers', steps: () => moduleTourSteps(t, { moduleName: t('entry_authorizers.plural') }) });
...
// (`entry_authorizers.*`), así que el diálogo de edición tiene todos los campos.
...
useKeyboardShortcuts({
    'ctrl+n': () => can('entry_authorizers.create') && openCreate(),
    'esc': () => {
...
const goDelete = (record) => router.visit(route('business_management.entry_authorizers.delete', record.slug));
</script>

<template>
    <Head :title="$t('entry_authorizers.plural')" />

    <div class="sap-index">
        <!-- Título + acciones (las acciones viven en el toolbar de la tabla). -->
        <div class="mi-title" data-tour="module">
            <EntryAuthorizersPageHeader
                :title="$t('entry_authorizers.plural')"
            />
        </div>
        ...
        <Drawer v-model:open="filtersOpen" :title="$t('global.filters')" placement="right" :width="380">
            <FilterBar :fields="filterFields" v-model="filters" storage-key="entry_authorizers" />
        </Drawer>
        ...
        <div class="mi-tabletoolbar">
            ...
            <div class="mi-tabletoolbar__right">
                <label class="mi-bar mi-bar--toolbar" :class="{ 'is-active': quickSearch }">
                    ...
                    <input v-model="quickSearch" class="mi-bar__input" :placeholder="$t('global.search_in', { item: $t('entry_authorizers.singular').toLowerCase() })" autocomplete="off" spellcheck="false" type="text" />
                ...
                <Tooltip v-if="can('entry_authorizers.export')" :title="$t('global.export_hint')" data-tour="export-import">
                ...
                <Dropdown :trigger="['click']" placement="bottomRight">
                    ...
                    <template #overlay>
                        <Menu>
                            <MenuItem v-if="can('entry_authorizers.create') && can('entry_authorizers.import') && canUsePlanFeature('imports')" key="import" @click="importOpen = true"><UploadOutlined /> {{ $t('global.import') }}</MenuItem>
                            <MenuItem v-if="can('entry_authorizers.edit') && canUsePlanFeature('edit_all')" key="editall" @click="router.visit(route('business_management.entry_authorizers.edit_all'))"><FormOutlined /> {{ $t('global.edit_all') }}</MenuItem>
                            <MenuItem v-if="isSuper" key="trash" @click="router.visit(route('business_management.entry_authorizers.trash'))"><InboxOutlined /> {{ $t('global.view_deleted') }}</MenuItem>
                ...
                <Tooltip v-if="can('entry_authorizers.create')" :title="$t('entry_authorizers.new')" data-tour="new">
                    <Button type="primary" class="mi-iconbtn mi-create-btn" :aria-label="$t('entry_authorizers.new')" @click="openCreate"><PlusOutlined /></Button>
                </Tooltip>
            </div>
        </div>
        ...
        <Card :bodyStyle="{ padding: 0 }" class="grid-card">
            ...
            <ResponsiveTable
                ...
                :row-selection="(can('entry_authorizers.delete') || can('entry_authorizers.edit')) ? rowSelection : null"
            ...
            >
                <template #empty>
                    <EntryAuthorizersEmptyState
                        :has-filters="hasActiveFilters"
                        :can-create="can('entry_authorizers.create')"
                ...
                <template #bodyCell="{ column, record, text, isMobile, compact }">
                    <EntryAuthorizersFavoriteCell
                        v-if="column.key === 'favorite'"
                        :record="record"
                        :submitting="favoriteSubmitting"
                        :data-tour="record === entry_authorizers.data[0] ? 'favorites' : null"
                    ...
                    <template v-else-if="column.key === 'name'">
                        <div class="lead">
                            <div class="lead__txt">
                                <Link :href="route('business_management.entry_authorizers.show', record.slug)" class="lead__name lead__link">{{ record.name }}</Link>
        ...
        <EntryAuthorizersBulkDeleteModal
            v-model:open="bulkOpen"
            v-model:reason="bulkReason"
            :count="selectedRowKeys.length"
            :submitting="bulkSubmitting"
            :error-msg="bulkError"
            :resource-label="selectedRowKeys.length === 1 ? $t('entry_authorizers.record') : $t('entry_authorizers.records')"
        ...
        <ExportDialog
            ...
            :default-title="$t('entry_authorizers.export_title')"
            :endpoints="exportEndpoints"
            :limits="exportLimits"
            :total-rows="entry_authorizers.total ?? 0"
            :total-unfiltered="entry_authorizers.total_unfiltered ?? entry_authorizers.total ?? 0"
        />

        <ImportDialog
            v-model:open="importOpen"
            :endpoint="route('business_management.entry_authorizers.import')"
            :template-url="route('business_management.entry_authorizers.import_template')"
            :resource-label="$t('entry_authorizers.records')"
        />
