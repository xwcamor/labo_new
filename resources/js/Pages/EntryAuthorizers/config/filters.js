...
/**
 * Schema de filtros del módulo EntryAuthorizers (catálogo global de marcas).
 * Campos de dominio: name + code + is_active. Mismo patrón que Regions/Tenants.
 */
export const entryAuthorizersFilterFields = (t) => [
    { key: 'name',           label: t('entry_authorizers.filter_name'), type: 'tags' },
    { key: 'code',           label: t('entry_authorizers.code'),        type: 'text' },
    { key: 'is_active',      label: t('entry_authorizers.is_active'),   type: 'select', options: [
...
export const entryAuthorizersEmptyFilters = () => ({
    name: [],
    code: '',
    is_active: null,
    created_at: null,
...
export const hydrateEntryAuthorizersFilters = (server) => ({
...
export const entryAuthorizersFiltersToQuery = (f) => ({
...
export const entryAuthorizersFiltersSummary = (f, t) => {
    const parts = [];
    if (f.name?.length)        parts.push(`${t('entry_authorizers.filter_name')}: ${f.name.join(', ')}`);
    if (f.code)                parts.push(`${t('entry_authorizers.code')}: ${f.code}`);
    if (f.is_active !== null && f.is_active !== undefined) {
        parts.push(`${t('entry_authorizers.is_active')}: ${f.is_active ? t('global.active') : t('global.inactive')}`);
    }
