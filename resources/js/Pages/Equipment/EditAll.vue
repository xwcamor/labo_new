...
import { SaveOutlined, UndoOutlined, EditOutlined, ApartmentOutlined } from '@ant-design/icons-vue';

import AppLayout from '@/Layouts/AppLayout.vue';
import SectionHeader from '@/Components/Common/SectionHeader.vue';
import EditAllFooter from '@/Components/Common/EditAllFooter.vue';
import EquipmentEditAllTable from '@/Components/Equipment/EquipmentEditAllTable.vue';
...
/**
 * Edit-All de Equipment. Mantenemos snapshot `original` + `draft` mutable
...
const source = computed(() => props.equipment.data);
...
const saveAll = () => {
    ...
    router.post(
        route('business_management.equipment.edit_all.update'),
...
<template>
    <Head :title="$t('equipment.edit_all_title')" />

    <div class="edit-all sap-form">
        <SectionHeader
            :back-href="route('business_management.equipment.index')"
            :title="$t('global.edit_all') + ' — ' + $t('equipment.plural')"
            :subtitle="$t('equipment.edit_all_subtitle')"
        >
        ...
        <Alert
            ...
            type="error"
            show-icon
            :message="$t('equipment.name_unique')"
            class="status-bar"
        />

        <Card :bodyStyle="{ padding: 0 }" class="edit-table-card">
            <EquipmentEditAllTable
                v-model:draft="draft"
                :is-dirty="isDirty"
                :duplicate-rows="duplicateRows"
            />

        <div v-if="equipment.total > equipment.per_page" class="edit-pagination">
            <Pagination
        ...
        <EditAllFooter
            :discard-label="$t('equipment.edit_all_discard')"
            :save-label="$t('equipment.edit_all_save_all')"
            :discard-disabled="dirtyCount === 0"
            :save-disabled="dirtyCount === 0 || duplicateRows.size > 0"
            :submitting="submitting"
            :status-text="dirtyCount > 0 ? $tc('equipment.edit_all_changes', dirtyCount) : ''"
            @discard="discardAll"
            @save="saveAll"
        />
