...
import EquipmentBulkBar from '@/Components/Equipment/EquipmentBulkBar.vue';
import EquipmentBulkDeleteModal from '@/Components/Equipment/EquipmentBulkDeleteModal.vue';
import EquipmentFavoriteCell from '@/Components/Equipment/EquipmentFavoriteCell.vue';
import EquipmentPageHeader from '@/Components/Equipment/EquipmentPageHeader.vue';
import EquipmentActionsCell from '@/Components/Equipment/EquipmentActionsCell.vue';
import EquipmentEmptyState from '@/Components/Equipment/EquipmentEmptyState.vue';
...
// El toolbar inline de Equipment (no usa ModuleToolbar) repite manualmente
// estos gates para no mostrar botones que no funcionan al user free/basic.
const { canUse: canUsePlanFeature } = usePlanFeatures();

import {
    equipmentFilterFields, equipmentEmptyFilters, hydrateEquipmentFilters,
    equipmentFiltersToQuery, equipmentFiltersSummary,
    serializeSavedFilters, deserializeSavedFilters,
} from './config/filters';
import { equipmentTableColumns } from './config/columns';
import { equipmentExportableColumns, equipmentExportEndpoints } from './config/exports';
...
const {
    ...
    hydrate:       hydrateEquipmentFilters,
    toQuery:       equipmentFiltersToQuery,
    summary:       equipmentFiltersSummary,
    empty:         equipmentEmptyFilters,
...
const { counterLabel } = useModuleListMeta({
    pagination: computed(() => props.equipment),
    hasActiveFilters,
    t,
});
...
const allColumns = computed(() =>
    equipmentTableColumns(t, { isSuper: isSuper.value, isMobile: isMobileScreen.value }),
);
const { visibleColumnKeys, columns } = useColumnPreferences(allColumns);

// ─── Paginacion + sort ──────────────────────────────────────────────────────
const tablePagination = computed(() => ({
    current:  props.equipment.current_page,
    pageSize: props.equipment.per_page,
    total:    props.equipment.total,
...
const applyInlineFilters = (cleaned) => {
    ...
    inlineTimer = setTimeout(() => {
        ...
        router.get(route('business_management.equipment.index'), data, {
            preserveScroll: true,
            preserveState: true,
...
const VIEW_KEY = 'equipment_view_mode';
...
useModuleUndoToast('business_management.equipment.undo_last_delete');

// ─── Favoritos polimorficos ────────────────────────────────────────────────
const { submitting: favoriteSubmitting, toggle: toggleFavorite } = useModuleFavorites('equipment', 'equipment');
...
const {
    ...
    bulkSetActiveRoute: 'business_management.equipment.bulk_set_active',
    bulkDeleteRoute:    'business_management.equipment.bulk_delete',
    resourceLabel:      t('equipment.records'),
...
const duplicate = (record) => {
    ...
    router.post(route('business_management.equipment.duplicate', record.slug), {}, {
        preserveScroll: true,
...
const exportableColumns = computed(() => equipmentExportableColumns(t, { isSuper: isSuper.value }));
const exportEndpoints   = computed(() => equipmentExportEndpoints());
...
const tour = useModuleTour({ module: 'equipment', steps: () => moduleTourSteps(t, { moduleName: t('equipment.plural') }) });

// ─── Keyboard shortcuts ────────────────────────────────────────────────────
useKeyboardShortcuts({
    'ctrl+n': () => can('equipment.create') && router.visit(route('business_management.equipment.create')),
    'esc': () => {
...
// ─── Acciones ───────────────────────────────────────────────────────────────
const goEdit   = (record) => router.visit(route('business_management.equipment.edit',   record.slug));
const goDelete = (record) => router.visit(route('business_management.equipment.delete', record.slug));
</script>

<template>
    <Head :title="$t('equipment.plural')" />

    <div class="sap-index">
        <!-- Título + acciones (las acciones viven en el toolbar de la tabla). -->
        <div class="mi-title" data-tour="module">
            <EquipmentPageHeader
                :title="$t('equipment.plural')"
            />
        </div>
        ...
        <Drawer v-model:open="filtersOpen" :title="$t('global.filters')" placement="right" :width="380">
            <FilterBar :fields="filterFields" v-model="filters" storage-key="equipment" />
        </Drawer>
        ...
        <div class="mi-tabletoolbar">
            ...
            <div class="mi-tabletoolbar__right">
                <label class="mi-bar mi-bar--toolbar" :class="{ 'is-active': quickSearch }">
                    ...
                    <input v-model="quickSearch" class="mi-bar__input" :placeholder="$t('global.search_in', { item: $t('equipment.singular').toLowerCase() })" autocomplete="off" spellcheck="false" type="text" />
                ...
                <Tooltip v-if="can('equipment.export')" :title="$t('global.export_hint')" data-tour="export-import">
                ...
                <Dropdown :trigger="['click']" placement="bottomRight">
                    ...
                    <template #overlay>
                        <Menu>
                            <MenuItem v-if="can('equipment.create') && can('equipment.import') && canUsePlanFeature('imports')" key="import" @click="importOpen = true"><UploadOutlined /> {{ $t('global.import') }}</MenuItem>
                            <MenuItem v-if="can('equipment.edit') && canUsePlanFeature('edit_all')" key="editall" @click="router.visit(route('business_management.equipment.edit_all'))"><FormOutlined /> {{ $t('global.edit_all') }}</MenuItem>
                            <MenuItem v-if="isSuper" key="trash" @click="router.visit(route('business_management.equipment.trash'))"><InboxOutlined /> {{ $t('global.view_deleted') }}</MenuItem>
                ...
                <Tooltip v-if="can('equipment.create')" :title="$t('equipment.new')" data-tour="new">
                    <Link :href="route('business_management.equipment.create')">
                        <Button type="primary" class="mi-iconbtn mi-create-btn" :aria-label="$t('equipment.new')"><PlusOutlined /></Button>
                    </Link>
                </Tooltip>
            </div>
        </div>
        ...
        <Card :bodyStyle="{ padding: 0 }" class="grid-card">
            ...
            <ResponsiveTable
                ...
                :row-selection="(can('equipment.delete') || can('equipment.edit')) ? rowSelection : null"
        ...
        <EquipmentBulkDeleteModal
            ...
            :error-msg="bulkError"
            :resource-label="selectedRowKeys.length === 1 ? $t('equipment.record') : $t('equipment.records')"
        ...
        <ExportDialog
            ...
            :default-title="$t('equipment.export_title')"
            :endpoints="exportEndpoints"
            :limits="exportLimits"
            :total-rows="equipment.total ?? 0"
            :total-unfiltered="equipment.total_unfiltered ?? equipment.total ?? 0"
        />

        <ImportDialog
            v-model:open="importOpen"
            :endpoint="route('business_management.equipment.import')"
            :template-url="route('business_management.equipment.import_template')"
            :resource-label="$t('equipment.records')"
        />
    </div>
</template>
