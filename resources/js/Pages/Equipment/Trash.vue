...
import EquipmentTrashBulkBar from '@/Components/Equipment/EquipmentTrashBulkBar.vue';
import EquipmentForceDeleteModal from '@/Components/Equipment/EquipmentForceDeleteModal.vue';

import { useModuleRestore } from '@/Composables/useModuleRestore';
import { useDateFormat } from '@/Composables/useDateFormat';
import { useI18n } from '@/Plugins/i18n';
import { equipmentTrashColumns } from './config/trashColumns';
...
if (!isSuper) {
    router.visit(route('business_management.equipment.index'));
}
...
const {
    ...
    restoreRouteName:     'business_management.equipment.restore',
    bulkRestoreRouteName: 'business_management.equipment.bulk_restore',
});
...
const openForceDelete = (equipment) => {
    forceDeleteTarget.value = equipment;
...
const submitForceDelete = () => {
    ...
    forceDeleteErrors.value = {};
    router.delete(
        route('business_management.equipment.force_delete', forceDeleteTarget.value.slug),
        {
...
const columns = computed(() => equipmentTrashColumns(t));
...
const subtitle = computed(() => {
    const word = props.equipment.total === 1 ? t('global.record') : t('global.records');
    return `${props.equipment.total} ${word} · ${t('global.super_only')}`;
});
</script>

<template>
    <Head :title="$t('global.view_deleted') + ' — ' + $t('equipment.plural')" />

    <div v-if="isSuper" class="sap-form trash-page">
        <SectionHeader
            :back-href="route('business_management.equipment.index')"
            :title="$t('global.view_deleted') + ' — ' + $t('equipment.plural')"
            :subtitle="subtitle"
                    ...
                    <template v-else-if="column.key === 'actions'">
                        <Space :size="4">
                            <Popconfirm
                                ...
                                :description="$t('equipment.restore_hint')"
