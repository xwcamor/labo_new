...
/**
 * Schema de filtros del módulo Equipment (catálogo global de marcas).
 * Campos de dominio: name + code + is_active. Mismo patrón que Regions/Tenants.
 */
export const equipmentFilterFields = (t) => [
    { key: 'name',           label: t('equipment.filter_name'), type: 'tags' },
    { key: 'code',           label: t('equipment.code'),        type: 'text' },
    { key: 'is_active',      label: t('equipment.is_active'),   type: 'select', options: [
...
export const equipmentEmptyFilters = () => ({
    name: [],
    code: '',
    is_active: null,
    created_at: null,
...
export const hydrateEquipmentFilters = (server) => ({
...
export const equipmentFiltersToQuery = (f) => ({
...
export const equipmentFiltersSummary = (f, t) => {
    const parts = [];
    if (f.name?.length)        parts.push(`${t('equipment.filter_name')}: ${f.name.join(', ')}`);
    if (f.code)                parts.push(`${t('equipment.code')}: ${f.code}`);
    if (f.is_active !== null && f.is_active !== undefined) {
        parts.push(`${t('equipment.is_active')}: ${f.is_active ? t('global.active') : t('global.inactive')}`);
    }
