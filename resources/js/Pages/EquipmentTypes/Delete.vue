...
const submit = () => {
    form.delete(route('business_management.equipment_types.deleteSave', props.equipmentType.slug), {
        preserveScroll: true,
    });
};
</script>
...
<template>
    <Head :title="$t('global.delete') + ' — ' + $t('equipment_types.singular')" />

    <DeletePage
        :back-href="route('business_management.equipment_types.index')"
        :title="$t('global.delete') + ' ' + $t('equipment_types.record')"
        :subtitle="equipmentType.name"
    ...
    >
        ...
        <template #summary>
            <DeleteSummaryRow :label="$t('equipment_types.name')">{{ equipmentType.name }}</DeleteSummaryRow>
            <DeleteSummaryRow v-if="equipmentType.code" :label="$t('equipment_types.code')">
                <code>{{ equipmentType.code }}</code>
            </DeleteSummaryRow>
            <DeleteSummaryRow :label="$t('equipment_types.is_active')">
                <Tag :color="equipmentType.is_active ? 'success' : 'error'" :bordered="false">
