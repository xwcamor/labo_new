...
import EquipmentTypesBulkBar from '@/Components/EquipmentTypes/EquipmentTypesBulkBar.vue';
import EquipmentTypesBulkDeleteModal from '@/Components/EquipmentTypes/EquipmentTypesBulkDeleteModal.vue';
import EquipmentTypesFavoriteCell from '@/Components/EquipmentTypes/EquipmentTypesFavoriteCell.vue';
...
// El toolbar inline de EquipmentTypes (no usa ModuleToolbar) repite manualmente
// estos gates para no mostrar botones que no funcionan al user free/basic.
const { canUse: canUsePlanFeature } = usePlanFeatures();

import {
    equipmentTypesFilterFields, equipmentTypesEmptyFilters, hydrateEquipmentTypesFilters,
    equipmentTypesFiltersToQuery, equipmentTypesFiltersSummary,
    serializeSavedFilters, deserializeSavedFilters,
} from './config/filters';
import { equipmentTypesTableColumns } from './config/columns';
import { equipmentTypesExportableColumns, equipmentTypesExportEndpoints } from './config/exports';
...
const {
    ...
    hydrate:       hydrateEquipmentTypesFilters,
    toQuery:       equipmentTypesFiltersToQuery,
    summary:       equipmentTypesFiltersSummary,
    empty:         equipmentTypesEmptyFilters,
    only:          ['equipment_types', 'filters'],
    t,
});
...
const { counterLabel } = useModuleListMeta({
    pagination: computed(() => props.equipmentTypes),
    hasActiveFilters,
    t,
});
...
const allColumns = computed(() =>
    equipmentTypesTableColumns(t, { isSuper: isSuper.value, isMobile: isMobileScreen.value }),
);
const { visibleColumnKeys, columns } = useColumnPreferences(allColumns);

// ─── Paginacion + sort ──────────────────────────────────────────────────────
const tablePagination = computed(() => ({
    current:  props.equipmentTypes.current_page,
    pageSize: props.equipmentTypes.per_page,
    total:    props.equipmentTypes.total,
...
const applyInlineFilters = (cleaned) => {
    ...
    inlineTimer = setTimeout(() => {
        ...
        router.get(route('business_management.equipment_types.index'), data, {
            preserveScroll: true,
            preserveState: true,
...
const VIEW_KEY = 'equipment_types_view_mode';
...
useModuleUndoToast('business_management.equipment_types.undo_last_delete');

// ─── Favoritos polimorficos ────────────────────────────────────────────────
const { submitting: favoriteSubmitting, toggle: toggleFavorite } = useModuleFavorites('equipment_types', 'equipment_types');
...
const {
    ...
    bulkSetActiveRoute: 'business_management.equipment_types.bulk_set_active',
    bulkDeleteRoute:    'business_management.equipment_types.bulk_delete',
    resourceLabel:      t('equipment_types.records'),
});
...
const duplicate = (record) => {
    ...
    router.post(route('business_management.equipment_types.duplicate', record.slug), {}, {
        preserveScroll: true,
...
const exportableColumns = computed(() => equipmentTypesExportableColumns(t));
const exportEndpoints   = computed(() => equipmentTypesExportEndpoints());
...
<template>
    ...
    <div class="sap-index">
        ...
        <Drawer v-model:open="filtersOpen" :title="$t('global.filters')" placement="right" :width="380">
            <FilterBar :fields="filterFields" v-model="filters" storage-key="equipment_types" />
        </Drawer>
        ...
        <div class="mi-tabletoolbar">
            ...
            <div class="mi-tabletoolbar__right">
                <label class="mi-bar mi-bar--toolbar" :class="{ 'is-active': quickSearch }">
                    ...
                    <input v-model="quickSearch" class="mi-bar__input" :placeholder="$t('global.search_in', { item: $t('equipment_types.singular').toLowerCase() })" autocomplete="off" spellcheck="false" type="text" />
                ...
                <Dropdown :trigger="['click']" placement="bottomRight">
                    ...
                    <template #overlay>
                        <Menu>
                            <MenuItem v-if="can('equipment_types.create') && canUsePlanFeature('imports')" key="import" @click="importOpen = true">
                                <UploadOutlined /> {{ $t('global.import') }}
                            </MenuItem>
                            <MenuItem v-if="can('equipment_types.edit') && canUsePlanFeature('edit_all')" key="editall" @click="router.visit(route('business_management.equipment_types.edit_all'))">
                                <EditOutlined /> {{ $t('global.edit_all') }}
                            </MenuItem>
                            <MenuItem v-if="isSuper" key="trash" @click="router.visit(route('business_management.equipment_types.trash'))">
        ...
        <Card :bodyStyle="{ padding: 0 }" class="grid-card">
            ...
            <ResponsiveTable
                :loading="isFetching"
                :dataSource="equipmentTypes.data"
                :columns="columns"
                :pagination="tablePagination"
                :row-selection="(can('equipment_types.delete') || can('equipment_types.edit')) ? rowSelection : null"
            ...
            >
                ...
                <template #bodyCell="{ column, record, text, isMobile, compact }">
                    <EquipmentTypesFavoriteCell
                        v-if="column.key === 'favorite'"
                        :record="record"
                        :submitting="favoriteSubmitting"
                        :data-tour="record === equipmentTypes.data[0] ? 'favorites' : null"
                    ...
                    <template v-else-if="column.key === 'name'">
                        <div class="lead">
                            <div class="lead__txt">
                                <Link :href="route('business_management.equipment_types.show', record.slug)" class="lead__name lead__link">{{ record.name }}</Link>
                    ...
                    <template v-else-if="column.key === 'shape'">
                        <span class="shape-cell">
                            <EquipmentGlyph :shape="record.shape || 'tank'" :animated="false" />
                            {{ $t('equipment_types.shape_' + (record.shape || 'tank')) }}
                        </span>
                    </template>
                    ...
                    <EquipmentTypesActionsCell
                        v-else-if="column.key === 'actions'"
                        :record="record"
                        :is-mobile="isMobile"
                        :compact="compact"
                        :can-edit="can('equipment_types.edit')"
                        :can-create="can('equipment_types.create')"
                        :can-delete="can('equipment_types.delete')"
        ...
        <EquipmentTypesBulkDeleteModal
            v-model:open="bulkOpen"
            v-model:reason="bulkReason"
            :count="selectedRowKeys.length"
            :submitting="bulkSubmitting"
            :error-msg="bulkError"
            :resource-label="selectedRowKeys.length === 1 ? $t('equipment_types.record') : $t('equipment_types.records')"
        ...
        <ExportDialog
            ...
            :default-title="$t('equipment_types.export_title')"
            :endpoints="exportEndpoints"
            :limits="exportLimits"
            :total-rows="equipmentTypes.total ?? 0"
            :total-unfiltered="equipmentTypes.total_unfiltered ?? equipmentTypes.total ?? 0"
        />
