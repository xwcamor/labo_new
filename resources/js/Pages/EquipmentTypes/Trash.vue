...
import EquipmentTypesTrashBulkBar from '@/Components/EquipmentTypes/EquipmentTypesTrashBulkBar.vue';
import EquipmentTypesForceDeleteModal from '@/Components/EquipmentTypes/EquipmentTypesForceDeleteModal.vue';

import { useModuleRestore } from '@/Composables/useModuleRestore';
import { useDateFormat } from '@/Composables/useDateFormat';
import { useI18n } from '@/Plugins/i18n';
import { equipmentTypesTrashColumns } from './config/trashColumns';
...
if (!isSuper) {
    router.visit(route('business_management.equipment_types.index'));
}
...
const {
    ...
    restoreRouteName:     'business_management.equipment_types.restore',
    bulkRestoreRouteName: 'business_management.equipment_types.bulk_restore',
});
...
const openForceDelete = (equipmentType) => {
    forceDeleteTarget.value = equipmentType;
...
const submitForceDelete = () => {
    ...
    forceDeleteErrors.value = {};
    router.delete(
        route('business_management.equipment_types.force_delete', forceDeleteTarget.value.slug),
        {
...
const columns = computed(() => equipmentTypesTrashColumns(t));
...
const subtitle = computed(() => {
    const word = props.equipmentTypes.total === 1 ? t('global.record') : t('global.records');
    return `${props.equipmentTypes.total} ${word} · ${t('global.super_only')}`;
});
</script>

<template>
    <Head :title="$t('global.view_deleted') + ' — ' + $t('equipment_types.plural')" />

    <div v-if="isSuper" class="sap-form trash-page">
        <SectionHeader
            :back-href="route('business_management.equipment_types.index')"
            :title="$t('global.view_deleted') + ' — ' + $t('equipment_types.plural')"
            :subtitle="subtitle"
        ...
        <Card :bodyStyle="{ padding: 0 }" class="grid-card">
                    ...
                    <template v-else-if="column.key === 'actions'">
                        <Space :size="4">
                            <Popconfirm
                                ...
                                :description="$t('equipment_types.restore_hint')"
            ...
            <Empty
                v-if="equipmentTypes.data.length === 0"
                :description="$t('global.no_deleted_records')"
                style="padding: 48px 16px"
            />
        </Card>

        <EquipmentTypesTrashBulkBar
        ...
        <EquipmentTypesForceDeleteModal
