...
/**
 * Schema de filtros del módulo EquipmentTypes (catálogo global de tipos de transformador).
 * Campos de dominio: name + code + is_active. Mismo patrón que Regions/Tenants.
 */
export const equipmentTypesFilterFields = (t) => [
    { key: 'name',           label: t('equipment_types.filter_name'), type: 'tags' },
    { key: 'code',           label: t('equipment_types.code'),        type: 'text' },
    { key: 'is_active',      label: t('equipment_types.is_active'),   type: 'select', options: [
...
export const equipmentTypesEmptyFilters = () => ({
    name: [],
    code: '',
    is_active: null,
    created_at: null,
...
export const hydrateEquipmentTypesFilters = (server) => ({
...
export const equipmentTypesFiltersToQuery = (f) => ({
...
export const equipmentTypesFiltersSummary = (f, t) => {
    const parts = [];
    if (f.name?.length)        parts.push(`${t('equipment_types.filter_name')}: ${f.name.join(', ')}`);
    if (f.code)                parts.push(`${t('equipment_types.code')}: ${f.code}`);
    if (f.is_active !== null && f.is_active !== undefined) {
        parts.push(`${t('equipment_types.is_active')}: ${f.is_active ? t('global.active') : t('global.inactive')}`);
    }
