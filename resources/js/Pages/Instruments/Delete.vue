...
const submit = () => {
    form.delete(route('business_management.instruments.deleteSave', props.instrument.slug), {
        preserveScroll: true,
    });
};
</script>
...
<template>
    <Head :title="$t('global.delete') + ' — ' + $t('instruments.singular')" />

    <DeletePage
        :back-href="route('business_management.instruments.index')"
        :title="$t('global.delete') + ' ' + $t('instruments.record')"
    ...
    >
        ...
        <template #summary>
            <DeleteSummaryRow :label="$t('instruments.name')">{{ instrument.name }}</DeleteSummaryRow>
            <DeleteSummaryRow v-if="instrument.description" :label="$t('instruments.description')">
                <span>{{ instrument.description }}</span>
            </DeleteSummaryRow>
            <DeleteSummaryRow :label="$t('instruments.is_active')">
                <Tag :color="instrument.is_active ? 'success' : 'error'" :bordered="false">
