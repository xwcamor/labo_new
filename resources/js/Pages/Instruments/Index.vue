...
import InstrumentsBulkBar from '@/Components/Instruments/InstrumentsBulkBar.vue';
import InstrumentsBulkDeleteModal from '@/Components/Instruments/InstrumentsBulkDeleteModal.vue';
import InstrumentsFavoriteCell from '@/Components/Instruments/InstrumentsFavoriteCell.vue';
import InstrumentsPageHeader from '@/Components/Instruments/InstrumentsPageHeader.vue';
import InstrumentsActionsCell from '@/Components/Instruments/InstrumentsActionsCell.vue';
import InstrumentsEmptyState from '@/Components/Instruments/InstrumentsEmptyState.vue';
...
// El toolbar inline de Instruments (no usa ModuleToolbar) repite manualmente
// estos gates para no mostrar botones que no funcionan al user free/basic.
const { canUse: canUsePlanFeature } = usePlanFeatures();

import {
    instrumentsFilterFields, instrumentsEmptyFilters, hydrateInstrumentsFilters,
    instrumentsFiltersToQuery, instrumentsFiltersSummary,
    serializeSavedFilters, deserializeSavedFilters,
} from './config/filters';
import { instrumentsTableColumns, calibrationTagColor } from './config/columns';
import { instrumentsExportableColumns, instrumentsExportEndpoints } from './config/exports';
...
/**
 * Frase corta de días para la celda de calibración. El semáforo dice el estado
 * y esto dice cuánto falta: sin la cuenta, "vence el 12/09" obliga a mirar el
 * calendario para saber si el equipo sirve hoy.
 */
const calDetail = (record) => {
    const d = record.calibration_days_left;
...
const {
    ...
    hydrate:       hydrateInstrumentsFilters,
    toQuery:       instrumentsFiltersToQuery,
    summary:       instrumentsFiltersSummary,
    empty:         instrumentsEmptyFilters,
    only:          ['instruments', 'filters'],
    t,
});
...
const { counterLabel } = useModuleListMeta({
    pagination: computed(() => props.instruments),
    hasActiveFilters,
    t,
});
...
const allColumns = computed(() =>
    instrumentsTableColumns(t, { isSuper: isSuper.value, isMobile: isMobileScreen.value }),
);
const { visibleColumnKeys, columns } = useColumnPreferences(allColumns);

// ─── Paginacion + sort ──────────────────────────────────────────────────────
const tablePagination = computed(() => ({
    current:  props.instruments.current_page,
    pageSize: props.instruments.per_page,
    total:    props.instruments.total,
...
const applyInlineFilters = (cleaned) => {
    ...
    inlineTimer = setTimeout(() => {
        ...
        router.get(route('business_management.instruments.index'), data, {
            preserveScroll: true,
            preserveState: true,
...
const VIEW_KEY = 'instruments_view_mode';
...
useModuleUndoToast('business_management.instruments.undo_last_delete');

// ─── Favoritos polimorficos ────────────────────────────────────────────────
const { submitting: favoriteSubmitting, toggle: toggleFavorite } = useModuleFavorites('instruments', 'instruments');
...
const {
    ...
    bulkSetActiveRoute: 'business_management.instruments.bulk_set_active',
    bulkDeleteRoute:    'business_management.instruments.bulk_delete',
    resourceLabel:      t('instruments.records'),
...
const duplicate = (record) => {
    ...
    router.post(route('business_management.instruments.duplicate', record.slug), {}, {
        preserveScroll: true,
...
const exportableColumns = computed(() => instrumentsExportableColumns(t, { isSuper: isSuper.value }));
const exportEndpoints   = computed(() => instrumentsExportEndpoints());
...
const tour = useModuleTour({ module: 'instruments', steps: () => moduleTourSteps(t, { moduleName: t('instruments.plural') }) });

// ─── Keyboard shortcuts ────────────────────────────────────────────────────
useKeyboardShortcuts({
    'ctrl+n': () => can('instruments.create') && router.visit(route('business_management.instruments.create')),
    'esc': () => {
...
// ─── Acciones ───────────────────────────────────────────────────────────────
const goEdit   = (record) => router.visit(route('business_management.instruments.edit',   record.slug));
const goDelete = (record) => router.visit(route('business_management.instruments.delete', record.slug));
</script>

<template>
    <Head :title="$t('instruments.plural')" />

    <div class="sap-index">
        <!-- Título + acciones (las acciones viven en el toolbar de la tabla). -->
        <div class="mi-title" data-tour="module">
            <InstrumentsPageHeader
                :title="$t('instruments.plural')"
            />
        </div>
        ...
        <Drawer v-model:open="filtersOpen" :title="$t('global.filters')" placement="right" :width="380">
            <FilterBar :fields="filterFields" v-model="filters" storage-key="instruments" />
        </Drawer>
        ...
        <div class="mi-tabletoolbar">
            ...
            <div class="mi-tabletoolbar__right">
                <label class="mi-bar mi-bar--toolbar" :class="{ 'is-active': quickSearch }">
                    ...
                    <input v-model="quickSearch" class="mi-bar__input" :placeholder="$t('global.search_in', { item: $t('instruments.singular').toLowerCase() })" autocomplete="off" spellcheck="false" type="text" />
                ...
                <Tooltip v-if="can('instruments.export')" :title="$t('global.export_hint')" data-tour="export-import">
                ...
                <Dropdown :trigger="['click']" placement="bottomRight">
                    ...
                    <template #overlay>
                        <Menu>
                            <MenuItem v-if="can('instruments.create') && can('instruments.import') && canUsePlanFeature('imports')" key="import" @click="importOpen = true"><UploadOutlined /> {{ $t('global.import') }}</MenuItem>
                            <MenuItem v-if="can('instruments.edit') && canUsePlanFeature('edit_all')" key="editall" @click="router.visit(route('business_management.instruments.edit_all'))"><FormOutlined /> {{ $t('global.edit_all') }}</MenuItem>
                            <MenuItem v-if="isSuper" key="trash" @click="router.visit(route('business_management.instruments.trash'))"><InboxOutlined /> {{ $t('global.view_deleted') }}</MenuItem>
                ...
                <Tooltip v-if="can('instruments.create')" :title="$t('instruments.new')" data-tour="new">
                    <Link :href="route('business_management.instruments.create')">
                        <Button type="primary" class="mi-iconbtn mi-create-btn" :aria-label="$t('instruments.new')"><PlusOutlined /></Button>
                    </Link>
                </Tooltip>
            </div>
        </div>
        ...
        <Card :bodyStyle="{ padding: 0 }" class="grid-card">
            ...
            <ResponsiveTable
                ...
                :row-selection="(can('instruments.delete') || can('instruments.edit')) ? rowSelection : null"
                    ...
                    <template v-else-if="column.key === 'calibration'">
                        <Space :size="6" wrap>
                            <Tag :color="calibrationTagColor(record.calibration_status)" :bordered="false">
                                {{ $t('instruments.cal_status_' + (record.calibration_status || 'unknown')) }}
                            </Tag>
                            <span v-if="calDetail(record)" class="muted">{{ calDetail(record) }}</span>
                        </Space>
                    </template>

                    <template v-else-if="column.key === 'due_at'">
                        {{ record.calibration_due_at || '—' }}
                    </template>
        ...
        <InstrumentsBulkDeleteModal
            v-model:open="bulkOpen"
            v-model:reason="bulkReason"
            :count="selectedRowKeys.length"
            :submitting="bulkSubmitting"
            :error-msg="bulkError"
            :resource-label="selectedRowKeys.length === 1 ? $t('instruments.record') : $t('instruments.records')"
        ...
        <ExportDialog
            ...
            :default-title="$t('instruments.export_title')"
            :endpoints="exportEndpoints"
            :limits="exportLimits"
            :total-rows="instruments.total ?? 0"
            :total-unfiltered="instruments.total_unfiltered ?? instruments.total ?? 0"
        />

        <ImportDialog
            v-model:open="importOpen"
            :endpoint="route('business_management.instruments.import')"
            :template-url="route('business_management.instruments.import_template')"
            :resource-label="$t('instruments.records')"
        />
    </div>
</template>
