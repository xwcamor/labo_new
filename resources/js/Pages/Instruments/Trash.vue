...
import InstrumentsTrashBulkBar from '@/Components/Instruments/InstrumentsTrashBulkBar.vue';
import InstrumentsForceDeleteModal from '@/Components/Instruments/InstrumentsForceDeleteModal.vue';

import { useModuleRestore } from '@/Composables/useModuleRestore';
import { useDateFormat } from '@/Composables/useDateFormat';
import { useI18n } from '@/Plugins/i18n';
import { instrumentsTrashColumns } from './config/trashColumns';
...
if (!isSuper) {
    router.visit(route('business_management.instruments.index'));
}
...
const {
    ...
    restoreRouteName:     'business_management.instruments.restore',
    bulkRestoreRouteName: 'business_management.instruments.bulk_restore',
});
...
const openForceDelete = (instrument) => {
    forceDeleteTarget.value = instrument;
...
const submitForceDelete = () => {
    ...
    forceDeleteErrors.value = {};
    router.delete(
        route('business_management.instruments.force_delete', forceDeleteTarget.value.slug),
        {
...
const columns = computed(() => instrumentsTrashColumns(t));
...
const subtitle = computed(() => {
    const word = props.instruments.total === 1 ? t('global.record') : t('global.records');
    return `${props.instruments.total} ${word} · ${t('global.super_only')}`;
});
</script>

<template>
    <Head :title="$t('global.view_deleted') + ' — ' + $t('instruments.plural')" />

    <div v-if="isSuper" class="sap-form trash-page">
        <SectionHeader
            :back-href="route('business_management.instruments.index')"
            :title="$t('global.view_deleted') + ' — ' + $t('instruments.plural')"
            :subtitle="subtitle"
        ...
        <Card :bodyStyle="{ padding: 0 }" class="grid-card">
                    ...
                    <template v-else-if="column.key === 'actions'">
                        <Space :size="4">
                            <Popconfirm
                                ...
                                :description="$t('instruments.restore_hint')"
            ...
            <Empty
                v-if="instruments.data.length === 0"
                :description="$t('global.no_deleted_records')"
                style="padding: 48px 16px"
            />
        </Card>

        <InstrumentsTrashBulkBar
