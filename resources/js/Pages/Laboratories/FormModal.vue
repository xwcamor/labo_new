<script setup>
/**
 * Alta y edición del laboratorio, en diálogo (regla Fiori de la casa: menos
 ...
 * opcional: es la abreviatura con la que se identifica al laboratorio, no un
...
const title = computed(() => (isEdit.value
    ? `${t('global.edit')} ${t('laboratories.record')}`
    : t('laboratories.new')));
...
const submit = () => {
    ...
    if (isEdit.value) {
        form.put(route('business_management.laboratories.update', props.record.slug), opciones);
    } else {
        form.post(route('business_management.laboratories.store'), opciones);
    }
};
</script>
...
<template>
    <FormModal
        ...
        create-label-key="laboratories.new"
        @close="emit('close')"
        @submit="submit"
    >
        <FormItem
