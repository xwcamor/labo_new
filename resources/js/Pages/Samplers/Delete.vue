...
const submit = () => {
    form.delete(route('business_management.samplers.deleteSave', props.sampler.slug), {
        preserveScroll: true,
    });
};
</script>
...
<template>
    <Head :title="$t('global.delete') + ' — ' + $t('samplers.singular')" />

    <DeletePage
        :back-href="route('business_management.samplers.index')"
        :title="$t('global.delete') + ' ' + $t('samplers.record')"
        :subtitle="sampler.name"
    ...
    >
        ...
        <template #summary>
            <DeleteSummaryRow :label="$t('samplers.name')">{{ sampler.name }}</DeleteSummaryRow>
            <DeleteSummaryRow v-if="sampler.code" :label="$t('samplers.code')">
                <code>{{ sampler.code }}</code>
            </DeleteSummaryRow>
            <DeleteSummaryRow :label="$t('samplers.is_active')">
                <Tag :color="sampler.is_active ? 'success' : 'error'" :bordered="false">
