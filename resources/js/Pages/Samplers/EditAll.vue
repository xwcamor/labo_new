...
import { SaveOutlined, UndoOutlined, EditOutlined, UserSwitchOutlined } from '@ant-design/icons-vue';

import AppLayout from '@/Layouts/AppLayout.vue';
import SectionHeader from '@/Components/Common/SectionHeader.vue';
import EditAllFooter from '@/Components/Common/EditAllFooter.vue';
import SamplersEditAllTable from '@/Components/Samplers/SamplersEditAllTable.vue';
...
/**
 * Edit-All de Samplers. Mantenemos snapshot `original` + `draft` mutable
...
const source = computed(() => props.samplers.data);
...
const saveAll = () => {
    ...
    router.post(
        route('business_management.samplers.edit_all.update'),
...
<template>
    <Head :title="$t('samplers.edit_all_title')" />

    <div class="edit-all sap-form">
        <SectionHeader
            :back-href="route('business_management.samplers.index')"
            :title="$t('global.edit_all') + ' — ' + $t('samplers.plural')"
            :subtitle="$t('samplers.edit_all_subtitle')"
        >
        ...
        <Alert
            ...
            type="error"
            show-icon
            :message="$t('samplers.name_unique')"
            class="status-bar"
        />

        <Card :bodyStyle="{ padding: 0 }" class="edit-table-card">
            <SamplersEditAllTable
                v-model:draft="draft"
                :is-dirty="isDirty"
                :duplicate-rows="duplicateRows"
            />

        <div v-if="samplers.total > samplers.per_page" class="edit-pagination">
            <Pagination
                :current="samplers.current_page"
                :pageSize="samplers.per_page"
                :total="samplers.total"
        ...
        <EditAllFooter
            :discard-label="$t('samplers.edit_all_discard')"
            :save-label="$t('samplers.edit_all_save_all')"
            :discard-disabled="dirtyCount === 0"
            :save-disabled="dirtyCount === 0 || duplicateRows.size > 0"
            :submitting="submitting"
            :status-text="dirtyCount > 0 ? $tc('samplers.edit_all_changes', dirtyCount) : ''"
            @discard="discardAll"
            @save="saveAll"
        />
