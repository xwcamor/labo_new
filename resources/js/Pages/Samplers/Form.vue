...
const props = defineProps({
    sampler:       { type: Object, default: null },
});

const isEdit = computed(() => !!props.sampler);

const form = useForm({
    name:       props.sampler?.name ?? '',
    code:       props.sampler?.code ?? '',
    is_active:  props.sampler?.is_active ?? true,
});

const submit = () => {
    if (isEdit.value) {
        form.put(route('business_management.samplers.update', props.sampler.slug));
    } else {
        form.post(route('business_management.samplers.store'));
    }
};
</script>

<template>
    <Head :title="isEdit ? $t('global.edit') + ' — ' + $t('samplers.singular') : $t('samplers.new')" />
    ...
    <div class="form-page sap-form">
        ...
        <div class="form-body">
            ...
            >
                ...
                <FormFooter
                    :cancel-href="route('business_management.samplers.index')"
                    :is-edit="isEdit"
