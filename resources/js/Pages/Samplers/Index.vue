...
import SamplersBulkBar from '@/Components/Samplers/SamplersBulkBar.vue';
import SamplersBulkDeleteModal from '@/Components/Samplers/SamplersBulkDeleteModal.vue';
import SamplersFavoriteCell from '@/Components/Samplers/SamplersFavoriteCell.vue';
...
// El toolbar inline de Samplers (no usa ModuleToolbar) repite manualmente
// estos gates para no mostrar botones que no funcionan al user free/basic.
const { canUse: canUsePlanFeature } = usePlanFeatures();

import {
    samplersFilterFields, samplersEmptyFilters, hydrateSamplersFilters,
    samplersFiltersToQuery, samplersFiltersSummary,
    serializeSavedFilters, deserializeSavedFilters,
} from './config/filters';
import { samplersTableColumns } from './config/columns';
import { samplersExportableColumns, samplersExportEndpoints } from './config/exports';
...
const props = defineProps({
    samplers:      { type: Object, required: true },
...
const {
    ...
    hydrate:       hydrateSamplersFilters,
    toQuery:       samplersFiltersToQuery,
    summary:       samplersFiltersSummary,
    empty:         samplersEmptyFilters,
    only:          ['samplers', 'filters'],
    t,
});
...
const { counterLabel } = useModuleListMeta({
    pagination: computed(() => props.samplers),
    hasActiveFilters,
    t,
});
...
const allColumns = computed(() =>
    samplersTableColumns(t, { isSuper: isSuper.value, isMobile: isMobileScreen.value }),
);
const { visibleColumnKeys, columns } = useColumnPreferences(allColumns);

// ─── Paginacion + sort ──────────────────────────────────────────────────────
const tablePagination = computed(() => ({
    current:  props.samplers.current_page,
    pageSize: props.samplers.per_page,
    total:    props.samplers.total,
...
const applyInlineFilters = (cleaned) => {
    ...
    inlineTimer = setTimeout(() => {
        ...
        router.get(route('business_management.samplers.index'), data, {
            preserveScroll: true,
            preserveState: true,
...
const VIEW_KEY = 'samplers_view_mode';
...
useModuleUndoToast('business_management.samplers.undo_last_delete');

// ─── Favoritos polimorficos ────────────────────────────────────────────────
const { submitting: favoriteSubmitting, toggle: toggleFavorite } = useModuleFavorites('samplers', 'samplers');
...
const {
    ...
    bulkSetActiveRoute: 'business_management.samplers.bulk_set_active',
    bulkDeleteRoute:    'business_management.samplers.bulk_delete',
    resourceLabel:      t('samplers.records'),
...
const duplicate = (record) => {
    ...
    router.post(route('business_management.samplers.duplicate', record.slug), {}, {
        preserveScroll: true,
...
const exportableColumns = computed(() => samplersExportableColumns(t, { isSuper: isSuper.value }));
const exportEndpoints   = computed(() => samplersExportEndpoints());
...
<template>
    ...
    <div class="sap-index">
        ...
        <Drawer v-model:open="filtersOpen" :title="$t('global.filters')" placement="right" :width="380">
            <FilterBar :fields="filterFields" v-model="filters" storage-key="samplers" />
        </Drawer>
        ...
        <div class="mi-tabletoolbar">
            ...
            <div class="mi-tabletoolbar__right">
                <label class="mi-bar mi-bar--toolbar" :class="{ 'is-active': quickSearch }">
                    ...
                    <input v-model="quickSearch" class="mi-bar__input" :placeholder="$t('global.search_in', { item: $t('samplers.singular').toLowerCase() })" autocomplete="off" spellcheck="false" type="text" />
                ...
                <Tooltip v-if="can('samplers.export')" :title="$t('global.export_hint')" data-tour="export-import">
                ...
                <Dropdown :trigger="['click']" placement="bottomRight">
                    ...
                    <template #overlay>
                        <Menu>
                            <MenuItem v-if="can('samplers.create') && can('samplers.import') && canUsePlanFeature('imports')" key="import" @click="importOpen = true"><UploadOutlined /> {{ $t('global.import') }}</MenuItem>
                            <MenuItem v-if="can('samplers.edit') && canUsePlanFeature('edit_all')" key="editall" @click="router.visit(route('business_management.samplers.edit_all'))"><FormOutlined /> {{ $t('global.edit_all') }}</MenuItem>
                            <MenuItem v-if="isSuper" key="trash" @click="router.visit(route('business_management.samplers.trash'))"><InboxOutlined /> {{ $t('global.view_deleted') }}</MenuItem>
        ...
        <Card :bodyStyle="{ padding: 0 }" class="grid-card">
            ...
            <ResponsiveTable
                :loading="isFetching"
                :dataSource="samplers.data"
                :columns="columns"
                :pagination="tablePagination"
                :row-selection="(can('samplers.delete') || can('samplers.edit')) ? rowSelection : null"
            ...
            >
                ...
                <template #bodyCell="{ column, record, text, isMobile, compact }">
                    <SamplersFavoriteCell
                        v-if="column.key === 'favorite'"
                        :record="record"
                        :submitting="favoriteSubmitting"
                        :data-tour="record === samplers.data[0] ? 'favorites' : null"
                    ...
                    <template v-else-if="column.key === 'name'">
                        <div class="lead">
                            <div class="lead__txt">
                                <Link :href="route('business_management.samplers.show', record.slug)" class="lead__name lead__link">{{ record.name }}</Link>
        ...
        <SamplersBulkDeleteModal
            v-model:open="bulkOpen"
            v-model:reason="bulkReason"
            :count="selectedRowKeys.length"
            :submitting="bulkSubmitting"
            :error-msg="bulkError"
            :resource-label="selectedRowKeys.length === 1 ? $t('samplers.record') : $t('samplers.records')"
        ...
        <ExportDialog
            ...
            :default-title="$t('samplers.export_title')"
            :endpoints="exportEndpoints"
            :limits="exportLimits"
            :total-rows="samplers.total ?? 0"
            :total-unfiltered="samplers.total_unfiltered ?? samplers.total ?? 0"
        />
