...
import SamplersTrashBulkBar from '@/Components/Samplers/SamplersTrashBulkBar.vue';
import SamplersForceDeleteModal from '@/Components/Samplers/SamplersForceDeleteModal.vue';

import { useModuleRestore } from '@/Composables/useModuleRestore';
import { useDateFormat } from '@/Composables/useDateFormat';
import { useI18n } from '@/Plugins/i18n';
import { samplersTrashColumns } from './config/trashColumns';
...
if (!isSuper) {
    router.visit(route('business_management.samplers.index'));
}
...
const {
    ...
    restoreRouteName:     'business_management.samplers.restore',
    bulkRestoreRouteName: 'business_management.samplers.bulk_restore',
});
...
const openForceDelete = (sampler) => {
    forceDeleteTarget.value = sampler;
...
const submitForceDelete = () => {
    ...
    forceDeleteErrors.value = {};
    router.delete(
        route('business_management.samplers.force_delete', forceDeleteTarget.value.slug),
        {
...
const columns = computed(() => samplersTrashColumns(t));
...
const subtitle = computed(() => {
    const word = props.samplers.total === 1 ? t('global.record') : t('global.records');
    return `${props.samplers.total} ${word} · ${t('global.super_only')}`;
});
</script>

<template>
    <Head :title="$t('global.view_deleted') + ' — ' + $t('samplers.plural')" />

    <div v-if="isSuper" class="sap-form trash-page">
        <SectionHeader
            :back-href="route('business_management.samplers.index')"
            :title="$t('global.view_deleted') + ' — ' + $t('samplers.plural')"
            :subtitle="subtitle"
        ...
        <Card :bodyStyle="{ padding: 0 }" class="grid-card">
                    ...
                    <template v-else-if="column.key === 'actions'">
                        <Space :size="4">
                            <Popconfirm
                                ...
                                :description="$t('samplers.restore_hint')"
            ...
            <Empty
                v-if="samplers.data.length === 0"
                :description="$t('global.no_deleted_records')"
                style="padding: 48px 16px"
            />
        </Card>

        <SamplersTrashBulkBar
