...
/**
 * Schema de filtros del módulo Samplers (catálogo global de marcas).
 * Campos de dominio: name + code + is_active. Mismo patrón que Regions/Tenants.
 */
export const samplersFilterFields = (t) => [
    { key: 'name',           label: t('samplers.filter_name'), type: 'tags' },
    { key: 'code',           label: t('samplers.code'),        type: 'text' },
    { key: 'is_active',      label: t('samplers.is_active'),   type: 'select', options: [
...
export const samplersEmptyFilters = () => ({
    name: [],
    code: '',
    is_active: null,
    created_at: null,
...
export const hydrateSamplersFilters = (server) => ({
...
export const samplersFiltersToQuery = (f) => ({
...
export const samplersFiltersSummary = (f, t) => {
    const parts = [];
    if (f.name?.length)        parts.push(`${t('samplers.filter_name')}: ${f.name.join(', ')}`);
    if (f.code)                parts.push(`${t('samplers.code')}: ${f.code}`);
    if (f.is_active !== null && f.is_active !== undefined) {
        parts.push(`${t('samplers.is_active')}: ${f.is_active ? t('global.active') : t('global.inactive')}`);
    }
