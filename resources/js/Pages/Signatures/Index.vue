...
import SignaturesBulkBar from '@/Components/Signatures/SignaturesBulkBar.vue';
import SignaturesBulkDeleteModal from '@/Components/Signatures/SignaturesBulkDeleteModal.vue';
import SignaturesFavoriteCell from '@/Components/Signatures/SignaturesFavoriteCell.vue';
import SignaturesPageHeader from '@/Components/Signatures/SignaturesPageHeader.vue';
import SignaturesActionsCell from '@/Components/Signatures/SignaturesActionsCell.vue';
...
// El toolbar inline de Signatures (no usa ModuleToolbar) repite manualmente
// estos gates para no mostrar botones que no funcionan al user free/basic.
const { canUse: canUsePlanFeature } = usePlanFeatures();

import {
    signaturesFilterFields, signaturesEmptyFilters, hydrateSignaturesFilters,
    signaturesFiltersToQuery, signaturesFiltersSummary,
    serializeSavedFilters, deserializeSavedFilters,
} from './config/filters';
import { signaturesTableColumns } from './config/columns';
import { signaturesExportableColumns, signaturesExportEndpoints } from './config/exports';
...
const {
    ...
    hydrate:       hydrateSignaturesFilters,
    toQuery:       signaturesFiltersToQuery,
    summary:       signaturesFiltersSummary,
    empty:         signaturesEmptyFilters,
    only:          ['signatures', 'filters'],
    t,
});
...
const { counterLabel } = useModuleListMeta({
    pagination: computed(() => props.signatures),
    hasActiveFilters,
    t,
});
...
const allColumns = computed(() =>
    signaturesTableColumns(t, { isSuper: isSuper.value, isMobile: isMobileScreen.value }),
);
const { visibleColumnKeys, columns } = useColumnPreferences(allColumns);

// ─── Paginacion + sort ──────────────────────────────────────────────────────
const tablePagination = computed(() => ({
    current:  props.signatures.current_page,
    pageSize: props.signatures.per_page,
    total:    props.signatures.total,
...
const applyInlineFilters = (cleaned) => {
    ...
    inlineTimer = setTimeout(() => {
        ...
        router.get(route('business_management.signatures.index'), data, {
            preserveScroll: true,
            preserveState: true,
...
const VIEW_KEY = 'signatures_view_mode';
...
useModuleUndoToast('business_management.signatures.undo_last_delete');

// ─── Favoritos polimorficos ────────────────────────────────────────────────
const { submitting: favoriteSubmitting, toggle: toggleFavorite } = useModuleFavorites('signatures', 'signatures');
...
const {
    ...
    bulkSetActiveRoute: 'business_management.signatures.bulk_set_active',
    bulkDeleteRoute:    'business_management.signatures.bulk_delete',
    resourceLabel:      t('signatures.records'),
...
const duplicate = (record) => {
    ...
    router.post(route('business_management.signatures.duplicate', record.slug), {}, {
        preserveScroll: true,
...
const exportableColumns = computed(() => signaturesExportableColumns(t, { isSuper: isSuper.value }));
const exportEndpoints   = computed(() => signaturesExportEndpoints());
...
const tour = useModuleTour({ module: 'signatures', steps: () => moduleTourSteps(t, { moduleName: t('signatures.plural') }) });

// ─── Keyboard shortcuts ────────────────────────────────────────────────────
useKeyboardShortcuts({
    'ctrl+n': () => can('signatures.create') && router.visit(route('business_management.signatures.create')),
    'esc': () => {
...
// ─── Acciones ───────────────────────────────────────────────────────────────
const goEdit   = (record) => router.visit(route('business_management.signatures.edit',   record.slug));
const goDelete = (record) => router.visit(route('business_management.signatures.delete', record.slug));
</script>

<template>
    <Head :title="$t('signatures.plural')" />

    <div class="sap-index">
        <!-- Título + acciones (las acciones viven en el toolbar de la tabla). -->
        <div class="mi-title" data-tour="module">
            <SignaturesPageHeader
                :title="$t('signatures.plural')"
            />
        </div>
        ...
        <Drawer v-model:open="filtersOpen" :title="$t('global.filters')" placement="right" :width="380">
            <FilterBar :fields="filterFields" v-model="filters" storage-key="signatures" />
        </Drawer>
        ...
        <div class="mi-tabletoolbar">
            ...
            <div class="mi-tabletoolbar__right">
                <label class="mi-bar mi-bar--toolbar" :class="{ 'is-active': quickSearch }">
                    ...
                    <input v-model="quickSearch" class="mi-bar__input" :placeholder="$t('global.search_in', { item: $t('signatures.singular').toLowerCase() })" autocomplete="off" spellcheck="false" type="text" />
                ...
                <Tooltip v-if="can('signatures.export')" :title="$t('global.export_hint')" data-tour="export-import">
                ...
                <Dropdown :trigger="['click']" placement="bottomRight">
                    ...
                    <template #overlay>
                        <Menu>
                            <MenuItem v-if="can('signatures.create') && can('signatures.import') && canUsePlanFeature('imports')" key="import" @click="importOpen = true"><UploadOutlined /> {{ $t('global.import') }}</MenuItem>
                            <MenuItem v-if="can('signatures.edit') && canUsePlanFeature('edit_all')" key="editall" @click="router.visit(route('business_management.signatures.edit_all'))"><FormOutlined /> {{ $t('global.edit_all') }}</MenuItem>
                            <MenuItem v-if="isSuper" key="trash" @click="router.visit(route('business_management.signatures.trash'))"><InboxOutlined /> {{ $t('global.view_deleted') }}</MenuItem>
                ...
                <Tooltip v-if="can('signatures.create')" :title="$t('signatures.new')" data-tour="new">
                    <Link :href="route('business_management.signatures.create')">
                        <Button type="primary" class="mi-iconbtn mi-create-btn" :aria-label="$t('signatures.new')"><PlusOutlined /></Button>
                    </Link>
                </Tooltip>
            </div>
        </div>
        ...
        <Card :bodyStyle="{ padding: 0 }" class="grid-card">
            ...
            <ResponsiveTable
                ...
                :row-selection="(can('signatures.delete') || can('signatures.edit')) ? rowSelection : null"
            ...
            >
                ...
                <template #bodyCell="{ column, record, text, isMobile, compact }">
                    ...
                    <template v-else-if="column.key === 'name'">
                        <div class="lead">
                            <div class="lead__txt">
                                <Link :href="route('business_management.signatures.show', record.slug)" class="lead__name lead__link">{{ record.name }}</Link>
        ...
        <SignaturesBulkDeleteModal
            v-model:open="bulkOpen"
            v-model:reason="bulkReason"
            :count="selectedRowKeys.length"
            :submitting="bulkSubmitting"
            :error-msg="bulkError"
            :resource-label="selectedRowKeys.length === 1 ? $t('signatures.record') : $t('signatures.records')"
        ...
        <ExportDialog
            ...
            :default-title="$t('signatures.export_title')"
            :endpoints="exportEndpoints"
            :limits="exportLimits"
            :total-rows="signatures.total ?? 0"
            :total-unfiltered="signatures.total_unfiltered ?? signatures.total ?? 0"
        />

        <ImportDialog
            v-model:open="importOpen"
            :endpoint="route('business_management.signatures.import')"
            :template-url="route('business_management.signatures.import_template')"
            :resource-label="$t('signatures.records')"
        />
    </div>
</template>
