...
import SignaturesTrashBulkBar from '@/Components/Signatures/SignaturesTrashBulkBar.vue';
import SignaturesForceDeleteModal from '@/Components/Signatures/SignaturesForceDeleteModal.vue';

import { useModuleRestore } from '@/Composables/useModuleRestore';
import { useDateFormat } from '@/Composables/useDateFormat';
import { useI18n } from '@/Plugins/i18n';
import { signaturesTrashColumns } from './config/trashColumns';
...
if (!isSuper) {
    router.visit(route('business_management.signatures.index'));
}
...
const {
    ...
    restoreRouteName:     'business_management.signatures.restore',
    bulkRestoreRouteName: 'business_management.signatures.bulk_restore',
});
...
const openForceDelete = (signature) => {
    forceDeleteTarget.value = signature;
...
const submitForceDelete = () => {
    ...
    forceDeleteErrors.value = {};
    router.delete(
        route('business_management.signatures.force_delete', forceDeleteTarget.value.slug),
        {
...
const columns = computed(() => signaturesTrashColumns(t));
...
const subtitle = computed(() => {
    const word = props.signatures.total === 1 ? t('global.record') : t('global.records');
    return `${props.signatures.total} ${word} · ${t('global.super_only')}`;
});
</script>

<template>
    <Head :title="$t('global.view_deleted') + ' — ' + $t('signatures.plural')" />

    <div v-if="isSuper" class="sap-form trash-page">
        <SectionHeader
            :back-href="route('business_management.signatures.index')"
            :title="$t('global.view_deleted') + ' — ' + $t('signatures.plural')"
            :subtitle="subtitle"
                    ...
                    <template v-else-if="column.key === 'actions'">
                        <Space :size="4">
                            <Popconfirm
                                ...
                                :description="$t('signatures.restore_hint')"
