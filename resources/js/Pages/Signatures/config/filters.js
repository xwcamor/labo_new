...
/**
 * Schema de filtros del módulo Signatures (catálogo global de marcas).
 * Campos de dominio: name + code + is_active. Mismo patrón que Regions/Tenants.
 */
export const signaturesFilterFields = (t) => [
    { key: 'name',           label: t('signatures.filter_name'), type: 'tags' },
    { key: 'code',           label: t('signatures.code'),        type: 'text' },
    { key: 'is_active',      label: t('signatures.is_active'),   type: 'select', options: [
...
export const signaturesEmptyFilters = () => ({
    name: [],
    code: '',
    is_active: null,
    created_at: null,
...
export const hydrateSignaturesFilters = (server) => ({
...
export const signaturesFiltersToQuery = (f) => ({
...
export const signaturesFiltersSummary = (f, t) => {
    const parts = [];
    if (f.name?.length)        parts.push(`${t('signatures.filter_name')}: ${f.name.join(', ')}`);
    if (f.code)                parts.push(`${t('signatures.code')}: ${f.code}`);
    if (f.is_active !== null && f.is_active !== undefined) {
        parts.push(`${t('signatures.is_active')}: ${f.is_active ? t('global.active') : t('global.inactive')}`);
    }
