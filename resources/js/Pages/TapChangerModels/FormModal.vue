<script setup>
/**
 * Alta y edición del modelo del cambiador de tomas, en diálogo (regla Fiori de la
...
const title = computed(() => (isEdit.value
    ? `${t('global.edit')} ${t('tap_changer_models.record')}`
    : t('tap_changer_models.new')));

const submit = () => {
    const opciones = { preserveScroll: true, onSuccess: () => emit('close') };

    if (isEdit.value) {
        form.put(route('business_management.tap_changer_models.update', props.record.slug), opciones);
    } else {
        form.post(route('business_management.tap_changer_models.store'), opciones);
    }
};
</script>
...
<template>
    <FormModal
        ...
        create-label-key="tap_changer_models.new"
        @close="emit('close')"
        @submit="submit"
    >
        <FormItem
