<script setup>
/**
 * Alta y edición de la tecnología del cambiador de tomas, en diálogo (regla Fiori de la
...
const title = computed(() => (isEdit.value
    ? `${t('global.edit')} ${t('tap_changer_technologies.record')}`
    : t('tap_changer_technologies.new')));

const submit = () => {
    const opciones = { preserveScroll: true, onSuccess: () => emit('close') };

    if (isEdit.value) {
        form.put(route('business_management.tap_changer_technologies.update', props.record.slug), opciones);
    } else {
        form.post(route('business_management.tap_changer_technologies.store'), opciones);
    }
};
</script>
...
<template>
    <FormModal
        ...
        create-label-key="tap_changer_technologies.new"
        @close="emit('close')"
        @submit="submit"
    >
        <FormItem
