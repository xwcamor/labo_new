...
const submit = () => {
    form.delete(route('lab_management.test_definitions.deleteSave', props.testDefinition.slug), {
        preserveScroll: true,
    });
};
</script>
...
<template>
    <Head :title="$t('global.delete') + ' — ' + $t('test_definitions.singular')" />

    <DeletePage
        :back-href="route('lab_management.test_definitions.index')"
        :title="$t('global.delete') + ' ' + $t('test_definitions.record')"
        :subtitle="testDefinition.name"
    ...
    >
        ...
        <template #summary>
            <DeleteSummaryRow :label="$t('test_definitions.name')">{{ testDefinition.name }}</DeleteSummaryRow>
            <DeleteSummaryRow v-if="testDefinition.code" :label="$t('test_definitions.code')">
                <code>{{ testDefinition.code }}</code>
            </DeleteSummaryRow>
            <DeleteSummaryRow :label="$t('test_definitions.is_active')">
                <Tag :color="testDefinition.is_active ? 'success' : 'error'" :bordered="false">
