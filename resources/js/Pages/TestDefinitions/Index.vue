...
import TestDefinitionsBulkBar from '@/Components/TestDefinitions/TestDefinitionsBulkBar.vue';
import TestDefinitionsBulkDeleteModal from '@/Components/TestDefinitions/TestDefinitionsBulkDeleteModal.vue';
import TestDefinitionsFavoriteCell from '@/Components/TestDefinitions/TestDefinitionsFavoriteCell.vue';
import TestDefinitionsPageHeader from '@/Components/TestDefinitions/TestDefinitionsPageHeader.vue';
import TestDefinitionsActionsCell from '@/Components/TestDefinitions/TestDefinitionsActionsCell.vue';
import TestDefinitionsEmptyState from '@/Components/TestDefinitions/TestDefinitionsEmptyState.vue';
...
// El toolbar inline de TestDefinitions (no usa ModuleToolbar) repite manualmente
// estos gates para no mostrar botones que no funcionan al user free/basic.
const { canUse: canUsePlanFeature } = usePlanFeatures();

import {
    testDefinitionsFilterFields, testDefinitionsEmptyFilters, hydrateTestDefinitionsFilters,
    testDefinitionsFiltersToQuery, testDefinitionsFiltersSummary,
    serializeSavedFilters, deserializeSavedFilters,
} from './config/filters';
import { testDefinitionsTableColumns } from './config/columns';
import { testDefinitionsExportableColumns, testDefinitionsExportEndpoints } from './config/exports';
...
const props = defineProps({
    ...
    // Opciones del filtro por grupo, tal como las manda el controlador. Son
    // tres filas: se mandan enteras en vez de montar un endpoint de búsqueda.
    groups:         { type: Array,  default: () => [] },
});

// ─── Filtros (schema + (de)serialización en config/filters.js) ──────────────
const filterFields = computed(() =>
    testDefinitionsFilterFields(t, { groups: props.groups }),
);
...
const {
    ...
    hydrate:       hydrateTestDefinitionsFilters,
    toQuery:       testDefinitionsFiltersToQuery,
    summary:       (f, tt) => testDefinitionsFiltersSummary(f, tt, { groups: props.groups }),
    empty:         testDefinitionsEmptyFilters,
    only:          ['test_definitions', 'filters'],
    t,
});
...
const { counterLabel } = useModuleListMeta({
    pagination: computed(() => props.test_definitions),
    hasActiveFilters,
    t,
});
...
const allColumns = computed(() =>
    testDefinitionsTableColumns(t, { isSuper: isSuper.value, isMobile: isMobileScreen.value }),
);
const { visibleColumnKeys, columns } = useColumnPreferences(allColumns);

// ─── Paginacion + sort ──────────────────────────────────────────────────────
const tablePagination = computed(() => ({
    current:  props.test_definitions.current_page,
    pageSize: props.test_definitions.per_page,
    total:    props.test_definitions.total,
...
const applyInlineFilters = (cleaned) => {
    ...
    inlineTimer = setTimeout(() => {
        ...
        router.get(route('lab_management.test_definitions.index'), data, {
            preserveScroll: true,
            preserveState: true,
...
const VIEW_KEY = 'test_definitions_view_mode';
...
useModuleUndoToast('lab_management.test_definitions.undo_last_delete');

// ─── Favoritos polimorficos ────────────────────────────────────────────────
const { submitting: favoriteSubmitting, toggle: toggleFavorite } = useModuleFavorites('test_definitions', 'test_definitions');
...
const {
    ...
    bulkSetActiveRoute: 'lab_management.test_definitions.bulk_set_active',
    bulkDeleteRoute:    'lab_management.test_definitions.bulk_delete',
    resourceLabel:      t('test_definitions.records'),
...
const duplicate = (record) => {
    ...
    router.post(route('lab_management.test_definitions.duplicate', record.slug), {}, {
        preserveScroll: true,
...
const exportableColumns = computed(() => testDefinitionsExportableColumns(t, { isSuper: isSuper.value }));
const exportEndpoints   = computed(() => testDefinitionsExportEndpoints());
...
const tour = useModuleTour({ module: 'test_definitions', steps: () => moduleTourSteps(t, { moduleName: t('test_definitions.plural') }) });

// ─── Keyboard shortcuts ────────────────────────────────────────────────────
useKeyboardShortcuts({
    'ctrl+n': () => can('test_definitions.create') && router.visit(route('lab_management.test_definitions.create')),
    'esc': () => {
...
// ─── Acciones ───────────────────────────────────────────────────────────────
const goEdit   = (record) => router.visit(route('lab_management.test_definitions.edit',   record.slug));
const goDelete = (record) => router.visit(route('lab_management.test_definitions.delete', record.slug));
</script>

<template>
    <Head :title="$t('test_definitions.plural')" />

    <div class="sap-index">
        <!-- Título + acciones (las acciones viven en el toolbar de la tabla). -->
        <div class="mi-title" data-tour="module">
            <TestDefinitionsPageHeader
                :title="$t('test_definitions.plural')"
            />
        </div>
        ...
        <Drawer v-model:open="filtersOpen" :title="$t('global.filters')" placement="right" :width="380">
            <FilterBar :fields="filterFields" v-model="filters" storage-key="test_definitions" />
        </Drawer>
        ...
        <div class="mi-tabletoolbar">
            ...
            <div class="mi-tabletoolbar__right">
                <label class="mi-bar mi-bar--toolbar" :class="{ 'is-active': quickSearch }">
                    ...
                    <input v-model="quickSearch" class="mi-bar__input" :placeholder="$t('global.search_in', { item: $t('test_definitions.singular').toLowerCase() })" autocomplete="off" spellcheck="false" type="text" />
                ...
                <Tooltip v-if="can('test_definitions.export')" :title="$t('global.export_hint')" data-tour="export-import">
                ...
                <Dropdown :trigger="['click']" placement="bottomRight">
                    ...
                    <template #overlay>
                        <Menu>
                            <MenuItem v-if="can('test_definitions.create') && can('test_definitions.import') && canUsePlanFeature('imports')" key="import" @click="importOpen = true"><UploadOutlined /> {{ $t('global.import') }}</MenuItem>
                            <MenuItem v-if="can('test_definitions.edit') && canUsePlanFeature('edit_all')" key="editall" @click="router.visit(route('lab_management.test_definitions.edit_all'))"><FormOutlined /> {{ $t('global.edit_all') }}</MenuItem>
                            <MenuItem v-if="isSuper" key="trash" @click="router.visit(route('lab_management.test_definitions.trash'))"><InboxOutlined /> {{ $t('global.view_deleted') }}</MenuItem>
                ...
                <Tooltip v-if="can('test_definitions.create')" :title="$t('test_definitions.new')" data-tour="new">
                    <Link :href="route('lab_management.test_definitions.create')">
                        <Button type="primary" class="mi-iconbtn mi-create-btn" :aria-label="$t('test_definitions.new')"><PlusOutlined /></Button>
                    </Link>
                </Tooltip>
            </div>
        </div>
        ...
        <Card :bodyStyle="{ padding: 0 }" class="grid-card">
            ...
            <ResponsiveTable
                :loading="isFetching"
                :dataSource="test_definitions.data"
                :columns="columns"
                :pagination="tablePagination"
                :row-selection="(can('test_definitions.delete') || can('test_definitions.edit')) ? rowSelection : null"
            ...
            >
                <template #empty>
                    <TestDefinitionsEmptyState
                        :has-filters="hasActiveFilters"
                        :can-create="can('test_definitions.create')"
                        @clear-filters="clearFilters"
                        @open-import="importOpen = true"
                    />
                </template>
                <template #bodyCell="{ column, record, text, isMobile, compact }">
                    <TestDefinitionsFavoriteCell
                        v-if="column.key === 'favorite'"
                        :record="record"
                        :submitting="favoriteSubmitting"
                        :data-tour="record === test_definitions.data[0] ? 'favorites' : null"
                    ...
                    <template v-else-if="column.key === 'name'">
                        <div class="lead">
                            <div class="lead__txt">
                                <Link :href="route('lab_management.test_definitions.show', record.slug)" class="lead__name lead__link">{{ record.name }}</Link>
                    ...
                    <template v-else-if="column.key === 'group'">
                        <Tag v-if="record.group" color="blue" :bordered="false">{{ record.group.name }}</Tag>
                        <span v-else class="muted">{{ $t('test_definitions.group_none') }}</span>
                    </template>

                    <!-- Las tres banderas juntas: es lo que decide qué exige la
                         hoja de trabajo antes de aceptar muestras. Solo se
                         muestran las que están en si; una fila con las tres
                         apagadas queda limpia. -->
                    <template v-else-if="column.key === 'control'">
                        <Space :size="4" wrap>
                            <Tag v-if="record.requires_control" color="orange" :bordered="false">
                                {{ $t('test_definitions.requires_control') }}
                            </Tag>
                            <Tag v-else-if="record.has_control" :bordered="false">
                                {{ $t('test_definitions.has_control') }}
                            </Tag>
                            <Tag v-if="record.requires_duplicate" color="purple" :bordered="false">
                                {{ $t('test_definitions.requires_duplicate') }}
                            </Tag>
                            <span v-if="!record.has_control && !record.requires_control && !record.requires_duplicate" class="muted">—</span>
                        </Space>
                    </template>

                    <!-- 1 repetición es el caso normal y no aporta nada leerlo
                         en 28 de las 29 filas; se resalta solo cuando hay más. -->
                    <template v-else-if="column.key === 'replicates'">
                        <strong v-if="record.replicates > 1">{{ record.replicates }}</strong>
        ...
        <TestDefinitionsBulkDeleteModal
            v-model:open="bulkOpen"
            v-model:reason="bulkReason"
            :count="selectedRowKeys.length"
            :submitting="bulkSubmitting"
            :error-msg="bulkError"
            :resource-label="selectedRowKeys.length === 1 ? $t('test_definitions.record') : $t('test_definitions.records')"
        ...
        <ExportDialog
            ...
            :default-title="$t('test_definitions.export_title')"
            :endpoints="exportEndpoints"
            :limits="exportLimits"
            :total-rows="test_definitions.total ?? 0"
            :total-unfiltered="test_definitions.total_unfiltered ?? test_definitions.total ?? 0"
        />

        <ImportDialog
            v-model:open="importOpen"
            :endpoint="route('lab_management.test_definitions.import')"
            :template-url="route('lab_management.test_definitions.import_template')"
            :resource-label="$t('test_definitions.records')"
        />
    </div>
</template>
