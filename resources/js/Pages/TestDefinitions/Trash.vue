...
import TestDefinitionsTrashBulkBar from '@/Components/TestDefinitions/TestDefinitionsTrashBulkBar.vue';
import TestDefinitionsForceDeleteModal from '@/Components/TestDefinitions/TestDefinitionsForceDeleteModal.vue';

import { useModuleRestore } from '@/Composables/useModuleRestore';
import { useDateFormat } from '@/Composables/useDateFormat';
import { useI18n } from '@/Plugins/i18n';
import { testDefinitionsTrashColumns } from './config/trashColumns';
...
if (!isSuper) {
    router.visit(route('lab_management.test_definitions.index'));
}
...
const {
    ...
    restoreRouteName:     'lab_management.test_definitions.restore',
    bulkRestoreRouteName: 'lab_management.test_definitions.bulk_restore',
});
...
const openForceDelete = (testDefinition) => {
    forceDeleteTarget.value = testDefinition;
...
const submitForceDelete = () => {
    ...
    forceDeleteErrors.value = {};
    router.delete(
        route('lab_management.test_definitions.force_delete', forceDeleteTarget.value.slug),
        {
...
const columns = computed(() => testDefinitionsTrashColumns(t));
...
const subtitle = computed(() => {
    const word = props.test_definitions.total === 1 ? t('global.record') : t('global.records');
    return `${props.test_definitions.total} ${word} · ${t('global.super_only')}`;
});
</script>

<template>
    <Head :title="$t('global.view_deleted') + ' — ' + $t('test_definitions.plural')" />

    <div v-if="isSuper" class="sap-form trash-page">
        <SectionHeader
            :back-href="route('lab_management.test_definitions.index')"
            :title="$t('global.view_deleted') + ' — ' + $t('test_definitions.plural')"
            :subtitle="subtitle"
        ...
        <Card :bodyStyle="{ padding: 0 }" class="grid-card">
                    ...
                    <template v-else-if="column.key === 'actions'">
                        <Space :size="4">
                            <Popconfirm
                                ...
                                :description="$t('test_definitions.restore_hint')"
            ...
            <Empty
                v-if="test_definitions.data.length === 0"
                :description="$t('global.no_deleted_records')"
                style="padding: 48px 16px"
            />
        </Card>

        <TestDefinitionsTrashBulkBar
        ...
        <TestDefinitionsForceDeleteModal
