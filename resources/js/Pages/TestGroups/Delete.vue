...
const submit = () => {
    form.delete(route('lab_management.test_groups.deleteSave', props.testGroup.slug), {
        preserveScroll: true,
    });
};
</script>
...
<template>
    <Head :title="$t('global.delete') + ' — ' + $t('test_groups.singular')" />

    <DeletePage
        :back-href="route('lab_management.test_groups.index')"
        :title="$t('global.delete') + ' ' + $t('test_groups.record')"
        :subtitle="testGroup.name"
    ...
    >
        ...
        <template #summary>
            <DeleteSummaryRow :label="$t('test_groups.name')">{{ testGroup.name }}</DeleteSummaryRow>
            <DeleteSummaryRow v-if="testGroup.code" :label="$t('test_groups.code')">
                <code>{{ testGroup.code }}</code>
            </DeleteSummaryRow>
            <DeleteSummaryRow :label="$t('test_groups.is_active')">
                <Tag :color="testGroup.is_active ? 'success' : 'error'" :bordered="false">
