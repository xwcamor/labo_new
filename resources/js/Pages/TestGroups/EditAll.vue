...
import TestGroupsEditAllTable from '@/Components/TestGroups/TestGroupsEditAllTable.vue';
...
/**
 * Edit-All de TestGroups. Mantenemos snapshot `original` + `draft` mutable
...
const saveAll = () => {
    ...
    router.post(
        route('lab_management.test_groups.edit_all.update'),
...
<template>
    <Head :title="$t('test_groups.edit_all_title')" />
    ...
    <div class="edit-all sap-form">
        ...
        <Card :bodyStyle="{ padding: 0 }" class="edit-table-card">
            <TestGroupsEditAllTable
                v-model:draft="draft"
                :is-dirty="isDirty"
                :duplicate-rows="duplicateRows"
            />

        <div v-if="test_groups.total > test_groups.per_page" class="edit-pagination">
            <Pagination
                :current="test_groups.current_page"
                :pageSize="test_groups.per_page"
        ...
        <EditAllFooter
            :discard-label="$t('test_groups.edit_all_discard')"
            :save-label="$t('test_groups.edit_all_save_all')"
            :discard-disabled="dirtyCount === 0"
            :save-disabled="dirtyCount === 0 || duplicateRows.size > 0"
            :submitting="submitting"
            :status-text="dirtyCount > 0 ? $tc('test_groups.edit_all_changes', dirtyCount) : ''"
            @discard="discardAll"
            @save="saveAll"
        />
