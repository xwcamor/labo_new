...
import TestGroupsBulkBar from '@/Components/TestGroups/TestGroupsBulkBar.vue';
import TestGroupsBulkDeleteModal from '@/Components/TestGroups/TestGroupsBulkDeleteModal.vue';
import TestGroupsFavoriteCell from '@/Components/TestGroups/TestGroupsFavoriteCell.vue';
...
// El toolbar inline de TestGroups (no usa ModuleToolbar) repite manualmente
// estos gates para no mostrar botones que no funcionan al user free/basic.
const { canUse: canUsePlanFeature } = usePlanFeatures();

import {
    testGroupsFilterFields, testGroupsEmptyFilters, hydrateTestGroupsFilters,
    testGroupsFiltersToQuery, testGroupsFiltersSummary,
    serializeSavedFilters, deserializeSavedFilters,
} from './config/filters';
import { testGroupsTableColumns } from './config/columns';
import { testGroupsExportableColumns, testGroupsExportEndpoints } from './config/exports';
...
const props = defineProps({
    test_groups:      { type: Object, required: true },
...
const {
    ...
    hydrate:       hydrateTestGroupsFilters,
    toQuery:       testGroupsFiltersToQuery,
    summary:       testGroupsFiltersSummary,
    empty:         testGroupsEmptyFilters,
    only:          ['test_groups', 'filters'],
    t,
});
...
const { counterLabel } = useModuleListMeta({
    pagination: computed(() => props.test_groups),
    hasActiveFilters,
    t,
});
...
const allColumns = computed(() =>
    testGroupsTableColumns(t, { isSuper: isSuper.value, isMobile: isMobileScreen.value }),
);
const { visibleColumnKeys, columns } = useColumnPreferences(allColumns);

// ─── Paginacion + sort ──────────────────────────────────────────────────────
const tablePagination = computed(() => ({
    current:  props.test_groups.current_page,
    pageSize: props.test_groups.per_page,
    total:    props.test_groups.total,
...
const applyInlineFilters = (cleaned) => {
    ...
    inlineTimer = setTimeout(() => {
        ...
        router.get(route('lab_management.test_groups.index'), data, {
            preserveScroll: true,
            preserveState: true,
...
const VIEW_KEY = 'test_groups_view_mode';
...
useModuleUndoToast('lab_management.test_groups.undo_last_delete');

// ─── Favoritos polimorficos ────────────────────────────────────────────────
const { submitting: favoriteSubmitting, toggle: toggleFavorite } = useModuleFavorites('test_groups', 'test_groups');
...
const {
    ...
    bulkSetActiveRoute: 'lab_management.test_groups.bulk_set_active',
    bulkDeleteRoute:    'lab_management.test_groups.bulk_delete',
    resourceLabel:      t('test_groups.records'),
...
const duplicate = (record) => {
    ...
    router.post(route('lab_management.test_groups.duplicate', record.slug), {}, {
        preserveScroll: true,
...
const exportableColumns = computed(() => testGroupsExportableColumns(t, { isSuper: isSuper.value }));
const exportEndpoints   = computed(() => testGroupsExportEndpoints());
...
<template>
    ...
    <div class="sap-index">
        ...
        <Drawer v-model:open="filtersOpen" :title="$t('global.filters')" placement="right" :width="380">
            <FilterBar :fields="filterFields" v-model="filters" storage-key="test_groups" />
        </Drawer>
        ...
        <div class="mi-tabletoolbar">
            ...
            <div class="mi-tabletoolbar__right">
                <label class="mi-bar mi-bar--toolbar" :class="{ 'is-active': quickSearch }">
                    ...
                    <input v-model="quickSearch" class="mi-bar__input" :placeholder="$t('global.search_in', { item: $t('test_groups.singular').toLowerCase() })" autocomplete="off" spellcheck="false" type="text" />
                ...
                <Tooltip v-if="can('test_groups.export')" :title="$t('global.export_hint')" data-tour="export-import">
                ...
                <Dropdown :trigger="['click']" placement="bottomRight">
                    ...
                    <template #overlay>
                        <Menu>
                            <MenuItem v-if="can('test_groups.create') && can('test_groups.import') && canUsePlanFeature('imports')" key="import" @click="importOpen = true"><UploadOutlined /> {{ $t('global.import') }}</MenuItem>
                            <MenuItem v-if="can('test_groups.edit') && canUsePlanFeature('edit_all')" key="editall" @click="router.visit(route('lab_management.test_groups.edit_all'))"><FormOutlined /> {{ $t('global.edit_all') }}</MenuItem>
                            <MenuItem v-if="isSuper" key="trash" @click="router.visit(route('lab_management.test_groups.trash'))"><InboxOutlined /> {{ $t('global.view_deleted') }}</MenuItem>
        ...
        <Card :bodyStyle="{ padding: 0 }" class="grid-card">
            ...
            <ResponsiveTable
                :loading="isFetching"
                :dataSource="test_groups.data"
                :columns="columns"
                :pagination="tablePagination"
                :row-selection="(can('test_groups.delete') || can('test_groups.edit')) ? rowSelection : null"
            ...
            >
                ...
                <template #bodyCell="{ column, record, text, isMobile, compact }">
                    <TestGroupsFavoriteCell
                        v-if="column.key === 'favorite'"
                        :record="record"
                        :submitting="favoriteSubmitting"
                        :data-tour="record === test_groups.data[0] ? 'favorites' : null"
                    ...
                    <!-- Cuántas pruebas cuelgan del grupo. El 0 va en gris: es
                         justo el caso que dice "este grupo se puede borrar". -->
                    <template v-else-if="column.key === 'tests_count'">
                        <span :class="{ muted: !record.tests_count }">{{ record.tests_count ?? 0 }}</span>
                    </template>
        ...
        <TestGroupsBulkDeleteModal
            v-model:open="bulkOpen"
            v-model:reason="bulkReason"
            :count="selectedRowKeys.length"
            :submitting="bulkSubmitting"
            :error-msg="bulkError"
            :resource-label="selectedRowKeys.length === 1 ? $t('test_groups.record') : $t('test_groups.records')"
        ...
        <ExportDialog
            ...
            :default-title="$t('test_groups.export_title')"
            :endpoints="exportEndpoints"
            :limits="exportLimits"
            :total-rows="test_groups.total ?? 0"
            :total-unfiltered="test_groups.total_unfiltered ?? test_groups.total ?? 0"
        />
