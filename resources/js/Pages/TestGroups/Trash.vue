...
import TestGroupsTrashBulkBar from '@/Components/TestGroups/TestGroupsTrashBulkBar.vue';
import TestGroupsForceDeleteModal from '@/Components/TestGroups/TestGroupsForceDeleteModal.vue';

import { useModuleRestore } from '@/Composables/useModuleRestore';
import { useDateFormat } from '@/Composables/useDateFormat';
import { useI18n } from '@/Plugins/i18n';
import { testGroupsTrashColumns } from './config/trashColumns';
...
if (!isSuper) {
    router.visit(route('lab_management.test_groups.index'));
}
...
const {
    ...
    restoreRouteName:     'lab_management.test_groups.restore',
    bulkRestoreRouteName: 'lab_management.test_groups.bulk_restore',
});
...
const openForceDelete = (testGroup) => {
    forceDeleteTarget.value = testGroup;
...
const submitForceDelete = () => {
    ...
    forceDeleteErrors.value = {};
    router.delete(
        route('lab_management.test_groups.force_delete', forceDeleteTarget.value.slug),
        {
...
const columns = computed(() => testGroupsTrashColumns(t));
...
const subtitle = computed(() => {
    const word = props.test_groups.total === 1 ? t('global.record') : t('global.records');
    return `${props.test_groups.total} ${word} · ${t('global.super_only')}`;
});
</script>

<template>
    <Head :title="$t('global.view_deleted') + ' — ' + $t('test_groups.plural')" />

    <div v-if="isSuper" class="sap-form trash-page">
        <SectionHeader
            :back-href="route('lab_management.test_groups.index')"
            :title="$t('global.view_deleted') + ' — ' + $t('test_groups.plural')"
            :subtitle="subtitle"
        ...
        <Card :bodyStyle="{ padding: 0 }" class="grid-card">
                    ...
                    <template v-else-if="column.key === 'actions'">
                        <Space :size="4">
                            <Popconfirm
                                ...
                                :description="$t('test_groups.restore_hint')"
            ...
            <Empty
                v-if="test_groups.data.length === 0"
                :description="$t('global.no_deleted_records')"
                style="padding: 48px 16px"
            />
        </Card>

        <TestGroupsTrashBulkBar
