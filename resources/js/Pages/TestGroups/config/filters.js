...
/**
 * Filtros del módulo Grupos de pruebas. El catálogo tiene tres filas
 * (Físico Químico, Cromatografía, Otros), así que alcanza con nombre, código y
 * estado: un filtro por cantidad de pruebas sería ruido sobre tres registros.
 */
export const testGroupsFilterFields = (t) => [
    { key: 'name',           label: t('test_groups.filter_name'), type: 'tags' },
    { key: 'code',           label: t('test_groups.code'),        type: 'text' },
    { key: 'is_active',      label: t('test_groups.is_active'),   type: 'select', options: [
...
export const testGroupsEmptyFilters = () => ({
    name: [],
    code: '',
    is_active: null,
    created_at: null,
...
export const hydrateTestGroupsFilters = (server) => ({
...
export const testGroupsFiltersToQuery = (f) => ({
...
export const testGroupsFiltersSummary = (f, t) => {
    const parts = [];
    if (f.name?.length)        parts.push(`${t('test_groups.filter_name')}: ${f.name.join(', ')}`);
    if (f.code)                parts.push(`${t('test_groups.code')}: ${f.code}`);
    if (f.is_active !== null && f.is_active !== undefined) {
        parts.push(`${t('test_groups.is_active')}: ${f.is_active ? t('global.active') : t('global.inactive')}`);
    }
