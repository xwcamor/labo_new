...
<head>
    ...
    <style>
        ...
        .cal-valid    { color: #1D7044; font-weight: bold; }
        .cal-due-soon { color: #B45309; font-weight: bold; }
        .cal-expired  { color: #C8281D; font-weight: bold; }
        .cal-unknown  { color: #6A6D70; }
...
<body>
    ...
    @php
        $headings = [
            ...
            'calibrated_at'           => __('instruments.calibrated_at'),
            'calibration_due_at'      => __('instruments.calibration_due_at'),
            'calibration_status'      => __('instruments.calibration_status'),
            'calibration_certificate' => __('instruments.calibration_certificate'),
            'location'   => __('instruments.location'),
            'notes'      => __('instruments.notes'),
        ...
        // El semáforo de calibración se pinta también en el PDF: un listado
        // impreso para auditoría sin marcar los equipos vencidos obliga a
        // cotejar 200 fechas a mano.
        $calClass = [
            'expired'  => 'cal-expired',
            'due_soon' => 'cal-due-soon',
            'valid'    => 'cal-valid',
            'unknown'  => 'cal-unknown',
        ];
    @endphp
                        ...
                        @foreach ($columns as $col)
                            <td>
                                @switch($col)
                                    ...
                                    @case('calibrated_at')      {{ $item->calibrated_at?->format('Y-m-d') }} @break
                                    @case('calibration_due_at') {{ $item->calibration_due_at?->format('Y-m-d') }} @break
                                    @case('calibration_status')
                                        <span class="{{ $calClass[$item->calibration_status] ?? '' }}">
