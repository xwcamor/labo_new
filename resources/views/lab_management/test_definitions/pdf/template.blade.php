...
<body>
    ...
    @php
        $headings = [
            ...
            'group'      => __('test_definitions.group'),
            'container'  => __('test_definitions.container'),
            'chart_unit' => __('test_definitions.chart_unit'),
            'has_control'        => __('test_definitions.has_control'),
            'requires_control'   => __('test_definitions.requires_control'),
            'requires_duplicate' => __('test_definitions.requires_duplicate'),
            'is_grouped'         => __('test_definitions.is_grouped'),
            'replicates'  => __('test_definitions.replicates'),
            'description' => __('test_definitions.description'),
            'legacy_id'   => __('test_definitions.legacy_id'),
        ...
        // Las banderas se imprimen como Sí/No: un 1/0 en un informe de papel no
        // lo lee nadie.
        $flag = fn ($v) => __($v ? 'global.yes' : 'global.no');
    @endphp
    ...
    @else
        <table class="data">
            ...
            <tbody>
                @foreach ($test_definitions as $item)
                    <tr>
                        @foreach ($columns as $col)
                            <td>
                                @switch($col)
                                    ...
                                    @case('group')      {{ $item->group?->name ?? '' }} @break
                                    @case('has_control')        {{ $flag($item->has_control) }} @break
                                    @case('requires_control')   {{ $flag($item->requires_control) }} @break
                                    @case('requires_duplicate') {{ $flag($item->requires_duplicate) }} @break
                                    @case('is_grouped')         {{ $flag($item->is_grouped) }} @break
