...
class EquipmentTypeCrudTest extends TestCase
{
    ...
    protected function setUp(): void
    {
        ...
        foreach (['equipment_types.view', 'equipment_types.create', 'equipment_types.edit', 'equipment_types.delete', 'equipment_types.show'] as $perm) {
    ...
    {
        $response = $this->post(route('business_management.equipment_types.store'), [
            'name' => 'Éster Natural', 'code' => 'ester_natural', 'is_active' => true,
        ]);

        $response->assertRedirect();
        $response->assertSessionHasNoErrors();
        $this->assertDatabaseHas('equipment_types', ['name' => 'Éster Natural', 'code' => 'ester_natural']);
    }

    public function test_admin_can_update_brand(): void
    {
        $oil = EquipmentType::create(['name' => 'Prueba', 'code' => 'prueba', 'is_active' => true]);

        $response = $this->put(route('business_management.equipment_types.update', $oil->slug), [
            'name' => 'Prueba Editada', 'code' => 'prueba', 'is_active' => false,
        ]);

        $response->assertRedirect();
        $response->assertSessionHasNoErrors();
        $this->assertDatabaseHas('equipment_types', ['id' => $oil->id, 'name' => 'Prueba Editada', 'is_active' => false]);
    }

    public function test_name_is_unique_globally(): void
    {
        EquipmentType::create(['name' => 'Mineral', 'code' => 'mineral', 'is_active' => true]);
    ...
    {
        EquipmentType::create(['name' => 'Mineral', 'code' => 'mineral', 'is_active' => true]);
        $other = EquipmentType::create(['name' => 'Silicona', 'code' => 'silicona', 'is_active' => true]);

        $response = $this->put(route('business_management.equipment_types.update', $other->slug), [
    ...
    public function test_import_crea_y_deduplica_global_sin_tenant(): void
    {
        ...
        $importer = new \App\Imports\BusinessManagement\EquipmentTypes\EquipmentTypesImport('update_or_create', false);
        ...
        $this->assertDatabaseHas('equipment_types', ['name' => 'Girasol']);
        // No existe columna tenant_id: si el import la tocara, esto reventaria.
        $this->assertCount(2, EquipmentType::all());
    }
    ...
    public function test_import_maneja_code_y_deduplica_por_code(): void
    {
        ...
        $this->assertDatabaseHas('equipment_types', ['name' => 'Silicona', 'code' => 'silicona']);
        $this->assertDatabaseMissing('equipment_types', ['name' => 'Otro']);
    }
}
